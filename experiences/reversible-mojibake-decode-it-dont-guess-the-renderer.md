...
**۵) در هر ورودی ببندش — و **همهٔ** ورودی‌ها را بشمار.**
تقریباً همیشه بیش از یک مسیر وجود دارد: یکی که «متنِ خوانا» برمی‌گرداند و یکی که
سند را مستقیم به مدل می‌دهد و JSON می‌گیرد. بستنِ اولی حسِ کاذبِ امنیت می‌دهد
در حالی که دومی همچنان دادهٔ خراب را **مستقیم در رکوردهای دیتابیس** می‌نویسد.

**۶) برای دادهٔ موجود، ترمیمِ روی READ + روی WRITE را به مهاجرتِ دسته‌جمعی ترجیح بده.**
- روی **read**: هر رکورد پیش از رسیدن به کاربر ترمیم می‌شود ⇒ **همه‌چیز فوراً
  درست دیده می‌شود**، بدونِ یک UPDATEِ انبوهِ برگشت‌ناپذیر روی دادهٔ واقعی.
- روی **write**: هر رکورد در اولین ذخیره برای همیشه درست می‌شود.
- این جفت، همان نتیجهٔ مهاجرت را می‌دهد با کسری از ریسکش — و اگر تشخیص اشتباه
  از آب دربیاید، فقط کدِ ترمیم را برمی‌گردانی، نه دادهٔ نوشته‌شده را.

**۷) خودکار کن، نه دکمه‌ای.** کاربر نباید برای درست‌دیدنِ دادهٔ خودش چیزی را
پیدا کند و بزند. ولی ترمیمِ خودکار **باید ارزان باشد**: یک پیش‌فیلترِ regex روی
رشتهٔ خام قبل از هر کارِ DOM، وگرنه در هر کلیدفشاری کلِ سند پارس می‌شود.
...
- **ترمیم باید idempotent باشد** — و اگر خودکار است و state را عوض می‌کند،
  idempotent نبودن یعنی **حلقهٔ بی‌نهایتِ رندر**. تستِ «نقطهٔ ثابت» بنویس.
- **گلوگاهِ ظاهراً یگانه معمولاً یگانه نیست** — همهٔ مسیرهایی که این داده را وارد
  سیستم می‌کنند فهرست کن، نه فقط آن یکی که باگ را دیدی.
