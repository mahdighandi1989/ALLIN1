---
task_id: 616e241a-619a-4e56-92d1-8c535af13058
...
created_at: '2026-06-14T16:40:40.486082+00:00'
updated_at: '2026-06-14T16:40:40.486084+00:00'
target_files:
...
## 📋 شرح
در `backend/app/services/telegram.py` (line 689) یک exception handler خاموش (مثل `except: pass`) در مسیر اصلی کد پیدا شد.
...
- `backend/app/services/telegram.py:689`
...
- `backend/app/config.py` — `telegram.py` این فایل را import می‌کند
- `backend/app/routers/settings.py` — `telegram.py` این فایل را import می‌کند
- `backend/app/models/system_setting.py` — `telegram.py` این فایل را import می‌کند
- `backend/app/database.py` — `telegram.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `telegram.py` را import می‌کند (caller)
- `backend/app/services/expiry.py` — این فایل `telegram.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/services/telegram.py`
- `ruff check backend/app/services/telegram.py`
- `pytest -x`
...
1. نوع exception specific شده (نه bare except/catch)
2. log با level مناسب (warning/error) + context کامل اضافه شد
3. تست unit برای edge case شکست‌خورده عبور می‌کند
4. اگر failure قابل recovery است، fallback مستند شده
