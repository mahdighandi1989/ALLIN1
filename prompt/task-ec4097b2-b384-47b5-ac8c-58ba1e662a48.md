---
task_id: ec4097b2-b384-47b5-ac8c-58ba1e662a48
title: Inconsistent Python version requirement between `README.md` and `pyproject.toml`
type: docs
priority: medium
...
created_at: '2026-06-07T16:57:40.907067+00:00'
updated_at: '2026-06-07T16:57:40.907075+00:00'
target_files:
- pyproject.toml
---

# Inconsistent Python version requirement between `README.md` and `pyproject.toml`

## Raw Idea

The `README.md` clearly states 'Python 3.11+' as a prerequisite for the project. However, the `pyproject.toml` file, which defines the project's build system and dependencies, specifies `requires-python = ">=3.8"`. This discrepancy can lead to confusion for developers setting up the project, potentially causing them to use an older Python version that might not fully support all features or optimizations expected by the application. Aligning these declarations ensures clarity and consistency in the project's environment requirements.
...
- `pyproject.toml:10` — Update this line to reflect Python 3.11+.
  ```
  requires-python = ">=3.8"
  ```
...
Python project configuration (`pyproject.toml`), documentation.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `README.md` (سطر 60) — States the desired Python version.
- `backend/Dockerfile` — Should be checked to ensure it uses Python 3.11+.
- `.github/workflows/ci.yml` — CI environment should use Python 3.11+.

## 🌐 نقشهٔ وابستگی‌ها
This change affects the declared minimum Python version for the backend, ensuring consistency across project documentation and configuration.
...
- [ ] `pyproject.toml`'s `requires-python` field is set to `">=3.11"`.
- [ ] The `README.md` and `pyproject.toml` are consistent regarding the minimum Python version.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Update the `requires-python` field in `pyproject.toml` to `"=3.11"` or `">=3.11"` to match the `README.md`'s stated requirement of Python 3.11+.
...
## 🧪 دستورات اعتبارسنجی
- `grep -E "requires-python|Python 3.11+" pyproject.toml README.md`
...
1. `pyproject.toml`'s `requires-python` field is set to `">=3.11"`. _(verify: static)_
2. The `README.md` and `pyproject.toml` are consistent regarding the minimum Python version. _(verify: static)_
