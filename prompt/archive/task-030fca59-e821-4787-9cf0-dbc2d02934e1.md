---
task_id: 030fca59-e821-4787-9cf0-dbc2d02934e1
title: 'رسیدگی به endpoint بلااستفاده: GET /export.csv'
type: audit
priority: medium
...
created_at: '2026-05-31T16:21:46.590577+00:00'
...
# رسیدگی به endpoint بلااستفاده: GET /export.csv
...
---
[scan #2 at 2026-05-31T16:21:46.872509+00:00]
## 📋 شرح
endpoint `GET /export.xlsx` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/export.xlsx`
- فایل: `backend/app/routers/customers.py`
- علت: no frontend fetch + no recent call 
---
[scan #3 at 2026-05-31T16:21:46.998645+00:00]
## 📋 شرح
endpoint `GET /stats/summary` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-31T16:21:47.121659+00:00]
## 📋 شرح
endpoint `GET /{customer_id}/facilities` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{customer_id}/facilities`
- فایل: `backend/app/routers/customers.py`
- علت: no fronte
---
[scan #5 at 2026-05-31T16:21:47.244023+00:00]
## 📋 شرح
endpoint `GET /{customer_id}/detail` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{customer_id}/detail`
- فایل: `backend/app/routers/customers.py`
- علت: no frontend fetch
---
[scan #6 at 2026-05-31T16:21:47.363735+00:00]
## 📋 شرح
endpoint `GET /{customer_id}` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `GET /export.csv` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
