---
task_id: 9f1a6790-aa39-42a4-8ac6-14b8d62c4ce8
title: Integrate Facility Authorization in Facilities Router
type: logic_audit
priority: critical
...
created_at: '2026-05-31T16:19:35.340122+00:00'
updated_at: '2026-06-05T19:06:34.342982+00:00'
archived: true
archived_at: '2026-06-05T19:06:27.532967+00:00'
---

# Integrate Facility Authorization in Facilities Router
...
The `backend/app/routers/facilities.py` router explicitly states 'requiring user authentication' and uses `app.utils.security.get_current_user`. This ensures the user is authenticated. However, the description does not mention any mechanism for *authorization* beyond authentication, specifically for controlling *which* facilities an authenticated user is allowed to view (e.g., based on their organization, role, or specific permissions). It only lists `app.services.audit` as an interaction, which

## 💥 پیامد (impact)
Authenticated users could potentially access facility data that they are not authorized to see, leading to data breaches, privacy violations, or compliance issues. This is a critical authorization bypass vulnerability for read operations.

## 🛠 پیشنهاد رفع اولیه
Implement explicit authorization checks within `backend/app/routers/facilities.py` or a dedicated service layer (e.g., `app.services.facility_authorization`) that takes the `Authenticated user` object and filters/validates access to facility data based on the user's roles, permissions, or associated entities (e.g., customer ID).
...
[منطق] Missing Fine-Grained Authorization for Data Retrieval in Facilities Router
...
### Step 1: بررسی و تحلیل ناسازگاری منطقی در pipeline auth برای facilities router
**Status:** `done` (100%)
**Scope:** این مرحله شامل تحلیل کامل کد موجود در `backend/app/routers/facilities.py` و وابستگی‌های آن (مانند `app.utils.security.get_current_user` و `app.services.audit`) است. هدف، شناسایی دقیق شکاف بین احراز هویت (authentication) و مجوز (authorization) برای دسترسی به facility data است. این مرحله شامل پیاده‌سازی هیچ تغییری نمی‌شود، بلکه صرفاً یک بررسی و مستندسازی از وضعیت فعلی است. نکته حیاتی: باید مشخص شود که آیا مکانیزم authorization از قبل وجود دارد یا خیر، و اگر وجود دارد، چرا ناقص است.
**Excerpt:**
```
The `backend/app/routers/facilities.py` router explicitly states 'requiring user authentication' and uses `app.utils.security.get_current_user`. This ensures the user is authenticated. However, the description does not mention any mechanism for *authorization* beyond authentication, specifically for controlling *which* facilities an authenticated user is allowed to view (e.g., based on their organization, role, or specific permissions). It only lists `app.services.audit` as an interaction, which
```

### Step 2: طراحی و پیاده‌سازی سرویس مجوزدهی facility_authorization
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک سرویس جدید به نام `app.services.facility_authorization` است. این سرویس باید تابعی (یا توابعی) ارائه دهد که با دریافت `Authenticated user` object و شناسه facility (یا سایر پارامترهای مورد نیاز)، تعیین کند که آیا کاربر مجاز به دسترسی به آن facility خاص است یا خیر. منطق مجوزدهی باید بر اساس نقش‌های کاربر (roles)، مجوزها (permissions)، یا موجودیت‌های مرتبط (مانند customer ID) پیاده‌سازی شود. نکته حیاتی: این سرویس باید مستقل از router باشد و قابلیت استفاده مجدد در سایر بخش‌های برنامه را داشته باشد. این مرحله شامل تغییر در router نیست.
**Excerpt:**
```
Implement explicit authorization checks within `backend/app/routers/facilities.py` or a dedicated service layer (e.g., `app.services.facility_authorization`) that takes the `Authenticated user` object and filters/validates access to facility data based on the user's roles, permissions, or associated entities (e.g., customer ID).
```

### Step 3: یکپارچه‌سازی سرویس مجوزدهی در facilities router
**Status:** `done` (100%)
**Scope:** این مرحله شامل تغییر در `backend/app/routers/facilities.py` برای استفاده از سرویس `facility_authorization` است. پس از احراز هویت کاربر با `get_current_user`، باید یک فراخوانی به تابع مجوزدهی سرویس جدید اضافه شود تا دسترسی کاربر به facility درخواستی (یا لیست facilities) بررسی شود. اگر کاربر مجاز نباشد، باید یک پاسخ HTTP 403 Forbidden برگردانده شود. نکته حیاتی: این تغییر باید برای تمام endpointهای مربوط به facilities (خواندن، نوشتن، به‌روزرسانی، حذف) اعمال شود، مگر اینکه در تحلیل مرحله 1 مشخص شود که برخی endpointها نیاز به مجوز متفاوت دارند.
**Excerpt:**
```
Implement explicit authorization checks within `backend/app/routers/facilities.py` or a dedicated service layer (e.g., `app.services.facility_authorization`) that takes the `Authenticated user` object and filters/validates access to facility data based on the user's roles, permissions, or associated entities (e.g., customer ID).
```

### Step 4: نوشتن تست‌های واحد برای سرویس facility_authorization
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد (unit tests) برای سرویس `app.services.facility_authorization` است. تست‌ها باید سناریوهای مختلف را پوشش دهند: کاربر مجاز با نقش مناسب، کاربر غیرمجاز با نقش نامناسب، کاربر بدون نقش، و موارد لبه (edge cases) مانند facility ناموجود یا کاربر ناشناس. نکته حیاتی: تست‌ها باید مستقل از router و دیتابیس واقعی باشند (با استفاده از mock).
**Excerpt:**
```
Implement explicit authorization checks within `backend/app/routers/facilities.py` or a dedicated service layer (e.g., `app.services.facility_authorization`) that takes the `Authenticated user` object and filters/validates access to facility data based on the user's roles, permissions, or associated entities (e.g., customer ID).
```

### Step 5: نوشتن تست‌های integration برای facilities router با مجوزدهی جدید
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های integration برای endpointهای `backend/app/routers/facilities.py` است که سرویس مجوزدهی یکپارچه شده را آزمایش می‌کند. تست‌ها باید از یک دیتابیس تستی واقعی (یا in-memory) استفاده کنند و سناریوهای زیر را پوشش دهند: کاربر مجاز می‌تواند facility خود را ببیند، کاربر غیرمجاز 403 دریافت می‌کند، کاربر admin می‌تواند همه facilities را ببیند (در صورت وجود چنین rule). نکته حیاتی: این تست‌ها باید فرآیند کامل احراز هویت و مجوزدهی را آزمایش کنند.
**Excerpt:**
```
Authenticated users could potentially access facility data that they are not authorized to see, leading to data breaches, privacy violations, or compliance issues. This is a critical authorization bypass vulnerability for read operations.
```

### Step 6: بررسی وابستگی‌های upstream و downstream و همگام‌سازی
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی کامل وابستگی‌های upstream (چیزهایی که این تسک به آن‌ها متکی است) و downstream (چیزهایی که به این تسک متکی هستند) است. برای upstream: اطمینان از وجود و صحت `get_current_user`، مدل User، و ساختار دیتابیس facilities. برای downstream: بررسی اینکه آیا هیچ سرویس یا router دیگری از `facilities.py` یا `facility_authorization` استفاده می‌کند و نیاز به به‌روزرسانی دارد. نکته حیاتی: این مرحله باید قبل از merge نهایی انجام شود تا از broken references جلوگیری شود.
**Excerpt:**
```
پیش از و حین تغییر، تمام وابستگی‌ها را در **چهار جهت** به‌طور **کامل و بدون هیچ خلاصه‌سازی** شناسایی و همگام کن: ۱. وابستگی‌های upstream (این تسک به چه چیزهایی متکی است): - چه فایل‌ها، توابع، کلاس‌ها، API endpoint ها، schema های دیتابیس، env vars، یا config هایی که این تسک نیاز دارد؟ - آیا قرار است چیزی را ویرایش/حذف کنی که جای دیگر (signature، رفتار، return type، side effect) از آن انتظار خاصی می‌رود؟ - اگر dependency جدیدی اضافه می‌کنی، آیا با dependencyهای موجود تداخل دارد (نسخه، compat، lock file)؟ ۲. وابستگی‌های downstream (چه چیزهایی به این تسک متکی‌اند؟)
```
