...
updated_at: '2026-06-07T20:18:13.871160+00:00'
archived: true
archived_at: '2026-06-07T20:18:13.871127+00:00'
target_files:
...
ادامه (دور 4): رفع 500، پیاده‌سازی OAuth و بهبود UI

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/app/api/endpoints/auth.py:20-40` — `login_access_token`
- `backend/app/services/google_auth_service.py:1-30` — `GoogleAuthService`
- `frontend/src/app/login/page.tsx:10-40` — `LoginPage`
- `backend/app/api/endpoints/users.py:15-25` — `read_users`
- `backend/app/routers/auth.py` — از evidence verifier در دور 3
- `backend/app/routers/google_auth.py` — از evidence verifier در دور 3
- `backend/app/services/google_oauth.py` — از evidence verifier در دور 3
- `backend/app/routers/users.py` — از evidence verifier در دور 3

## 🧭 هدف اصلی پروژه (از یادداشت کاربر)
(کاربر یادداشتی ثبت نکرده است)

## 🔍 Context و وضعیت فعلی
این پرامپت برای **دور 4** ادامهٔ کار است. verifier در دور قبلی نشان داد کار به‌طور کامل انجام نشده.

📋 وضعیت چک‌لیست مراحل (1/6 انجام‌شده) — پیشرفت کلی: **56%**:
...
✅ بخش‌هایی که در دور قبل انجام شد:
  - الگوی معماری Google OAuth با استفاده از لایه سرویس و قراردادهای Python/FastAPI در backend پیاده‌سازی شده است.
  - الگوی برداشت‌شده از پروژه مرجع با dependency و naming پروژه فعلی در backend سازگار است.
...
```json
{
  ...
  "remaining_parts": [
    "صفحه ورود بازطراحی شده و دکمه 'Login with Google' به دلیل تنظیمات `AUTH_DISABLED` در محیط deploy شده قابل مشاهده و تعامل نیست.",
    "قابلیت کلیک بر روی دکمه 'Login with Google' و هدایت به صفحه احراز هویت

🪜 اقدامات بعدی پیشنهادی verifier:
  - اطمینان حاصل شود که `AUTH_DISABLED` در محیط deploy شده `False` است تا صفحه ورود و دکمه Google Login قابل دسترسی باشند.
  - خطای 500 در endpoint `GET /api/users?page=1&page_size=100` دیباگ و رفع شود.
  - جریان کامل احراز هویت Google OAuth از UI تا دریافت توکن JWT در backend تست شود.
  - پس از رفع مشکلات فوق، ظاهر و عملکرد کلی ناوبری و صفحات فرانت‌اند مجدداً بررسی شود.
...
- [~] **مرحله 1: رفع خطای 500 در endpoint لیست کاربران (api/users)** — این مرحله شامل دیباگ و رفع خطای سرور 500 روی endpoint `api/users/?page=1&page_size=100` است که در کنسول مرورگر چندین بار تکرار می‌شود. باید علت خطا (مثلاً مشکل در pagination با پارامتر `page_size`، خطای کوئری دیتابیس، یا handler ناقص controller) شناسایی و اصلاح شود. خارج از این مرحله: تغییرات UI یا 
      _باقی‌مانده: خطای 500 در endpoint `GET /api/users?page=1&page_size=100` هنوز رفع نشده است._
...
- [~] **مرحله 3: بازطراحی صفحهٔ ورود و افزودن منوی ناوبری و نمایش گزینه‌های دیگر** — این مرحله شامل بهبود ظاهر و کارکرد صفحهٔ ورود است: صفحهٔ ورود فعلی جذاب نیست، گزینه‌های دیگر در آن دیده نمی‌شوند و منوی ناوبری ندارد. باید طراحی صفحهٔ ورود بهبود یابد، گزینه‌های دیگر (مثل ورود با Gmail که در مرحلهٔ ۲ اضافه شد) به‌وضوح نمایش داده شوند و یک منوی ناوبری (nav bar) اضافه شود. خارج از این
      _باقی‌مانده: بازطراحی صفحه ورود و منوی ناوبری به دلیل فعال بودن `AUTH_DISABLED` در محیط deploy شده قابل مشاهده و تعامل نیست._
- [~] **مرحله 4: تکمیل و رفع صفحات ناقص یا غیرفعال و دسته‌بندی صحیح آن‌ها** — این مرحله شامل بررسی، تکمیل و اصلاح صفحاتی است که کاربر گفته ناقص هستند، کار نمی‌کنند یا درست دسته‌بندی نشده‌اند. باید هر صفحهٔ معیوب شناسایی، تکمیل و در ساختار/دسته‌بندی منطقی قرار گیرد (مثلاً گروه‌بندی منطقی صفحات در منو). خارج از این مرحله: رفع خطای endpoint کاربران و طراحی بصری کلی. نکتهٔ حیاتی:
      _باقی‌مانده: بررسی و تکمیل صفحات ناقص یا غیرفعال و دسته‌بندی صحیح آن‌ها به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده نامشخص است._
- [~] **مرحله 5: بهبود ارتباط و انسجام بین اجزا و صفحات (information architecture)** — این مرحله شامل ساماندهی ارتباط بین اجزا و صفحات است؛ کاربر گفته ارتباط اجزا و صفحات به‌هم‌ریخته است. باید جریان ناوبری بین صفحات، لینک‌ها و انتقال‌ها منسجم و قابل پیش‌بینی شود (information architecture منظم، breadcrumb یا منوی یکپارچه). خارج از این مرحله: استایل بصری صرف و رفع backend. نکتهٔ حیاتی: 
      _باقی‌مانده: بهبود ارتباط و انسجام بین اجزا و صفحات به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده قابل ارزیابی نیست._
- [~] **مرحله 6: یکدست‌سازی و بهبود ظاهر کلی برنامه (visual consistency)** — این مرحله شامل رفع آشفتگی بصری کلی برنامه است؛ کاربر گفته از منظر ظاهری خیلی آشفته است. باید یک سیستم طراحی منسجم (رنگ‌ها، فاصله‌ها، تایپوگرافی، کامپوننت‌های مشترک) در سراسر صفحات اعمال شود تا ظاهر یکدست و تمیز شود. خارج از این مرحله: منطق backend و کارکرد صفحات. نکتهٔ حیاتی: consistency در همهٔ صفح
      _باقی‌مانده: یکدست‌سازی و بهبود ظاهر کلی برنامه به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده قابل ارزیابی نیست._

## Acceptance Criteria

1. [مرحله 1 — رفع خطای 500 در endpoint لیست کاربران (api/users)] باقی‌مانده: خطای 500 در endpoint `GET /api/users?page=1&page_size=100` هنوز رفع نشده است. _(verify: static)_
2. [مرحله 3 — بازطراحی صفحهٔ ورود و افزودن منوی ناوبری و نمایش گزینه‌های دیگر] باقی‌مانده: بازطراحی صفحه ورود و منوی ناوبری به دلیل فعال بودن `AUTH_DISABLED` در محیط deploy شده قابل مشاهده و تعامل نیست. _(verify: static)_
3. [مرحله 4 — تکمیل و رفع صفحات ناقص یا غیرفعال و دسته‌بندی صحیح آن‌ها] باقی‌مانده: بررسی و تکمیل صفحات ناقص یا غیرفعال و دسته‌بندی صحیح آن‌ها به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده نامشخص است. _(verify: static)_
4. [مرحله 5 — بهبود ارتباط و انسجام بین اجزا و صفحات (information architecture)] باقی‌مانده: بهبود ارتباط و انسجام بین اجزا و صفحات به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده قابل ارزیابی نیست. _(verify: static)_
5. [مرحله 6 — یکدست‌سازی و بهبود ظاهر کلی برنامه (visual consistency)] باقی‌مانده: یکدست‌سازی و بهبود ظاهر کلی برنامه به دلیل عدم دسترسی به UI کامل برنامه در محیط deploy شده قابل ارزیابی نیست. _(verify: static)_
6. صفحه ورود بازطراحی شده و دکمه 'Login with Google' به دلیل تنظیمات `AUTH_DISABLED` در محیط deploy شده قابل مشاهده و تعامل نیست. _(verify: static)_
7. قابلیت کلیک بر روی دکمه 'Login with Google' و هدایت به صفحه احراز هویت گوگل به دلیل عدم دسترسی به دکمه در UI کار نمی‌کند. _(verify: static)_
8. جریان کامل احراز هویت با گوگل و دریافت توکن JWT معتبر پس از بازگشت به برنامه در محیط deploy شده با خطا مواجه است. _(verify: static)_
9. endpoint `GET /api/users?page=1&page_size=100` همچنان خطای 500 برمی‌گرداند و لیست کاربران را نمایش نمی‌دهد. _(verify: static)_
10. ساختار کلی ناوبری و صفحات فرانت‌اند، به خصوص صفحه ورود، به دلیل مشکلات deploy شده کاملاً عملکردی و قابل ارزیابی نیست. _(verify: static)_
