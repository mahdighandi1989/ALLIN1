...
updated_at: '2026-06-05T20:29:57.004330+00:00'
archived: true
archived_at: '2026-06-05T20:29:57.004311+00:00'
tags:
- consolidated
- post_verify_merge
---
...
endpoint بک‌اند بلااستفاده: POST /register
...
## 📋 شرح
endpoint `POST /register` در `backend/app/routers/auth.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/register`
- فایل: `backend/app/routers/auth.py`
...
1. گام ۱: grep روی `/register` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 10
  id: 432e2afb-e2f9-488d-bd7f-51b7776bc9e0
  عنوان اصلی: رسیدگی به endpoint بلااستفاده GET /metrics
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/main.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /metrics` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=api_response] [verify_plan={"method": "GET", "path": "/metrics", "headers": null, "json_body": null, "expected_status": 404, "required_fields": null, "json_contains": null}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /metrics
...
- `backend/app/routers/auth.py` — `main.py` این فایل را import می‌کند
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /metrics` در `backend/app/main.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/metrics` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 10
  id: 030fca59-e821-4787-9cf0-dbc2d02934e1
  عنوان اصلی: رسیدگی به endpoint بلااستفاده: GET /export.csv
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/routers/customers.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /export.csv` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=static] [verify_plan={"grep_patterns": ["@router.get(\"/export.csv\")"], "files_hint": ["backend/app/routers/customers.py"]}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["/export.csv", "test_export_csv"], "files_hint": ["openapi.json", "tests/"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /export.csv
...
- `backend/app/database.py` — `customers.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `customers.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `customers.py` این فایل را import می‌کند
- `backend/app/models/offer_letter.py` — `customers.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `customers.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `customers.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /export.csv` در `backend/app/routers/customers.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/export.csv`
- فایل: `backend/app/routers/customers.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/export.csv` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/customers.py`
- `ruff check backend/app/routers/customers.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 10
  id: e2ad2e02-54b7-466c-8759-238ffc3bc938
  عنوان اصلی: تعیین تکلیف endpoint بلااستفاده GET /search/advanced
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/routers/facilities.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /search/advanced` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["\"/search/advanced\"", "def test_advanced_search"], "files_hint": ["backend/app/routers/facilities.py", "tests/", "openapi.json"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /search/advanced
...
- `backend/app/database.py` — `facilities.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `facilities.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `facilities.py` این فایل را import می‌کند
- `backend/app/schemas/facility.py` — `facilities.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `facilities.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `facilities.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /search/advanced` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/search/advanced`
- فایل: `backend/app/routers/facilities.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/search/advanced` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/facilities.py`
- `ruff check backend/app/routers/facilities.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 5 از 10
  id: c59f790f-8c02-40bc-b42e-bf1e610e89b1
  عنوان اصلی: تعیین تکلیف endpoint بلااستفاده: GET /customers/template
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/app/routers/imports.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /customers/template` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["- router.get(\"/customers/template\")", "- def test_get_customers_template", "- /customers/template:"], "files_hint": ["backend/app/routers/imports.py", "tests/", "backend/app/main]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /customers/template
...
- `backend/app/database.py` — `imports.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `imports.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `imports.py` این فایل را import می‌کند
- `backend/app/services/excel_import.py` — `imports.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `imports.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `imports.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /customers/template` در `backend/app/routers/imports.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/customers/template`
- فایل: `backend/app/routers/imports.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/customers/template` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/imports.py`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 6 از 10
  id: 4a3ee503-9ccb-40b1-96e7-df16b421ff67
  عنوان اصلی: بررسی و اقدام endpoint بلااستفاده GET /{offer_id}
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/app/routers/offer_letters.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /{offer_id}` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /{offer_id}

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/app/routers/offer_letters.py`
...
- `backend/app/database.py` — `offer_letters.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `offer_letters.py` این فایل را import می‌کند
- `backend/app/models/offer_letter.py` — `offer_letters.py` این فایل را import می‌کند
- `backend/app/schemas/offer_letter.py` — `offer_letters.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `offer_letters.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `offer_letters.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /{offer_id}` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{offer_id}`
- فایل: `backend/app/routers/offer_letters.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/{offer_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/offer_letters.py`
- `ruff check backend/app/routers/offer_letters.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 7 از 10
  id: c04cd97f-f37f-49c3-aa55-921ad44d3e41
  عنوان اصلی: تعیین تکلیف endpoint بلااستفاده: POST /{notification_id}/read
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/routers/notifications.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `POST /{notification_id}/read` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": [], "files_hint": ["openapi.json", "tests/"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: POST /{notification_id}/read
...
- `backend/app/database.py` — `notifications.py` این فایل را import می‌کند
- `backend/app/models/notification.py` — `notifications.py` این فایل را import می‌کند
- `backend/app/models/user.py` — `notifications.py` این فایل را import می‌کند
- `backend/app/utils/security.py` — `notifications.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `notifications.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `notifications.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `POST /{notification_id}/read` در `backend/app/routers/notifications.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- `ruff check backend/app/routers/notifications.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 8 از 10
  id: d3e73821-24f1-47bb-82ca-abf1153851f5
  عنوان اصلی: بررسی و اقدام برای endpoint بلااستفاده GET /portfolio/export.pdf
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/routers/reports.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /portfolio/export.pdf` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["def test_export_portfolio_pdf", "\"/portfolio/export.pdf\""], "files_hint": ["tests/", "backend/app/main.py", "docs/openapi.yaml"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /portfolio/export.pdf
...
- `backend/app/database.py` — `reports.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `reports.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `reports.py` این فایل را import می‌کند
- `backend/app/services/exporters.py` — `reports.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `reports.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `reports.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /portfolio/export.pdf` در `backend/app/routers/reports.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/portfolio/export.pdf`
- فایل: `backend/app/routers/reports.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/portfolio/export.pdf` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/reports.py`
- `ruff check backend/app/routers/reports.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 9 از 10
  id: 4faeeac0-19f1-4d74-b6c6-4c01e3d282e8
  عنوان اصلی: تعیین تکلیف endpoint بلااستفاده: POST /{entity}/{item_id}/restore
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: backend/app/routers/trash.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `POST /{entity}/{item_id}/restore` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=api_response] [verify_plan={"method": "POST", "path": "/users/123/restore", "headers": {"Content-Type": "application/json"}, "json_body": {"reason": "testing restore"}, "expected_status": 200, "required_fields": ["id", "status"]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["POST /{entity}/{item_id}/restore", "@router.post(\"/{entity}/{item_id}/restore\")", "def test_restore_"], "files_hint": ["backend/app/routers/trash.py", "backend/app/openapi.yaml",]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: POST /{entity}/{item_id}/restore
...
- `backend/app/database.py` — `trash.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `trash.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `trash.py` این فایل را import می‌کند
- `backend/app/models/offer_letter.py` — `trash.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `trash.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `trash.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `POST /{entity}/{item_id}/restore` در `backend/app/routers/trash.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{entity}/{item_id}/restore`
- فایل: `backend/app/routers/trash.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/{entity}/{item_id}/restore` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/trash.py`
- `ruff check backend/app/routers/trash.py`
- `pytest -x`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 10 از 10
  id: a2e64c3e-7b35-4423-865e-8bf85785a468
  عنوان اصلی: مدیریت endpoint بلااستفاده: GET /{user_id}
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/app/routers/users.py

📋 acceptance_criteria کامل:
  - مشخص شد endpoint `GET /{user_id}` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["-@router.get(\"/{user_id}\")", "-def read_user(user_id:", "-test_read_user", "-client.get(\"/some_user_id\")", "-  /{user_id}:"], "files_hint": ["backend/app/routers/users.py", "ba]
...
## 🎯 هدف (خلاصه ساختاریافته)
endpoint بک‌اند بلااستفاده: GET /{user_id}
...
- `backend/app/database.py` — `users.py` این فایل را import می‌کند
- `backend/app/models/user.py` — `users.py` این فایل را import می‌کند
- `backend/app/schemas/admin_user.py` — `users.py` این فایل را import می‌کند
- `backend/app/routers/auth.py` — `users.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `users.py` را import می‌کند (caller)
- `backend/app/routers/__init__.py` — این فایل `users.py` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
endpoint `GET /{user_id}` در `backend/app/routers/users.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{user_id}`
- فایل: `backend/app/routers/users.py`
- علت: no frontend fetch + no recent call in logs
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: grep روی `/{user_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
گام ۲: اگر internal است، در README یا OpenAPI tag `internal` بزن.
گام ۳: اگر منسوخ است، حذف کن (شامل تست‌ها).
...
- `ruff check backend/app/routers/users.py`
- `pytest -x`
...
- در commit message: `merged-from: f2a29c7b-7e8e-46c9-bb04-d991a88f6f3c, 432e2afb-e2f9-488d-bd7f-51b7776bc9e0, 030fca59-e821-4787-9cf0-dbc2d02934e1, e2ad2e02-54b7-466c-8759-238ffc3bc938, c59f790f-8c02-40bc-b42e-bf1e610e89b1, 4a3ee503-9ccb-40b1-96e7-df16b421ff67, c04cd97f-f37f-49c3-aa55-921ad44d3e41, d3e73821-24f1-47bb-82ca-abf1153851f5, 4faeeac0-19f1-4d74-b6c6-4c01e3d282e8, a2e64c3e-7b35-4423-865e-8bf85785a468`
...
1. مشخص شد endpoint `POST /register` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
2. اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف _(verify: manual_only)_
3. اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد _(verify: static)_
4. مشخص شد endpoint `GET /metrics` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
5. مشخص شد endpoint `GET /export.csv` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
6. مشخص شد endpoint `GET /search/advanced` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
7. مشخص شد endpoint `GET /customers/template` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
8. مشخص شد endpoint `GET /{offer_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
9. مشخص شد endpoint `POST /{notification_id}/read` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
10. مشخص شد endpoint `GET /portfolio/export.pdf` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
11. مشخص شد endpoint `POST /{entity}/{item_id}/restore` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
12. مشخص شد endpoint `GET /{user_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
...
⛔ **ممنوعات مطلق TO-DO**:
  ❌ ساختن TO-DO برای کاری که می‌توانستی خودت ان
```
...
— [merged] این بخش یک یادداشت هشداردهنده و راهنمای جامع برای مدل اجراکننده است. شامل دستورالعمل‌های دقیق برای خواندن کامل پرامپت، بررسی وابستگی‌ها در چهار جهت (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با قوانین سخت، و الزام commit+push فوری per-task. این بخش خود یک مرحله اجرایی نیست، بلکه چارچوب اجرای تمام مراحل بعدی را تعریف می‌کند. تمام قواعد این بخش برای اجرای صحیح کل پروژه الزامی است.
— [merged] این بخش یک یادداشت الزام‌آور و غیرقابل‌انصراف برای مدل اجراکننده است. شامل قواعد خواندن کامل پرامپت، ممنوعیت خلاصه‌سازی، الزام بررسی وابستگی‌ها در چهار جهت (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با آستانه ۲۰٪، و قانون commit+push فوری per-task. این بخش خودش یک مرحله اجرایی نیست، بلکه یک precondition برای تمام مراحل بعدی است. مدل اجراکننده موظف است این قواعد را در تمام مراحل بعدی رعایت کند.
— [merged] این بخش یک یادداشت هشداردهنده و راهنمای کلی برای مدل اجراکننده است. شامل دستورالعمل‌های خواندن دقیق پرامپت، ممنوعیت خلاصه‌سازی، بررسی وابستگی‌های چهارجهته (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با قوانین سخت، و الزام commit+push فوری per-task. این بخش خود یک مرحله اجرایی نیست، بلکه چارچوب اجرای تمام مراحل بعدی را تعیین می‌کند. هیچ کد یا تغییری در این بخش درخواست نشده است.
**Excerpt:**
```
## ⚠️ یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
...
### Step 9: یادداشت مهم برای مدل اجراکننده — دستورالعمل‌های الزامی پیش از اجرا
**Status:** `done` (100%)
**Scope:** این بخش یک یادداشت متا-دستورالعملی است که نحوهٔ اجرای کل پرامپت را تعیین می‌کند. شامل قوانین خواندن دقیق، ممنوعیت خلاصه‌سازی، الزام بررسی وابستگی‌های چهارجهته، مدیریت TO-DO، و قوانین commit/push است. این بخش خود یک مرحلهٔ اجرایی نیست، بلکه چارچوب اجرای تمام مراحل بعدی را تعریف می‌کند. تمام قواعد این بخش برای اجرای صحیح تسک اصلی (حذف و بهینه‌سازی Endpoints بلااستفاده) الزامی است.
— [merged] این بخش یک یادداشت هشداردهنده و راهنمای جامع برای مدل اجراکننده است. شامل قوانین سختگیرانه برای خواندن کامل پرامپت، شناسایی وابستگی‌ها در چهار جهت (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با لیست سفید و سیاه، و الزام commit+push فوری per-task. این بخش هیچ کد یا تغییری را مشخص نمی‌کند، بلکه نحوه اجرای صحیح تسک‌های بعدی را تعریف می‌کند. خارج از scope: پیاده‌سازی واقعی endpointها، تغییر فایل‌ها، یا هر اقدام اجرایی.
**Excerpt:**
```
## ⚠️ یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
...
### Step 10: حذف endpoint بلااستفاده GET /export.csv از backend/app/routers/customers.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint GET /export.csv از فایل backend/app/routers/customers.py است. خارج از scope: تغییرات در سایر فایل‌ها، اضافه کردن endpoint جایگزین، یا مستندسازی. نکته حیاتی: قبل از حذف، باید با grep روی کل کدبیس (شامل frontend) بررسی شود که هیچ فراخوانی به این endpoint وجود نداشته باشد.
**Excerpt:**
```
endpoint بک‌اند بلااستفاده: GET /export.csv
...
### Step 12: حذف endpoint بلااستفاده GET /search/advanced از facilities.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint GET /search/advanced از فایل backend/app/routers/facilities.py است. شامل حذف تابع مربوطه و هرگونه import یا متغیر محلی که فقط توسط این endpoint استفاده می‌شود. خارج از scope: تغییر در سایر فایل‌ها، به‌روزرسانی تست‌ها، یا مستندسازی endpoint به‌عنوان internal/admin.
**Excerpt:**
```
endpoint `GET /search/advanced` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
### Step 14: بررسی ریسک‌های حذف endpoint GET /customers/template با چک کردن logs
**Status:** `pending` (0%)
**Scope:** این بخش شامل بررسی ریسک‌های حذف endpoint GET /customers/template است. تمرکز بر اطمینان از عدم استفاده endpoint در production توسط cron jobs یا webhookهای خارجی است. اقدامات شامل بررسی Render logs یا nginx access logs برای ۳۰ روز گذشته است. این بخش مستقل است و وابستگی به تسک دیگری ندارد.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
حذف endpoint که فقط در production مصرف می‌شود (cron/webhook خارجی) باعث silent failure می‌شود. قبل از حذف، Render logs یا nginx access logs آخرین ۳۰ روز را چک کن.
...
### Step 15: حذف endpoint بلااستفاده GET /customers/template از backend/app/routers/imports.py
**Status:** `pending` (0%)
**Scope:** این مرحله صرفاً به حذف کامل endpoint GET /customers/template از فایل backend/app/routers/imports.py می‌پردازد. شامل بررسی frontend callها، لاگ‌ها، یا مستندسازی نیست. همچنین شامل حذف سایر endpointهای موجود در imports.py یا تغییر در فایل‌های دیگر نمی‌شود. نکته حیاتی: قبل از حذف، باید مطمئن شوید که هیچ import یا تابعی در داخل imports.py به این endpoint وابسته نیست (مثلاً تابعی که در endpoint استفاده شده در جای دیگر هم صدا زده نشود).
**Excerpt:**
```
endpoint `GET /customers/template` در `backend/app/routers/imports.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
### Step 17: بررسی ریسک‌های حذف endpoint GET /{offer_id} با چک کردن لاگ‌های production
**Status:** `pending` (0%)
**Scope:** این بخش شامل بررسی ریسک‌های حذف endpoint GET /{offer_id} است. تمرکز بر چک کردن لاگ‌های ۳۰ روز اخیر (Render logs یا nginx access logs) برای اطمینان از عدم مصرف endpoint در production توسط cron/webhook خارجی است. این مرحله صرفاً یک اقدام احتیاطی قبل از حذف endpoint است و شامل خود حذف یا تغییر کد نمی‌شود.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
حذف endpoint که فقط در production مصرف می‌شود (cron/webhook خارجی) باعث silent failure می‌شود. قبل از حذف، Render logs یا nginx access logs آخرین ۳۰ روز را چک کن.
...
### Step 18: حذف endpoint بلااستفاده GET /{offer_id} از offer_letters.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint GET /{offer_id} از فایل backend/app/routers/offer_letters.py است. خارج از scope: تغییر در database.py, models, schemas, main.py, __init__.py یا هر فایل دیگر. نکته حیاتی: قبل از حذف، باید با grep روی کل کدبیس (شامل frontend) تأیید شود که هیچ caller دیگری (fetch, axios, apiClient) به این endpoint وجود ندارد. اگر endpoint در logs اخیر (مثلاً ۳۰ روز) فراخوانی شده، باید به جای حذف، مستندسازی شود.
**Excerpt:**
```
endpoint `GET /{offer_id}` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
### Step 20: بررسی و تعیین تکلیف endpoint بلااستفاده POST /{notification_id}/read
**Status:** `pending` (0%)
**Scope:** این بخش شامل بررسی ریسک‌های حذف endpoint در production (بررسی لاگ‌های ۳۰ روز گذشته)، تعیین دسته‌بندی endpoint (orphan/internal/deprecated)، و انجام اقدام مناسب (اتصال مجدد، تگ internal، یا حذف) است. فایل‌های دخیل: backend/app/routers/notifications.py. وابستگی به تسک‌های دیگر ندارد.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
حذف endpoint که فقط در production مصرف می‌شود (cron/webhook خارجی) باعث silent failure می‌شود. قبل از حذف، Render logs یا nginx access logs آخرین ۳۰ روز را چک کن.
...
📋 acceptance_criteria کامل:
  - مشخص شد endpoint `POST /{notification_id}/read` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=manual_only]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static]
```

### Step 21: حذف endpoint بلااستفاده POST /{notification_id}/read از backend/app/routers/notifications.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint POST /{notification_id}/read از فایل backend/app/routers/notifications.py است. خارج از scope: تغییر در سایر endpointهای موجود در notifications.py، تغییر در مدل‌ها، دیتابیس، یا فرانت‌اند. نکته حیاتی: قبل از حذف، باید با grep تأیید شود که هیچ فراخوانی از فرانت‌اند یا backend دیگر به این endpoint وجود ندارد. همچنین باید بررسی شود که آیا این endpoint در مستندات API (در صورت وجود) ذکر شده است یا خیر.
**Excerpt:**
```
endpoint `POST /{notification_id}/read` در `backend/app/routers/notifications.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
### Step 23: بررسی و اقدام برای endpoint بلااستفاده GET /portfolio/export.pdf
**Status:** `done` (100%)
**Scope:** این بخش شامل بررسی endpoint GET /portfolio/export.pdf در فایل backend/app/routers/reports.py است. هدف تعیین دسته‌بندی آن (orphan/internal/deprecated) و انجام اقدام مناسب (اتصال مجدد، تگ internal، یا حذف) می‌باشد. قبل از حذف، باید لاگ‌های ۳۰ روز اخیر (Render logs یا nginx access logs) برای اطمینان از عدم مصرف در production بررسی شوند. اگر حذف انجام شود، تست‌های مربوطه و OpenAPI نیز باید به‌روزرسانی شوند.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
حذف endpoint که فقط در production مصرف می‌شود (cron/webhook خارجی) باعث silent failure می‌شود. قبل از حذف، Render logs یا nginx access logs آخرین ۳۰ روز را چک کن.
...
### Step 24: حذف endpoint بلااستفاده GET /portfolio/export.pdf از reports.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint GET /portfolio/export.pdf از فایل backend/app/routers/reports.py است. همچنین شامل بررسی و حذف وابستگی‌های مرتبط با این endpoint (مانند توابع کمکی در exporters.py که فقط برای این endpoint استفاده می‌شوند) می‌باشد. خارج از scope: تغییر سایر endpointهای موجود در reports.py، تغییر frontend، یا اضافه کردن endpoint جدید.
**Excerpt:**
```
endpoint بک‌اند بلااستفاده: GET /portfolio/export.pdf
...
### Step 26: بررسی ریسک‌های حذف endpoint POST /{entity}/{item_id}/restore با بررسی لاگ‌های production
**Status:** `done` (100%)
**Scope:** این بخش شامل بررسی ریسک‌های حذف endpoint مذکور است. تمرکز بر چک کردن لاگ‌های Render یا nginx access logs برای ۳۰ روز گذشته است تا از عدم استفاده endpoint در production (cron/webhook خارجی) اطمینان حاصل شود. این مرحله صرفاً یک بررسی پیش‌نیاز است و شامل حذف یا تغییر endpoint نمی‌شود. وابستگی به تسک دیگری ندارد.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
حذف endpoint که فقط در production مصرف می‌شود (cron/webhook خارجی) باعث silent failure می‌شود. قبل از حذف، Render logs یا nginx access logs آخرین ۳۰ روز را چک کن.
...
📋 acceptance_criteria کامل:
  - مشخص شد endpoint `POST /{entity}/{item_id}/restore` در کدام دسته است (orphan/internal/deprecated) [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - اقدام مناسب انجام شد: یا connection باز شد، یا تگ internal، یا حذف [verify_method=api_response] [verify_plan={"method": "POST", "path": "/users/123/restore", "headers": {"Content-Type": "application/json"}, "json_body": {"reason": "testing restore"}, "expected_status": 200, "required_fields": ["id", "status"]
  - اگر حذف شد، تست‌های مربوطه هم حذف شدند و OpenAPI به‌روز شد [verify_method=static] [verify_plan={"grep_patterns": ["POST /{entity}/{item_id}/restore", "@router.post(\"/{entity}/{item_id}/restore\")", "def test_restore_"], "files_hint": ["backend/app/routers/trash.py", "docs/openapi.yaml",]

📝 idea_prompt اصلی (بدون تغییر و بدون خلاصه‌سازی):
```

### Step 27: حذف endpoint بلااستفاده POST /{entity}/{item_id}/restore از trash.py
**Status:** `pending` (0%)
**Scope:** این بخش صرفاً به حذف endpoint POST /{entity}/{item_id}/restore از فایل backend/app/routers/trash.py می‌پردازد. شامل حذف کامل تابع (function) مربوطه و هرگونه import یا متغیر محلی که تنها توسط این endpoint استفاده می‌شود. خارج از scope: تغییر در callerها (main.py, __init__.py) یا سایر فایل‌های مرتبط (database.py, models/*) مگر اینکه import بی‌استفاده ایجاد شود. همچنین شامل مستندسازی یا جایگزینی با 410 Gone نیست.
**Excerpt:**
```
endpoint `POST /{entity}/{item_id}/restore` در `backend/app/routers/trash.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
### Step 29: حذف endpoint بلااستفاده GET /{user_id} از backend/app/routers/users.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل endpoint GET /{user_id} از فایل backend/app/routers/users.py است. شامل حذف تابع مربوطه و هرگونه import یا متغیر محلی که فقط برای این endpoint استفاده می‌شود. خارج از scope: تغییر در سایر endpointهای همان فایل، تغییر در schemaها، مدل‌ها، database، یا فایل‌های frontend. نکته حیاتی: قبل از حذف، باید با grep روی نام تابع و مسیر endpoint در کل کدبیس (شامل frontend و تست‌ها) بررسی شود که هیچ caller دیگری وجود نداشته باشد.
**Excerpt:**
```
endpoint `GET /{user_id}` در `backend/app/routers/users.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
