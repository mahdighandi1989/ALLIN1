---
task_id: ecb1f050-33ba-4da5-936c-c0b7784f3b46
title: عدم تطابق بین فیچرهای ذکر شده در README و کد پیاده‌سازی شده
...
created_at: '2026-05-31T16:19:59.600034+00:00'
updated_at: '2026-06-05T19:36:07.103101+00:00'
archived: true
archived_at: '2026-06-05T19:36:00.601283+00:00'
target_files:
- README.md
---

# عدم تطابق بین فیچرهای ذکر شده در README و کد پیاده‌سازی شده

## Raw Idea

README پروژه فیچرهای متعددی مانند 'Checklist System', 'Guarantor Management', 'Property & Deposit Tracking', 'KYC Management', 'AI Integration', 'Google Drive Sync', 'Personal Notes Panel', 'Document Expiry Alerts', 'Email Notifications' را فهرست کرده است. با بررسی فایل‌های پروژه (مدل‌ها، روت‌ها، سرویس‌ها و فرانت‌اند)، هیچ اثری از این فیچرها در کد یافت نشد. این شکاف بزرگ بین مستندات و پیاده‌سازی می‌تواند منجر به سردرگمی توسعه‌دهندگان و ذی‌نفعان شود و نشان‌دهنده عدم تکمیل یا شروع نشدن این فیچرهاست.
...
- `README.md:15-30` — `Features` — لیست فیچرهایی که باید با کد تطبیق داده شوند
  ```markdown
  ### Features
...
- `backend/app/models/__init__.py` (سطر 1) — مدل‌های موجود در پروژه
- `backend/app/routers/__init__.py` (سطر 1) — روت‌های موجود در پروژه
- `backend/app/services/__init__.py` (سطر 1) — سرویس‌های موجود در پروژه
- `frontend/src/app/layout.tsx` (سطر 1) — صفحات فرانت‌اند موجود در پروژه

## 🌐 نقشهٔ وابستگی‌ها
این یافته به کل پروژه مربوط می‌شود و نیازمند بررسی جامع تمام ماژول‌ها و مقایسه با README است.
...
- [ ] README فقط فیچرهای موجود در کد را فهرست کند
- [ ] برای هر فیچر پیاده‌سازی نشده یک issue در گیت‌هاب ایجاد شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. دو اقدام پیشنهاد می‌شود: 1) README را به‌روزرسانی کنید تا فقط فیچرهای واقعاً پیاده‌سازی شده را فهرست کند. 2) برای هر فیچر ذکر شده اما پیاده‌سازی نشده، یک issue در گیت‌هاب ایجاد کرده و به عنوان feature backlog علامت‌گذاری کنید.
...
_قبل:_
```
لیست بلندبالایی از فیچرهای پیاده‌سازی نشده
```

_بعد:_
```
لیست محدودی از فیچرهای واقعاً پیاده‌سازی شده
```
...
## 🧪 دستورات اعتبارسنجی
- `grep -r 'checklist\|guarantor\|property.*tracking\|kyc\|AI.*Integration\|Google.*Drive\|notes.*panel\`
...
1. README فقط فیچرهای موجود در کد را فهرست کند _(verify: static)_
2. برای هر فیچر پیاده‌سازی نشده یک issue در گیت‌هاب ایجاد شود _(verify: manual_only)_
