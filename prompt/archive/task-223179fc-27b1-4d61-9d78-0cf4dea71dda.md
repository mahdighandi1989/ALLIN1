---
task_id: 223179fc-27b1-4d61-9d78-0cf4dea71dda
title: Return Consistent 401 for Login Errors
type: logic_audit
priority: high
...
created_at: '2026-05-31T16:20:43.233826+00:00'
...
# Return Consistent 401 for Login Errors
...
### Step 1: رفع ناسازگاری منطقی در endpoint احراز هویت: بازگرداندن 401 یکسان برای همه خطاهای login
**Status:** `not_done` (0%)
**Scope:** این مرحله شامل اصلاح endpoint لاگین در فایل backend/app/routers/auth.py است به‌طوری که برای تمام موارد شکست لاگین (اعم از عدم وجود کاربر یا رمز عبور اشتباه) یک پاسخ 401 Unauthorized یکسان بازگرداند. این مرحله شامل موارد زیر است: (1) شناسایی و حذف هرگونه منطق شرطی که بین 'کاربر وجود ندارد' و 'رمز عبور اشتباه است' تمایز قائل می‌شود، (2) اطمینان از اینکه پاسخ HTTP status code برای همه موارد شکست 401 است (نه 404)، (3) اطمینان از اینکه بدنه پاسخ (response body) نیز اطلاعاتی که امکان تشخیص علت شکست را بدهد (مثل 'user not found' یا 'invalid credentials' به صورت مجزا) فاش نمی‌کند. خارج از scope این مرحله: تغییر endpoint‌های دیگر (ثبت‌نام، تغییر رمز عبور)، تغییر rate limiting، اضافه کردن logging داخلی (که نباید به کاربر برگردد)، یا تغییر schema دیتابیس. نکته حیاتی: این تغییر امنیتی است و باید با دقت انجام شود تا هیچ مسیر جانبی (side channel) مانند timing attack ایجاد نشود.
**Excerpt:**
```
The `backend/app/routers/auth.py` component lists `404 Not Found` as a possible output for 'authentication/authorization failures'. If this `404` is returned specifically when a user account does not exist during a login attempt, while a `401 Unauthorized` is returned for incorrect credentials for an existing user, it allows attackers to enumerate valid usernames/emails. ... The login endpoint should return a generic `401 Unauthorized` (or `400 Bad Request` for invalid input) for all failed login attempts, regardless of whether the user account exists or the password is incorrect. Avoid differentiating between 'user not found' and 'wrong password' in the HTTP status code or response body.
```
