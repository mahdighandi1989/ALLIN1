...
updated_at: '2026-06-04T23:58:32.076294+00:00'
archived: true
archived_at: '2026-06-04T23:58:32.076273+00:00'
tags:
- consolidated
- post_verify_merge
---
...
Facilities page has incomplete TypeScript code that will cause runtime errors
...
The file `frontend/src/app/facilities/page.tsx` contains only a fragment of TypeScript code (lines 1-25) that appears to be a partial error handling block for a Promise.allSettled result. The code is incomplete - it starts with `if (facilitiesResult.status === 'fulfilled')` without any function definition, imports, or component structure. This will cause a compilation
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 5
  id: aba75d33-2d5b-4db9-bff6-7cb72ad16272
  عنوان اصلی: Fix Facilities page API client and incomplete code
  اولویت اصلی: critical
  وضعیت verify قبلی: pending
  فایل‌های دخیل: frontend/src/app/facilities/page.tsx

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "tests/test_all_existing_features.py", "timeout_seconds": 120}]
  - linter بدون warning عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/test_frontend_lint.py", "timeout_seconds": 60}]
  - type-check موفق است [verify_method=backend_test] [verify_plan={"test_node": "tests/test_frontend_typecheck.py", "timeout_seconds": 60}]
...
## 🔍 Context و وضعیت فعلی
The file `frontend/src/app/facilities/page.tsx` contains only a code snippet (lines 1-25) that is not a complete React component. It references `facilitiesResult` and `setData` without any component definition, state declarations, or imports. The built output in `backend/static/facilities/index.html` shows a static page with a spinner and no data. The source code is essentially broken and cannot be compiled or executed.
...
## 🪜 مراحل اجرایی پیشنهادی
1. Rewrite the facilities page as a complete React component, similar to the customers page (`frontend/src/app/customers/page.tsx`), using the existing `facilitiesApi` from `@/lib/api` (if it exists) or creating the necessary API calls.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 5
  id: 549dcd8f-d1e5-4fad-a142-d552c5ee2f44
  عنوان اصلی: فعال‌سازی بارگذاری داده در صفحات Customers/Facilities استاتیک
  اولویت اصلی: high
  وضعیت verify قبلی: pending
  فایل‌های دخیل: -
...
📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "tests/", "timeout_seconds": 120}]
  - linter بدون warning عبور می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["eslint", "lint"], "files_hint": ["frontend/package.json", "frontend/.eslintrc*"]}]
  - type-check موفق است [verify_method=static] [verify_plan={"grep_patterns": ["tsc", "type-check", "typescript"], "files_hint": ["frontend/tsconfig.json", "frontend/package.json"]}]
...
## 🔍 Context و وضعیت فعلی
صفحات Customers (frontend/src/app/customers/page.tsx) و Facilities (frontend/src/app/facilities/page.tsx
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 5
  id: c57f3e29-1926-4349-8133-48b0a71000cd
  عنوان اصلی: Refactor over-engineered code
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: frontend/src/app/page.tsx

📋 acceptance_criteria کامل:
  - ریشه anti-pattern تشخیص داده شد [verify_method=static] [verify_plan={"grep_patterns": ["useState", "useEffect", "async/await", "try/catch", "router.push"], "files_hint": ["frontend/src/app/page.tsx"]}]
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["router.push", "//"], "files_hint": ["frontend/src/app/page.tsx"]}]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_edge_cases.py::test_redirect_edge_cases", "timeout_seconds": 60}]
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 5 از 5
  id: 1586f712-c484-4344-966c-05370e954288
  عنوان اصلی: رفع مسیر فرانت‌اند بلااستفاده /customers
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - تأیید شد که `/customers` orphan است (هیچ Link/router.push اشاره نمی‌کند) [verify_method=static] [verify_plan={"grep_patterns": ["href=\"/customers\"", "router\\.push.*/customers", "to=\"/customers\"", "navigate.*/customers"], "files_hint": ["frontend/src/**/*.{tsx,jsx,ts,js}"]}]
  - یا navigation link اضافه شد، یا route حذف/redirect شد [verify_method=static] [verify_plan={"grep_patterns": ["href=\"/customers\"", "router\\.push.*/customers", "to=\"/customers\"", "navigate.*/customers", "redirect.*/customers"], "files_hint": ["frontend/src/**/*.{tsx,jsx,ts,js}"]}]
  - تست navigation: کاربر بتواند به این صفحه (یا destination) برسد [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/customers"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "assert_visible", "selector": "[data-testid='customers-pa]
...
- در commit message: `merged-from: 589f3f1d-0170-40d4-92ec-d75796ff83e6, aba75d33-2d5b-4db9-bff6-7cb72ad16272, 549dcd8f-d1e5-4fad-a142-d552c5ee2f44, c57f3e29-1926-4349-8133-48b0a71000cd, 1586f712-c484-4344-966c-05370e954288`
...
7. تأیید شد که `/customers` orphan است (هیچ Link/router.push اشاره نمی‌کند) _(verify: static)_
8. یا navigation link اضافه شد، یا route حذف/redirect شد _(verify: static)_
9. تست navigation: کاربر بتواند به این صفحه (یا destination) برسد _(verify: ui_interaction)_
...
- `frontend/src/app/facilities/page.tsx:6`
