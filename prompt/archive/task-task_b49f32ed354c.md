...
updated_at: '2026-06-05T17:38:10.488046+00:00'
archived: true
archived_at: '2026-06-05T17:38:04.164988+00:00'
tags:
- consolidated
- post_verify_merge
---

# اصلاح و استانداردسازی پیکربندی و متغیرهای محیطی بک‌اند

## Raw Idea

🧬 این یک تسک تلفیقی است — از 3 تسک منفرد ساخته شده.
📌 دلیل تلفیق (rationale توسط AI): این تسک‌ها به مسائل حیاتی پیکربندی، شامل تعریف متغیرهای محیطی حساس (مانند SECRET_KEY)، رفع الگوهای ضدطراحی در پیکربندی و به‌روزرسانی فایل‌های پیکربندی برای مسائل امنیتی می‌پردازند. همه این موارد بر روی تنظیمات پایه و حیاتی بک‌اند متمرکز هستند.
🎯 theme: رفع مشکلات پیکربندی و متغیرهای محیطی بک‌اند
💎 estimated_difficulty: medium

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 3
  id: 372a0af1-2e15-446c-b9a6-50110c4b6618
  عنوان اصلی: تعریف متغیر SECRET_KEY در .env.example
  اولویت اصلی: critical
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/.env.example

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "pytest", "timeout_seconds": 120}]
  - linter بدون warning عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "pytest::linter_checks", "timeout_seconds": 60}]
  - type-check موفق است [verify_method=backend_test] [verify_plan={"test_node": "pytest::type_checks", "timeout_seconds": 60}]
...
متغیر محیطی SECRET_KEY در .env.example تعریف نشده

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/.env.example:1-10` — `SECRET_KEY` — باید SECRET_KEY=your-secret-key-here به فایل اضافه شود
  ```
  # فایل .env.example موجود است اما SECRET_KEY در آن نیست
  ```
...
- `backend/app/config.py` — کلاس Settings که متغیرهای

## 🔍 Context و وضعیت فعلی
فایل backend/.env.example شامل متغیر SECRET_KEY نیست، در حالی که backend/app/config.py و backend/app/utils/security.py به آن نیاز دارند. عدم وجود این متغیر باعث خطای 500 در زمان اجرا می‌شود. این یک مشکل امنیتی جدی است زیرا اگر برنامه با مقدار پیش‌فرض یا خالی اجرا شود، امضای JWT قابل پیش‌بینی خواهد بود.
...
1. متغیر SECRET_KEY را با یک مقدار پیش‌فرض placeholder به .env.example اضافه کنید و در config.py یک validation برای اجباری بودن آن در محیط production قرار دهید.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 3
  id: 9d72bad1-a60e-463a-8ae6-9fd9a30ff02b
  عنوان اصلی: Address Under-engineering Anti-pattern
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/app/config.py
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["DATABASE_URL = None", "DATABASE_URL = os.getenv\\(", "ACCESS_TOKEN_EXPIRE_MINUTES = (?!10080)\\d+", "#.*(security|production|override|local dev).*DATABASE_URL", "#.*(security|produ]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_config_security.py::test_production_config_defaults", "timeout_seconds": 60}]
...
- `backend/app/main.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/utils/security.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/services/notifications.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/utils/rate_limit.py` — این فایل `config.py` را import می‌کند (caller)

## 🔍 Context و وضعیت فعلی
The default `DATABASE_URL` uses generic `user:password` credentials. While intended for local development, this poses a significant security risk if accidentally deployed to a non-local environment without being overridden. It should ideally be `None` or require explicit configuration in production.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 3
  id: 4ba5c474-d136-4662-bc9f-2600ef034695
  عنوان اصلی: بررسی و بروزرسانی config.py برای security.py
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/app/config.py

📋 acceptance_criteria کامل:
  - بررسی شد `backend/app/utils/security.py` چه API ای از `backend/app/config.py` می‌خواهد [verify_method=static] [verify_plan={"grep_patterns": ["settings\\.JWT_ISSUER", "settings\\.JWT_AUDIENCE"], "files_hint": ["backend/app/utils/security.py"]}]
  - اگر API در `backend/app/config.py` نیست، اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["JWT_ISSUER", "JWT_AUDIENCE"], "files_hint": ["backend/app/config.py"]}]
  - تست integration که هر دو سمت را پوشش می‌دهد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_auth.py::test_jwt_token_creation_and_validation", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
⬆️ config.py ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در security.py
...
- `backend/app/utils/security.py` — consumer که تغییر کرده
- `backend/app/main.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/services/notifications.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/utils/rate_limit.py` — این فایل `config.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
`backend/app/utils/security.py` به `backend/app/config.py` وابسته است.

## 🔍 Context و وضعیت فعلی
## 📋 شرح
یک فایل تغییر کرده و حالا از API/behavior جدیدی استفاده می‌کند که upstream فعلی ممکن است پشتیبانی نکند.

## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `backend/app/utils/security.py`
- 📂 فایل upstream (producer — نیازمند update): `backend/app/config.py`
- 🔥 risk: **high**
- 📝 reason: The `create_access_token` function now attempts to use `settings.JWT_ISSUER` and `settings.JWT_AUDIENCE` for standard JWT claims. If these attributes are not defined in `app/config.py`, accessing them will result in an AttributeError, causing the application to fail when creating access tokens.
- 🛠 recommended: `needs_update`

## 🤔 چرا مهم است
اگر upstream متد/تابع/فیلد لازم را اضافه نکند، consumer در runtime شکست می‌خورد یا silent failure می‌دهد.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] بررسی شد `backend/app/utils/security.py` چه API ای از `backend/app/config.py` می‌خواهد
- [ ] اگر API در `backend/app/config.py` نیست، اضافه شد
- [ ] تست integration که هر دو سمت را پوشش می‌دهد عبور می‌کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: `backend/app/utils/security.py` را بخوان و usages جدید از `backend/app/config.py` را پیدا کن.
گام ۲: API contract در `backend/app/config.py` چک کن.
گام ۳: اگر contract ناسازگار است، `backend/app/config.py` را extend کن.
...
## ⚠️ ریسک‌ها و موارد احتیاط
تغییر `backend/app/config.py` ممکن است بقیه consumers را تحت تأثیر قرار دهد — قبل از تغییر، همهٔ مصرف‌کنندگانش را check کن.
...
- در commit message: `merged-from: 372a0af1-2e15-446c-b9a6-50110c4b6618, 9d72bad1-a60e-463a-8ae6-9fd9a30ff02b, 4ba5c474-d136-4662-bc9f-2600ef034695`
...
4. ریشه anti-pattern تشخیص داده شد _(verify: manual_only)_
5. یا کد اصلاح شد، یا کامنت توجیهی اضافه شد _(verify: static)_
6. تست edge case نوشته شد _(verify: backend_test)_
7. بررسی شد `backend/app/utils/security.py` چه API ای از `backend/app/config.py` می‌خواهد _(verify: static)_
8. اگر API در `backend/app/config.py` نیست، اضافه شد _(verify: static)_
9. تست integration که هر دو سمت را پوشش می‌دهد عبور می‌کند _(verify: backend_test)_

## Task Steps

### Step 1: اضافه کردن متغیر SECRET_KEY به فایل backend/.env.example
**Status:** `done` (100%)
**Scope:** این مرحله شامل افزودن یک خط جدید با متغیر SECRET_KEY و یک مقدار placeholder (مانند your-secret-key-here) به فایل backend/.env.example است. خارج از این مرحله: تغییر در config.py، security.py، یا هر فایل دیگر. نکته حیاتی: مقدار placeholder باید به وضوح نشان دهد که این یک مقدار موقتی است و باید در production تغییر کند.
**Excerpt:**
```
متغیر محیطی SECRET_KEY در .env.example تعریف نشده
- `backend/.env.example:1-10` — `SECRET_KEY` — باید SECRET_KEY=your-secret-key-here به فایل اضافه شود
...
### Step 2: اعتبارسنجی اجباری بودن SECRET_KEY در محیط production در backend/app/config.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل افزودن validation در کلاس Settings فایل backend/app/config.py است تا در محیط production (مثلاً با بررسی ENVIRONMENT=production) مقدار SECRET_KEY اجباری باشد و اگر خالی یا placeholder بود، خطا بدهد. خارج از این مرحله: تغییر در .env.example یا security.py. نکته حیاتی: validation باید فقط در production فعال باشد تا در development اذیت نکند.
**Excerpt:**
```
متغیر SECRET_KEY را با یک مقدار پیش‌فرض placeholder به .env.example اضافه کنید و در config.py یک validation برای اجباری بودن آن در محیط production قرار دهید.
```

### Step 3: اجرای تست‌های موجود برای اطمینان از عدم شکستن آن‌ها (pytest)
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستور `pytest` در ریشه پروژه و اطمینان از عبور تمام تست‌های موجود است. خارج از این مرحله: نوشتن تست جدید. نکته حیاتی: اگر تستی fail شد، باید قبل از ادامه رفع شود.
— [merged] این مرحله شامل اجرای دستور `pytest -x` (توقف در اولین شکست) و اطمینان از عبور تمام تست‌ها است. خارج از این مرحله: رفع تست‌های شکست‌خورده. نکته حیاتی: این یک بررسی نهایی است.
**Excerpt:**
```
اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "pytest", "timeout_seconds": 120}]
```

### Step 4: اجرای linter (ruff) برای اطمینان از عدم وجود warning
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستور `ruff check backend/app/config.py backend/.env.example` (یا دستور مشابه) و اطمینان از عبور بدون warning است. خارج از این مرحله: رفع warningهای موجود در فایل‌های دیگر. نکته حیاتی: اگر warning جدیدی اضافه شده، باید رفع شود.
**Excerpt:**
```
linter بدون warning عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "pytest::linter_checks", "timeout_seconds": 60}]
```

### Step 5: اجرای type-checker (mypy) برای اطمینان از صحت نوع‌ها
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستور `mypy backend/app/config.py` (یا دستور مشابه) و اطمینان از عبور موفق است. خارج از این مرحله: رفع خطاهای type-check در فایل‌های دیگر. نکته حیاتی: اگر type-check fail شد، باید رفع شود.
**Excerpt:**
```
type-check موفق است [verify_method=backend_test] [verify_plan={"test_node": "pytest::type_checks", "timeout_seconds": 60}]
```

### Step 6: تشخیص ریشه anti-pattern Under-engineering در backend/app/config.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل بازبینی دستی فایل backend/app/config.py برای یافتن الگوی ضدطراحی Under-engineering (مانند مقدار پیش‌فرض insecure برای DATABASE_URL) و مستندسازی آن است. خارج از این مرحله: اصلاح کد یا نوشتن تست. نکته حیاتی: این مرحله صرفاً تشخیص و مستندسازی است.
**Excerpt:**
```
ریشه anti-pattern تشخیص داده شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
```

### Step 7: اصلاح کد یا افزودن کامنت توجیهی برای anti-pattern Under-engineering
**Status:** `done` (100%)
**Scope:** این مرحله شامل اصلاح کد در backend/app/config.py (مثلاً تغییر DATABASE_URL به None در production) یا افزودن یک کامنت توجیهی واضح است که دلیل انتخاب طراحی فعلی را توضیح می‌دهد. خارج از این مرحله: نوشتن تست. نکته حیاتی: اگر کد اصلاح می‌شود، باید با consumers دیگر هماهنگ باشد.
**Excerpt:**
```
یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["DATABASE_URL = None", "DATABASE_URL = os.getenv\\\(", "ACCESS_TOKEN_EXPIRE_MINUTES = (?!10080)\\d+", "#.*(security|production|override|local dev).*DATABASE_URL", "#.*(security|produ]
```

### Step 8: نوشتن تست edge case برای production config defaults در tests/test_config_security.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن یک تست جدید در فایل tests/test_config_security.py با نام test_production_config_defaults است که بررسی می‌کند در محیط production، مقادیر پیش‌فرض ناامن (مانند DATABASE_URL با user:password) باعث خطا می‌شوند. خارج از این مرحله: تغییر در config.py. نکته حیاتی: تست باید با monkeypatch یا environ برای شبیه‌سازی production نوشته شود.
**Excerpt:**
```
تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_config_security.py::test_production_config_defaults", "timeout_seconds": 60}]
```

### Step 9: بررسی API مورد نیاز security.py از config.py با grep
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای grep روی فایل backend/app/utils/security.py برای یافتن تمام ارجاعات به settings (مانند settings.JWT_ISSUER, settings.JWT_AUDIENCE) و مستندسازی آن‌هاست. خارج از این مرحله: تغییر در config.py. نکته حیاتی: لیست کامل attributeهای مورد نیاز باید استخراج شود.
**Excerpt:**
```
بررسی شد `backend/app/utils/security.py` چه API ای از `backend/app/config.py` می‌خواهد [verify_method=static] [verify_plan={"grep_patterns": ["settings\\.JWT_ISSUER", "settings\\.JWT_AUDIENCE"], "files_hint": ["backend/app/utils/security.py"]}]
```

### Step 10: اضافه کردن attributeهای JWT_ISSUER و JWT_AUDIENCE به config.py در صورت عدم وجود
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی فایل backend/app/config.py برای وجود attributeهای JWT_ISSUER و JWT_AUDIENCE و اضافه کردن آن‌ها با مقادیر پیش‌فرض مناسب (مانند خالی یا None) در صورت عدم وجود است. خارج از این مرحله: تغییر در security.py. نکته حیاتی: attributeها باید با type hint و validator مناسب اضافه شوند.
**Excerpt:**
```
اگر API در `backend/app/config.py` نیست، اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["JWT_ISSUER", "JWT_AUDIENCE"], "files_hint": ["backend/app/config.py"]}]
```

### Step 11: نوشتن تست integration برای JWT token creation and validation در tests/integration/test_auth.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن یک تست integration جدید در فایل tests/integration/test_auth.py با نام test_jwt_token_creation_and_validation است که بررسی می‌کند با تنظیمات جدید config.py، توکن JWT به درستی ساخته و اعتبارسنجی می‌شود. خارج از این مرحله: تغییر در config.py یا security.py. نکته حیاتی: تست باید از create_access_token استفاده کند.
**Excerpt:**
```
تست integration که هر دو سمت را پوشش می‌دهد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_auth.py::test_jwt_token_creation_and_validation", "timeout_seconds": 60}]
```

### Step 12: اجرای py_compile روی backend/app/config.py برای بررسی syntax
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستور `python -m py_compile backend/app/config.py` و اطمینان از عدم وجود خطای syntax است. خارج از این مرحله: اجرای تست‌های دیگر. نکته حیاتی: این یک بررسی سریع است.
**Excerpt:**
```
`python -m py_compile backend/app/config.py`
```

### Step 13: اجرای ruff check روی backend/app/config.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستور `ruff check backend/app/config.py` و اطمینان از عدم وجود warning است. خارج از این مرحله: رفع warningهای موجود در فایل‌های دیگر. نکته حیاتی: اگر warning جدیدی اضافه شده، باید رفع شود.
**Excerpt:**
```
`ruff check backend/app/config.py`
```

### Step 14: گام ۲: بررسی API contract در backend/app/config.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل بازبینی فایل backend/app/config.py برای یافتن attributeهای JWT_ISSUER و JWT_AUDIENCE و مقایسه با نیازمندی‌های security.py است. خارج از این مرحله: تغییر در کد. نکته حیاتی: این مرحله صرفاً بررسی است.
**Excerpt:**
```
گام ۲: API contract در `backend/app/config.py` چک کن.
```

### Step 15: گام ۳: Extend کردن backend/app/config.py در صورت ناسازگاری contract
**Status:** `done` (100%)
**Scope:** این مرحله شامل افزودن attributeهای JWT_ISSUER و JWT_AUDIENCE به کلاس Settings در backend/app/config.py در صورتی که در گام ۲缺失 تشخیص داده شوند، است. خارج از این مرحله: تغییر در security.py. نکته حیاتی: attributeها باید با validator و type hint مناسب اضافه شوند.
**Excerpt:**
```
گام ۳: اگر contract ناسازگار است، `backend/app/config.py` را extend کن.
```

### Step 16: بررسی وابستگی‌های upstream، downstream، cross-tier و side artifacts برای تمام تغییرات
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی کامل وابستگی‌ها در چهار جهت (upstream, downstream, cross-tier, side) برای تمام تغییرات اعمال شده در مراحل قبل است. این شامل جستجوی grep برای call sites، importها، و referenceهای مستقیم و غیرمستقیم است. خارج از این مرحله: اعمال تغییرات جدید. نکته حیاتی: این مرحله باید قبل از commit نهایی انجام شود.
**Excerpt:**
```
