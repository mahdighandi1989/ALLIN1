...
updated_at: '2026-06-05T18:55:46.853989+00:00'
archived: true
archived_at: '2026-06-05T18:55:46.853960+00:00'
target_files:
...
عدم اعتبارسنجی ورودی در فرم ایجاد/ویرایش تسهیلات

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `frontend/src/app/facilities/page.tsx:351-369` — `handleSubmit` — بدون اعتبارسنجی پیش از ارسال
  ```tsx
...
Next.js 14 + React + TypeScript + Axios

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/routers/facilities.py` (سطر 1) — API endpoint که داده را دریافت می‌کند
- `backend/app/schemas/facility.py` (سطر 1) — Pydantic schema که validation سمت سرور را انجام می‌دهد

## 🌐 نقشهٔ وابستگی‌ها
این فرم توسط 2 مسیر استفاده می‌شود: ایجاد و ویرایش تسهیلات. هر دو از یک handleSubmit استفاده می‌کنند.
...
1. اضافه کردن اعتبارسنجی سمت کلاینت قبل از ارسال به API: (1) amount > 0، (2) interest_rate بین 0 تا 100، (3) expiry_date > امروز، (4) currency در لیست ارزهای مجاز. همچنین اضافه کردن try/catch اختصاصی برای هر فیلد.
...
_بعد:_
```
if (form.expiry_date && new Date(form.expiry_date) <= new Date()) { toast.error('Expiry date must be in the future'); return; }
```
...
- `pytest backend/tests/test_facilities.py -k validation`
...
**Excerpt:**
```
(1) amount می‌تواند صفر یا منفی باشد
```
...
**Excerpt:**
```
(2) interest_rate می‌تواند خارج از بازه 0-100 باشد
```
...
**Excerpt:**
```
(3) expiry_date می‌تواند قبل از start_date باشد
```
...
**Excerpt:**
```
(4) currency می‌تواند کد نامعتبر ISO 4217 باشد
```
