---
task_id: e2ad2e02-54b7-466c-8759-238ffc3bc938
title: تعیین تکلیف endpoint بلااستفاده GET /search/advanced
type: audit
priority: medium
...
created_at: '2026-05-31T16:21:47.740534+00:00'
...
# تعیین تکلیف endpoint بلااستفاده GET /search/advanced
...
---
[scan #2 at 2026-05-31T16:21:47.994421+00:00]
## 📋 شرح
endpoint `GET /{facility_id}/detail` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/{facility_id}/detail`
- فایل: `backend/app/routers/facilities.py`
- علت: no frontend fet
---
[scan #3 at 2026-05-31T16:21:48.135141+00:00]
## 📋 شرح
endpoint `GET /{facility_id}` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-31T16:21:48.290916+00:00]
## 📋 شرح
endpoint `PUT /{facility_id}` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fetch + no recent
---
[scan #5 at 2026-05-31T16:21:48.423416+00:00]
## 📋 شرح
endpoint `DELETE /{facility_id}` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #6 at 2026-05-31T16:21:48.557988+00:00]
## 📋 شرح
endpoint `POST /{facility_id}/restore` در `backend/app/routers/facilities.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{facility_id}/restore`
- فایل: `backend/app/routers/facilities.py`
- علت: no frontend
...
1. مشخص شد endpoint `GET /search/advanced` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
