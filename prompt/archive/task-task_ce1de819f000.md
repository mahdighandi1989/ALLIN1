---
task_id: task_ce1de819f000
title: همگام‌سازی bcrypt و تطبیق تست‌های واحد
type: other
priority: high
...
created_at: '2026-06-05T15:53:09.547599+00:00'
updated_at: '2026-06-05T19:26:36.748530+00:00'
archived: true
archived_at: '2026-06-05T19:26:30.981786+00:00'
tags:
- consolidated
- post_verify_merge
---

# همگام‌سازی bcrypt و تطبیق تست‌های واحد

## Raw Idea

🧬 این یک تسک تلفیقی است — از 2 تسک منفرد ساخته شده.
📌 دلیل تلفیق (rationale توسط AI): این تسک‌ها شامل به‌روزرسانی وابستگی‌های پروژه (مانند نسخه bcrypt) و اطمینان از همگام بودن تست‌ها با تغییرات مدل‌های داده‌ای هستند. این اقدامات برای حفظ سلامت و پایداری پروژه ضروری‌اند.
🎯 theme: همگام‌سازی وابستگی‌ها و بروزرسانی تست‌ها
💎 estimated_difficulty: small

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 2
  id: 496b4ca8-39d9-4701-b640-c068952b45bf
  عنوان اصلی: همگام‌سازی نسخه bcrypt در وابستگی‌ها
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: pyproject.toml

📋 acceptance_criteria کامل:
  - pyproject.toml دقیقاً bcrypt==4.0.1 را مشخص کند [verify_method=static] [verify_plan={"grep_patterns": ["bcrypt = \"==4.0.1\""], "files_hint": ["pyproject.toml"]}]
  - نصب وابستگی‌ها با pip install -e . بدون خطا انجام شود [verify_method=backend_test] [verify_plan={"test_node": "tests/test_dependencies.py::test_install_dependencies_no_error", "timeout_seconds": 120}]
  - تابع hash_password در security.py بدون خطا کار کند [verify_method=backend_test] [verify_plan={"test_node": "tests/test_security.py::test_hash_password_functionality", "timeout_seconds": 30}]
...
عدم تطابق نسخه bcrypt بین requirements.txt و pyproject.toml

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `pyproject.toml:35` — `bcrypt` — باید به bcrypt==4.0.1 تغییر کند
  ```
  bcrypt>=4.0,<4.1
  ```
...
Python 3.8+ / FastAPI / passlib 1.7.4 / bcrypt

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `requirements.txt` (سطر 12) — نسخه صحیح bcrypt==4.0.1 در این فایل مشخص شده
- `backend/app/utils/security.py` — از passlib برای هش کردن پسورد استفاده می‌کند که به bcrypt وابسته است

## 🌐 نقشهٔ وابستگی‌ها
این تغییر روی تمام محیط‌هایی که از pyproject.toml برای نصب وابستگی‌ها استفاده می‌کنند (مانند CI/CD, Docker) تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
در requirements.txt نسخه bcrypt==4.0.1 پین شده است، اما در pyproject.toml محدوده 'bcrypt>=4.0,<4.1' تعریف شده. این ناسازگاری می‌تواند در محیط‌های مختلف (CI/CD, Docker) باعث نصب نسخه‌های متفاوت و شکست passlib در هش کردن پسورد شود. passlib 1.7.4 با bcrypt >= 4.1 به دلیل حذف ماژول `__about__` و خطا در ورودی‌های بیش از 72 بایت ناسازگار است.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] pyproject.toml دقیقاً bcrypt==4.0.1 را مشخص کند
- [ ] نصب وابستگی‌ها با pip install -e . بدون خطا انجام شود
- [ ] تابع hash_password در security.py بدون خطا کار کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. محدوده نسخه bcrypt در pyproject.toml را به 'bcrypt==4.0.1' تغییر دهید تا با requirements.txt هماهنگ شود و از نصب نسخه‌های ناسازگار جلوگیری کند.
...
## 🧪 دستورات اعتبارسنجی
- `pip install -e . 2>&1 | grep -i bcrypt`
- `python -c "from passlib.hash import bcrypt; print(bcrypt.hash('test'))"`

## ⚠️ ریسک‌ها و موارد احتیاط
بدون این تغییر، نصب در محیط‌های جدید ممکن است با خطا مواجه شود
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 2
  id: 198395d5-7cd5-4f3e-b3c7-17ad757d39ab
  عنوان اصلی: بروزرسانی تست‌های offer_letter با تغییرات مدل
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: backend/tests/test_offer_letter.py

📋 acceptance_criteria کامل:
  - diff `backend/app/models/offer_letter.py` بررسی شد و تأثیر بر `backend/tests/test_offer_letter.py` مستند شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - کد dependent با contract جدید align شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_offer_letter.py", "timeout_seconds": 60}]
  - integration test که هر دو فایل را پوشش می‌دهد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/test_offer_letter.py", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
🔄 test_offer_letter.py نیازمند بررسی به‌خاطر تغییر در offer_letter.py

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/tests/test_offer_letter.py`
...
- `backend/app/models/offer_letter.py` — فایل upstream که تغییر کرده
- `backend/app/models/__init__.py` — `test_offer_letter.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
فایل `backend/tests/test_offer_letter.py` یکی از مصرف‌کنندگان `backend/app/models/offer_letter.py` است.
...
## 🔍 جزئیات
- 📂 فایل تغییریافته: `backend/app/models/offer_letter.py`
- 📂 فایل وابسته (نیازمند بررسی): `backend/tests/test_offer_letter.py`
- 🔥 risk level: **high**
- 📝 reason: This test file directly interacts with the `OfferLetter` model and its components (enums, columns, relationships). Any change to column names, types, nullability, default values, enum values, or relationships will likely break existing tests or require new tests.
...
- [ ] diff `backend/app/models/offer_letter.py` بررسی شد و تأثیر بر `backend/tests/test_offer_letter.py` مستند شد
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: diff آخر در `backend/app/models/offer_letter.py` را بخوان — چی عوض شده.
گام ۲: کد در `backend/tests/test_offer_letter.py` که این تغییر را مصرف می‌کند پیدا کن.
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/tests/test_offer_letter.py`
- `ruff check backend/tests/test_offer_letter.py`
- `pytest -x`
...
- در commit message: `merged-from: 496b4ca8-39d9-4701-b640-c068952b45bf, 198395d5-7cd5-4f3e-b3c7-17ad757d39ab`
...
1. pyproject.toml دقیقاً bcrypt==4.0.1 را مشخص کند _(verify: static)_
2. نصب وابستگی‌ها با pip install -e . بدون خطا انجام شود _(verify: backend_test)_
3. تابع hash_password در security.py بدون خطا کار کند _(verify: backend_test)_
4. diff `backend/app/models/offer_letter.py` بررسی شد و تأثیر بر `backend/tests/test_offer_letter.py` مستند شد _(verify: manual_only)_
5. کد dependent با contract جدید align شد _(verify: backend_test)_
6. integration test که هر دو فایل را پوشش می‌دهد عبور می‌کند _(verify: backend_test)_

## Task Steps

### Step 1: ثبت یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
**Status:** `done` (100%)
**Scope:** این مرحله یک یادداشت هشداردهنده است که مدل اجراکننده باید قبل از هر اقدامی آن را بخواند. شامل قوانین خواندن کامل پرامپت، ممنوعیت خلاصه‌سازی، بررسی پیاده‌سازی قبلی، و مسئولیت‌های مدل است. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف اطمینان از اجرای دقیق و کامل درخواست کاربر است.
— [merged] این مرحله یک یادداشت هشداردهنده برای تسک دوم است که مدل اجراکننده باید قبل از هر اقدامی آن را بخواند. شامل قوانین خواندن کامل پرامپت، ممنوعیت خلاصه‌سازی، بررسی پیاده‌سازی قبلی، و مسئولیت‌های مدل است. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف اطمینان از اجرای دقیق و کامل درخواست کاربر است.
**Excerpt:**
```
## ⚠️ یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
...
### Step 2: ثبت هدف ساختاریافته: همگام‌سازی نسخه bcrypt در وابستگی‌ها
**Status:** `done` (100%)
**Scope:** این مرحله هدف اصلی تسک اول را ثبت می‌کند: رفع عدم تطابق نسخه bcrypt بین requirements.txt و pyproject.toml. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی هدف است.
**Excerpt:**
```
## 🎯 هدف (خلاصه ساختاریافته)
عدم تطابق نسخه bcrypt بین requirements.txt و pyproject.toml
...
### Step 3: ثبت معیار پذیرش (AC) برای تسک اول: همگام‌سازی bcrypt
**Status:** `done` (100%)
**Scope:** این مرحله معیارهای پذیرش (AC) برای تسک اول را ثبت می‌کند. شامل ۶ AC است که رفتار قابل مشاهده را تعریف می‌کنند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی معیارهای موفقیت است.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 4: ثبت نمونه‌های قبل/بعد برای تغییر محدوده نسخه bcrypt
**Status:** `done` (100%)
**Scope:** این مرحله نمونه‌های قبل و بعد از تغییر محدوده نسخه bcrypt در pyproject.toml را ثبت می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی تغییر مورد نیاز است.
**Excerpt:**
```
## 💡 نمونه‌های قبل/بعد
**تغییر محدوده نسخه**
...
### Step 5: ثبت دستورات اعتبارسنجی برای تسک اول
**Status:** `done` (100%)
**Scope:** این مرحله دستورات اعتبارسنجی برای تسک اول را ثبت می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی نحوه اعتبارسنجی تغییرات است.
**Excerpt:**
```
## 🧪 دستورات اعتبارسنجی
- `pip install -e . 2>&1 | grep -i bcrypt`
...
### Step 6: ثبت ریسک‌ها و موارد احتیاط برای تسک اول
**Status:** `done` (100%)
**Scope:** این مرحله ریسک‌ها و موارد احتیاط برای تسک اول را ثبت می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و آگاهی از ریسک‌های احتمالی است.
— [merged] این مرحله ریسک‌ها و موارد احتیاط برای تسک دوم را ثبت می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و آگاهی از ریسک‌های احتمالی است.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
بدون این تغییر، نصب در محیط‌های جدید ممکن است با خطا مواجه شود
...
### Step 7: ثبت هدف ساختاریافته: بروزرسانی تست‌های offer_letter با تغییرات مدل
**Status:** `done` (100%)
**Scope:** این مرحله هدف اصلی تسک دوم را ثبت می‌کند: بررسی و بروزرسانی test_offer_letter.py به‌خاطر تغییر در offer_letter.py. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی هدف است.
**Excerpt:**
```
## 🎯 هدف (خلاصه ساختاریافته)
🔄 test_offer_letter.py نیازمند بررسی به‌خاطر تغییر در offer_letter.py
...
### Step 8: ثبت معیار پذیرش (AC) برای تسک دوم: بروزرسانی تست‌های offer_letter
**Status:** `done` (100%)
**Scope:** این مرحله معیارهای پذیرش (AC) برای تسک دوم را ثبت می‌کند. شامل ۶ AC است که رفتار قابل مشاهده را تعریف می‌کنند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی معیارهای موفقیت است.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 9: ثبت گام ۲: پیدا کردن کد مصرف‌کننده تغییر در test_offer_letter.py
**Status:** `done` (100%)
**Scope:** این مرحله گام دوم از مراحل اجرایی پیشنهادی برای تسک دوم را ثبت می‌کند: پیدا کردن کد در test_offer_letter.py که تغییر در offer_letter.py را مصرف می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی مراحل اجرایی است.
**Excerpt:**
```
گام ۲: کد در `backend/tests/test_offer_letter.py` که این تغییر را مصرف می‌کند پیدا کن.
```

### Step 10: ثبت گام ۳: تطبیق و نوشتن integration test
**Status:** `done` (100%)
**Scope:** این مرحله گام سوم از مراحل اجرایی پیشنهادی برای تسک دوم را ثبت می‌کند: تطبیق کد test_offer_letter.py با تغییرات offer_letter.py و نوشتن integration test. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی مراحل اجرایی است.
**Excerpt:**
```
گام ۳: تطبیق بده + integration test بنویس.
```

### Step 11: ثبت دستورات اعتبارسنجی برای تسک دوم
**Status:** `done` (100%)
**Scope:** این مرحله دستورات اعتبارسنجی برای تسک دوم را ثبت می‌کند. هیچ تغییری در کد در این مرحله انجام نمی‌شود. هدف مستندسازی و شفاف‌سازی نحوه اعتبارسنجی تغییرات است.
**Excerpt:**
```
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/tests/test_offer_letter.py`
