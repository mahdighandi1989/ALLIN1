---
task_id: a2e64c3e-7b35-4423-865e-8bf85785a468
title: 'مدیریت endpoint بلااستفاده: GET /{user_id}'
type: audit
priority: medium
...
created_at: '2026-05-31T16:21:50.732086+00:00'
...
# مدیریت endpoint بلااستفاده: GET /{user_id}
...
---
[scan #2 at 2026-05-31T16:21:51.023185+00:00]
## 📋 شرح
endpoint `PUT /{user_id}` در `backend/app/routers/users.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
## 
---
[scan #3 at 2026-05-31T16:21:51.156986+00:00]
## 📋 شرح
endpoint `DELETE /{user_id}` در `backend/app/routers/users.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `GET /{user_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
