---
task_id: e5106cd5-d3f0-4de5-8cad-95106e9ba703
title: 'فایل‌های سرویس core بدون تست واحد: amortization.py, data_pipeline.py, data_scan.py, excel_import.py, exporters.py, fx.py, notifications.py, notify_inapp.py, snapshots.py'
type: bug
priority: high
...
created_at: '2026-05-31T16:20:28.728210+00:00'
updated_at: '2026-06-05T15:35:01.878546+00:00'
archived: true
archived_at: '2026-06-05T15:34:55.841285+00:00'
target_files:
...
# فایل‌های سرویس core بدون تست واحد: amortization.py, data_pipeline.py, data_scan.py, excel_import.py, exporters.py, fx.py, notifications.py, notify_inapp.py, snapshots.py

## Raw Idea

تحلیل پوشش تست نشان می‌دهد که ۹ فایل از ۱۰ فایل سرویس در `backend/app/services/` هیچ فایل تست متناظری ندارند. تنها `backend/app/services/audit.py` توسط تست‌های غیرمستقیم پوشش داده می‌شود. این سرویس‌ها شامل منطق حیاتی مانند محاسبه اقساط (amortization.py)، واردات اکسل (excel_import.py)، صادرات گزارش (exporters.py)، نرخ ارز (fx.py)، اعلان‌ها (notifications.py, notify_inapp.py)، و عکس‌های فوری (snapshots.py) هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
...
- `backend/app/services/amortization.py:1-100` — `amortization` — منطق محاسبه جدول اقساط وام
  ```python
  # کل فایل بدون تست
  ```
- `backend/app/services/excel_import.py:1-200` — `excel_import` — واردات داده از فایل‌های اکسل
  ```python
  # کل فایل بدون تست
  ```
- `backend/app/services/exporters.py:1-150` — `exporters` — صادرات به فرمت‌های xlsx و csv
  ```python
  # کل فایل بدون تست
  ```
...
FastAPI + SQLAlchemy 2.0 + pytest + pytest-asyncio

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/tests/conftest.py` (سطر 1) — فیxtureهای دیتابیس و کلاینت تست
- `backend/app/services/audit.py` (سطر 1) — تنها سرویس با پوشش تست نسبی
- `backend/tests/test_offer_letters.py` — این فایل `amortization.py` را import می‌کند (caller)
- `backend/app/db_init.py` — این فایل `amortization.py` را import می‌کند (caller)
- `backend/app/routers/offer_letters.py` — این فایل `amortization.py` را import می‌کند (caller)
- `backend/app/routers/imports.py` — این فایل `excel_import.py` را import می‌کند (caller)
- `backend/app/routers/customers.py` — این فایل `exporters.py` را import می‌کند (caller)
- `backend/app/routers/facilities.py` — این فایل `exporters.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این سرویس‌ها توسط روترهای مختلف (facilities, customers, imports, reports, fx) و همچنین توسط data_pipeline و data_scan استفاده می‌شوند. عدم تست‌دهی آن‌ها می‌تواند منجر به باگ‌های زنجیره‌ای در کل سیستم شود.
...
- [ ] فایل `backend/tests/test_amortization.py` ایجاد شود و حداقل ۳ تست داشته باشد
- [ ] فایل `backend/tests/test_excel_import.py` ایجاد شود و حداقل ۳ تست داشته باشد
- [ ] فایل `backend/tests/test_exporters.py` ایجاد شود و حداقل ۳ تست داشته باشد
- [ ] فایل `backend/tests
...
1. برای هر یک از فایل‌های سرویس ذکر شده، یک فایل تست جدید در `backend/tests/` ایجاد کنید. تست‌ها باید حداقل سناریوهای موفقیت، خطاهای ورودی، و موارد مرزی را پوشش دهند. برای سرویس‌های وابسته به دیتابیس، از فیxtureهای pytest موجود در `conftest.py` استفاده کنید.

## 💡 نمونه‌های قبل/بعد
**ایجاد تست برای amortization**
...
_بعد:_
```
# backend/tests/test_amortization.py
import pytest
from app.services.amortization import calculate_amortization_schedule

@pytest.mark.asyncio
async def test_calculate_amortization_schedule_standard_loan():
    schedule = await calculate_amortization_schedule(
        principal=100000, annual_rate=5.0, months=12
    )
...
1. فایل `backend/tests/test_amortization.py` ایجاد شود و حداقل ۳ تست داشته باشد _(verify: static)_
2. فایل `backend/tests/test_excel_import.py` ایجاد شود و حداقل ۳ تست داشته باشد _(verify: static)_
3. فایل `backend/tests/test_exporters.py` ایجاد شود و حداقل ۳ تست داشته باشد _(verify: static)_
4. فایل `backend/tests _(verify: manual_only)_

## Task Steps

### Step 1: ایجاد فایل تست برای سرویس amortization.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/amortization.py است. این فایل باید تمام توابع و کلاس‌های موجود در ماژول amortization.py را پوشش دهد، به ویژه منطق محاسبه اقساط و برنامه بازپرداخت وام. خارج از این مرحله: تست‌های یکپارچه‌سازی با دیتابیس یا API، تست‌های end-to-end، و تست ماژول‌های دیگر. نکته حیاتی: باید از چارچوب تست موجود (pytest) پیروی کند و از mock برای وابستگی‌های خارجی استفاده کند.
— [merged] این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/fx.py است. این فایل باید تمام توابع مربوط به نرخ ارز، از جمله دریافت نرخ‌ها از منابع خارجی، تبدیل ارز، و محاسبات نرخ را پوشش دهد. خارج از این مرحله: تست یکپارچه‌سازی با APIهای واقعی نرخ ارز، تست همزمانی، و تست ماژول‌های دیگر. نکته حیاتی: باید از mock برای APIهای خارجی استفاده کند تا تست‌ها وابسته به شبکه نباشند.
**Excerpt:**
```
تنها `backend/app/services/audit.py` توسط تست‌های غیرمستقیم پوشش داده می‌شود. این سرویس‌ها شامل منطق حیاتی مانند محاسبه اقساط (amortization.py)... هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 2: ایجاد فایل تست برای سرویس excel_import.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/excel_import.py است. این فایل باید تمام توابع مربوط به واردات اکسل، از جمله پارس کردن فایل، اعتبارسنجی داده‌ها، و تبدیل به فرمت داخلی را پوشش دهد. خارج از این مرحله: تست فایل‌های واقعی اکسل با حجم بالا، تست یکپارچه‌سازی با دیتابیس، و تست ماژول‌های دیگر. نکته حیاتی: باید از فایل‌های اکسل نمونه (mock) برای تست استفاده کند.
**Excerpt:**
```
این سرویس‌ها شامل منطق حیاتی مانند ... واردات اکسل (excel_import.py)... هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 3: ایجاد فایل تست برای سرویس exporters.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/exporters.py است. این فایل باید تمام توابع مربوط به صادرات گزارش، از جمله تولید فایل‌های CSV، Excel، و PDF را پوشش دهد. خارج از این مرحله: تست یکپارچه‌سازی با دیتابیس، تست تولید فایل‌های واقعی با حجم بالا، و تست ماژول‌های دیگر. نکته حیاتی: باید از mock برای وابستگی‌های خارجی و فایل‌های موقت برای تست خروجی استفاده کند.
**Excerpt:**
```
این سرویس‌ها شامل منطق حیاتی مانند ... صادرات گزارش (exporters.py)... هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 4: ایجاد فایل تست برای سرویس notifications.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/notifications.py است. این فایل باید تمام توابع مربوط به اعلان‌ها، از جمله ارسال ایمیل، پیامک، و اعلان درون‌برنامه‌ای را پوشش دهد. خارج از این مرحله: تست یکپارچه‌سازی با سرویس‌های واقعی ایمیل/پیامک، تست قالب‌های ایمیل، و تست ماژول‌های دیگر. نکته حیاتی: باید از mock برای سرویس‌های خارجی استفاده کند.
**Excerpt:**
```
این سرویس‌ها شامل منطق حیاتی مانند ... اعلان‌ها (notifications.py, notify_inapp.py)... هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 5: ایجاد فایل تست برای سرویس notify_inapp.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/notify_inapp.py است. این فایل باید تمام توابع مربوط به اعلان‌های درون‌برنامه‌ای، از جمله ایجاد، ارسال، و مدیریت اعلان‌های درون‌برنامه‌ای را پوشش دهد. خارج از این مرحله: تست یکپارچه‌سازی با فرانت‌اند، تست WebSocket، و تست ماژول‌های دیگر. نکته حیاتی: باید از mock برای وابستگی‌های دیتابیس و سایر سرویس‌ها استفاده کند.
**Excerpt:**
```
این سرویس‌ها شامل منطق حیاتی مانند ... اعلان‌ها (notifications.py, notify_inapp.py)... هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 6: ایجاد فایل تست برای سرویس snapshots.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد یک فایل تست جدید برای ماژول backend/app/services/snapshots.py است. این فایل باید تمام توابع مربوط به عکس‌های فوری (snapshots)، از جمله ایجاد، بازیابی، و مدیریت snapshotهای داده را پوشش دهد. خارج از این مرحله: تست یکپارچه‌سازی با دیتابیس، تست بازیابی واقعی داده، و تست ماژول‌های دیگر. نکته حیاتی: باید از mock برای دیتابیس و فایل‌سیستم استفاده کند.
**Excerpt:**
```
این سرویس‌ها شامل منطق حیاتی مانند ... عکس‌های فوری (snapshots.py) هستند. نبود تست برای این ماژول‌ها ریسک بالایی برای رگرسیون در عملیات‌های اصلی بانکی ایجاد می‌کند.
```

### Step 7: بررسی و به‌روزرسانی تست‌های موجود برای audit.py
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی تست‌های غیرمستقیم موجود برای backend/app/services/audit.py و اطمینان از پوشش کامل آن است. اگر تست‌های موجود ناقص هستند، تست‌های unit مستقیم برای audit.py اضافه می‌شوند. خارج از این مرحله: ایجاد تست برای ماژول‌های دیگر، بازنویسی منطق audit.py. نکته حیاتی: ابتدا تست‌های موجود را شناسایی کرده و شکاف‌های پوشش را مشخص کنید.
**Excerpt:**
```
تنها `backend/app/services/audit.py` توسط تست‌های غیرمستقیم پوشش داده می‌شود.
```
