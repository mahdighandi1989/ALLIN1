---
task_id: ebdd1439-51b2-4084-9c91-4bcb4b7f8178
title: نقطهٔ پایانی بک‌اند `/api/facilities/search/advanced` توسط فرانت‌اند استفاده نمی‌شود
type: refactor
priority: medium
...
created_at: '2026-06-07T16:57:56.566080+00:00'
updated_at: '2026-06-07T22:08:20.019659+00:00'
target_files:
- docs/ENDPOINT_AUDIT.md
- backend/app/routers/facilities.py
---

# نقطهٔ پایانی بک‌اند `/api/facilities/search/advanced` توسط فرانت‌اند استفاده نمی‌شود

## Raw Idea

فایل `docs/ENDPOINT_AUDIT.md` صراحتاً `GET /api/facilities/search/advanced` را به عنوان یک نقطهٔ پایانی «داخلی (جستجوی پیشرفته، هنوز متصل نشده)» پرچم‌گذاری کرده است. با این حال، `frontend/src/app/facilities/page.tsx` قبلاً فیلترگذاری پیشرفته (بر اساس نوع، وضعیت، محدودهٔ مبلغ، مرتب‌سازی) را با استفاده از نقطهٔ پایانی `facilitiesApi.list` پیاده‌سازی کرده است. این نشان‌دهندهٔ یک تکرار احتمالی است که در آن یک نقطهٔ پایانی اختصاصی «جستجوی پیشرفته» در بک‌اند وجود دارد اما توسط پیاده‌سازی فعلی فرانت‌اند استفاده نمی‌شود، یا `facilitiesApi.list` فرانت‌اند قبلاً قابلیت «جستجوی پیشرفته» مورد نظر را پوشش می‌دهد و نقطهٔ پایانی جداگانه را منسوخ می‌کند.
...
- `docs/ENDPOINT_AUDIT.md:4, 40` — منبع یافته.
  ```markdown
  ...
  ```
- `backend/app/routers/facilities.py` — فایلی که این نقطهٔ پایانی در آن تعریف می‌شود.
...
- `frontend/src/app/facilities/page.tsx` — نشان‌دهندهٔ فیلترگذاری پیشرفتهٔ فعلی از طریق `facilitiesApi.list` است.
- `backend/tests/test_facilities.py` — اگر تست‌هایی برای این نقطهٔ پایانی خاص وجود داشته باشد.
...
## 🌐 نقشهٔ وابستگی‌ها
حذف این نقطهٔ پایانی فقط بر روی فراخواننده‌های مستقیم `/api/facilities/search/advanced` تأثیر می‌گذارد. فرانت‌اند فعلی تحت تأثیر قرار نمی‌گیرد.
...
- [ ] بررسی کنید که آیا `GET /api/facilities/search/advanced` عملکردی متمایز از `GET /api/facilities/` با پارامترهای کوئری ارائه می‌دهد.
- [ ] در صورت تکراری بودن، مسیر `GET /api/facilities/search/advanced` را از `backend/app/routers/facilities.py` حذف کنید.
- [ ] فایل `docs/ENDPOINT_AUDIT.md` را برای بازتاب حذف یا ادغام به‌روزرسانی کنید.
- [ ] اطمینان حاصل کنید که هیچ تست موجودی خراب نمی‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. هدف `GET /api/facilities/search/advanced` را روشن کنید. اگر عملکرد آن به طور کامل توسط `facilitiesApi.list` (که به `GET /api/facilities/` نگاشت می‌شود) پوشش داده شده است، نقطهٔ پایانی `/api/facilities/search/advanced` باید از بک‌اند حذف شود تا سطح API و بار نگهداری کاهش یابد. اگر قابلیت منحصر به فردی را ارائه می‌دهد که در `facilitiesApi.list` وجود ندارد، اتصال آن به فرانت‌اند یا به‌روزرسانی مستندات را در نظر بگیرید. در حال حاضر، تکراری بودن را فرض کرده و حذف یا منسوخ کردن آن را پیشنهاد دهید.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_facilities.py`
- `فایل `backend/app/routers/facilities.py` را برای تعریف نقطهٔ پایانی بررسی کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
حذف تصادفی یک نقطهٔ پایانی واقعاً استفاده شده توسط یک کلاینت خارجی (اگرچه `include_in_schema=False` این خطر را برای مستندات عمومی کاهش می‌دهد).
...
1. بررسی کنید که آیا `GET /api/facilities/search/advanced` عملکردی متمایز از `GET /api/facilities/` با پارامترهای کوئری ارائه می‌دهد. _(verify: backend_test)_
2. در صورت تکراری بودن، مسیر `GET /api/facilities/search/advanced` را از `backend/app/routers/facilities.py` حذف کنید. _(verify: static)_
3. فایل `docs/ENDPOINT_AUDIT.md` را برای بازتاب حذف یا ادغام به‌روزرسانی کنید. _(verify: static)_
4. اطمینان حاصل کنید که هیچ تست موجودی خراب نمی‌شود. _(verify: backend_test)_

## Task Steps

### Step 1: بررسی و رفع ابهام نقطه پایانی جستجوی پیشرفته تسهیلات
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق نقطه پایانی `GET /api/facilities/search/advanced` که در `docs/ENDPOINT_AUDIT.md` به عنوان «داخلی (جستجوی پیشرفته، هنوز متصل نشده)» پرچم‌گذاری شده، و مقایسه آن با پیاده‌سازی فیلترگذاری پیشرفته موجود در `frontend/src/app/facilities/page.tsx` که از `facilitiesApi.list` استفاده می‌کند، می‌باشد. هدف این است که مشخص شود آیا نقطه پایانی بک‌اند اضافی است، یا فرانت‌اند باید برای استفاده از آن به‌روزرسانی شود، یا نقطه پایانی بک‌اند نیاز به تکمیل دارد. این مرحله شامل تصمیم‌گیری و اجرای تغییرات لازم برای رفع این ابهام و اطمینان از همگام‌سازی فرانت‌اند و بک‌اند است.
**Excerpt:**
```
فایل `docs/ENDPOINT_AUDIT.md` صراحتاً `GET /api/facilities/search/advanced` را به عنوان یک نقطهٔ پایانی «داخلی (جستجوی پیشرفته، هنوز متصل نشده)» پرچم‌گذاری کرده است. با این حال، `frontend/src/app/facilities/page.tsx` قبلاً فیلترگذاری پیشرفته (بر اساس نوع، وضعیت، محدودهٔ مبلغ، مرتب‌سازی) را با استفاده از نقطهٔ پایانی `facilitiesApi.list` پیاده‌سازی کرده است. این نشان‌دهندهٔ یک تکرار احتمالی است که در آن یک نقطهٔ پایانی اختصاصی «جستجوی پیشرفته» در بک‌اند وجود دارد اما توسط پیاده‌سازی فعلی فرانت‌اند استفاده نمی‌شود، یا `facilitiesApi.list` فرانت‌اند قبلاً قابلیت «جستجوی پیشرفته» مورد نظر را پوشش می‌دهد و نقطهٔ پایانی جداگانه را منسوخ می‌کند.
```
