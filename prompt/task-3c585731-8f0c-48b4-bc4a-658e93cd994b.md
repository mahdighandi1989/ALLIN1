---
task_id: 3c585731-8f0c-48b4-bc4a-658e93cd994b
title: تضاد default برای فیلد 'else'
type: bug_fix
priority: high
...
created_at: '2026-06-07T16:55:02.477068+00:00'
updated_at: '2026-06-07T16:55:02.695351+00:00'
---

# تضاد default برای فیلد 'else'

## Raw Idea

## 📋 شرح
فیلد `else` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field else has different defaults: ['token.split(".")', 'datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)', 'pattern.sub(rf"\\1\\2{_REDACTION}\\4", redacted)']
---
[scan #2 at 2026-06-07T16:55:02.505334+00:00]
## 📋 شرح
...
## 🔍 جزئیات
- علت: field user_id has different defaults: ['None', 'None) -> str:', 'Depends(get_db),
---
[scan #3 at 2026-06-07T16:55:02.520932+00:00]
## 📋 شرح
فیلد `installment_number` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field installment_number has different defaults: ['None', '"monthly",'
---
[scan #4 at 2026-06-07T16:55:02.531410+00:00]
## 📋 شرح
فیلد `grace_period_months` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field grace_period_months has different defaults: ['Field(default=0, 
---
[scan #5 at 2026-06-07T16:55:02.541615+00:00]
## 📋 شرح
فیلد `action` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field action has different defaults: ['None', 'None,', 'Query(None),']
---
[scan #6 at 2026-06-07T16:55:02.551983+00:00]
## 📋 شرح
فیلد `entity_id` در `` در دو یا چند جای مختلف default value متفاوت دارد.

## 🤔 چرا مهم است
defaults متناقض باعث می‌شود رفتار سیستم به ترتیب اجرا/import وابسته شود — bug های غیرقابل reproduce.

## 🔍 جزئیات
- علت: field entity_id has different defaults: ['None', 'None,']
...
- [ ] `else` در همه‌جا از یک منبع default می‌گیرد
...
1. گام ۱: همه جاهایی که `else` default می‌گیرد لیست کن.
...
1. `else` در همه‌جا از یک منبع default می‌گیرد
2. تست fixture رفتار پیش‌فرض را تأیید می‌کند
3. اگر default value تغییر کرد، migration یا backward-compat layer اضافه شد
