...
updated_at: '2026-06-07T16:55:02.462196+00:00'
target_files:
...
1. اگر هر حذف فردی در یک عملیات حذف انبوه شکست بخورد، هیچ موردی حذف نمی‌شود و وضعیت پایگاه داده بدون تغییر باقی می‌ماند.
2. یک تست یکپارچه‌سازی جدید برای `bulkDelete` اتمی بودن تراکنش را تأیید می‌کند (به عنوان مثال، با شبیه‌سازی یک شکست در میانه راه).
