---
task_id: c75cb62c-2423-41a7-bd93-092bcdd87cf7
title: پتانسیل تراکنش‌های ناقص پایگاه داده در عملیات حذف انبوه
type: bug
priority: high
...
created_at: '2026-06-07T16:55:02.462192+00:00'
updated_at: '2026-06-07T22:05:47.127618+00:00'
target_files:
...
# پتانسیل تراکنش‌های ناقص پایگاه داده در عملیات حذف انبوه

## Raw Idea

کامپوننت‌های فرانت‌اند برای مشتریان و تسهیلات (`customers/page.tsx` خط 96، `facilities/page.tsx` خط 112) قابلیت `bulkDelete` را پیاده‌سازی می‌کنند. این ویژگی لیستی از شناسه‌ها را برای حذف به API بک‌اند ارسال می‌کند. اگر هندلر API بک‌اند برای `bulkDelete` (به عنوان مثال، در `backend/app/routers/customers.py` یا `backend/app/routers/facilities.py`) لیست شناسه‌ها را پیمایش کرده و عملیات `DELETE` فردی یا به‌روزرسانی‌های مرتبط (مانند ثبت رویداد حسابرسی) را بدون بسته‌بندی آن‌ها در یک تراکنش اتمی پایگاه داده انجام دهد، یک شکست در حین حذف یک مورد می‌تواند پایگاه داده را در وضعیت ناسازگار رها کند. به عنوان مثال، ممکن است برخی از موارد حذف شوند، در حالی که برخی دیگر باقی بمانند و گزارش‌های حسابرسی ناقص باشند.
...
- `frontend/src/app/customers/page.tsx:91-103` — `bulkDelete` — فراخوانی `customersApi.bulkDelete` در فرانت‌اند.
  ```tsx
  ...
  ```
- `frontend/src/app/facilities/page.tsx:107-118` — `bulkDelete` — فراخوانی `facilitiesApi.bulkDelete` در فرانت‌اند.
  ```tsx
  const bulkDelete = async () => {
    const ids = Array.from(selected)
    if (ids.length === 0) return
  ...
  ```
- `backend/app/routers/customers.py:N/A` — `bulkDelete endpoint` — نقطه پایانی بک‌اند برای حذف انبوه مشتریان (ضمنی).
  ```python
  N/A
  ```
- `backend/app/routers/facilities.py:N/A` — `bulkDelete endpoint` — نقطه پایانی بک‌اند برای حذف انبوه تسهیلات (ضمنی).
  ```python
  N/A
  ```
...
FastAPI backend, SQLAlchemy ORM, PostgreSQL database.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/audit.py` — سرویس ثبت رویداد حسابرسی که ممکن است بخشی از فرآیند حذف انبوه باشد.
- `backend/app/database.py` — فایل ارائه دهنده سشن پایگاه داده.
- `backend/tests/test_bulk.py` — تست‌های موجود برای عملیات انبوه، که باید گسترش یابند.
...
## 🌐 نقشهٔ وابستگی‌ها
بر یکپارچگی داده‌ها برای عملیات انبوه در سراسر مشتریان و تسهیلات تأثیر می‌گذارد.
...
- [ ] اگر هر حذف فردی در یک عملیات حذف انبوه شکست بخورد، هیچ موردی حذف نمی‌شود و وضعیت پایگاه داده بدون تغییر باقی می‌ماند.
- [ ] یک تست یکپارچه‌سازی جدید برای `bulkDelete` اتمی بودن تراکنش را تأیید می‌کند (به عنوان مثال، با شبیه‌سازی یک شکست در میانه راه).
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. پیاده‌سازی‌های بک‌اند `customersApi.bulkDelete` و `facilitiesApi.bulkDelete` (واقع در `backend/app/routers/customers.py` و `backend/app/routers/facilities.py` به ترتیب) را بررسی کنید. اطمینان حاصل کنید که تمام عملیات نوشتن در پایگاه داده در داخل این هندلرهای حذف انبوه در یک تراکنش سشن SQLAlchemy واحد محصور شده‌اند. این کار اتمی بودن را تضمین می‌کند: یا همه موارد با موفقیت حذف و ثبت می‌شوند، یا در صورت بروز هر خطا، کل عملیات بازگردانده می‌شود و یکپارچگی داده‌ها حفظ می‌شود.
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_bulk.py -k transaction_atomicity`

## ⚠️ ریسک‌ها و موارد احتیاط
مدیریت نادرست تراکنش می‌تواند منجر به بن‌بست یا مشکلات عملکردی شود اگر با دقت پیاده‌سازی نشود.
...
1. اگر هر حذف فردی در یک عملیات حذف انبوه شکست بخورد، هیچ موردی حذف نمی‌شود و وضعیت پایگاه داده بدون تغییر باقی می‌ماند. _(verify: backend_test)_
2. یک تست یکپارچه‌سازی جدید برای `bulkDelete` اتمی بودن تراکنش را تأیید می‌کند (به عنوان مثال، با شبیه‌سازی یک شکست در میانه راه). _(verify: backend_test)_

## Task Steps

### Step 1: پیاده‌سازی حذف دسته‌ای اتمی برای مشتریان
**Status:** `pending` (0%)
**Scope:** این مرحله بر اصلاح هندلر `bulkDelete` در `backend/app/routers/customers.py` تمرکز دارد. هدف این است که اطمینان حاصل شود تمام عملیات حذف فردی و به‌روزرسانی‌های مرتبط (مانند ثبت رویداد حسابرسی) برای مشتریان، در یک تراکنش اتمی پایگاه داده بسته‌بندی شوند. این کار از وضعیت‌های ناسازگار پایگاه داده در صورت بروز خطا در حین عملیات حذف جلوگیری می‌کند. این مرحله شامل تغییرات فرانت‌اند یا منطق مربوط به تسهیلات نمی‌شود.
— [merged] این مرحله بر اصلاح هندلر `bulkDelete` در `backend/app/routers/facilities.py` تمرکز دارد. هدف این است که اطمینان حاصل شود تمام عملیات حذف فردی و به‌روزرسانی‌های مرتبط (مانند ثبت رویداد حسابرسی) برای تسهیلات، در یک تراکنش اتمی پایگاه داده بسته‌بندی شوند. این کار از وضعیت‌های ناسازگار پایگاه داده در صورت بروز خطا در حین عملیات حذف جلوگیری می‌کند. این مرحله شامل تغییرات فرانت‌اند یا منطق مربوط به مشتریان نمی‌شود.
**Excerpt:**
```
کامپوننت‌های فرانت‌اند برای مشتریان و تسهیلات (`customers/page.tsx` خط 96، `facilities/page.tsx` خط 112) قابلیت `bulkDelete` را پیاده‌سازی می‌کنند. این ویژگی لیستی از شناسه‌ها را برای حذف به API بک‌اند ارسال می‌کند. اگر هندلر API بک‌اند برای `bulkDelete` (به عنوان مثال، در `backend/app/routers/customers.py` یا `backend/app/routers/facilities.py`) لیست شناسه‌ها را پیمایش کرده و عملیات `DELETE` فردی یا به‌روزرسانی‌های مرتبط (مانند ثبت رویداد حسابرسی) را بدون بسته‌بندی آن‌ها در یک تراکنش اتمی پایگاه داده انجام دهد، یک شکست در حین حذف یک مورد می‌تواند پایگاه داده را در وضعیت ناسازگار رها کند. به عنوان مثال، ممکن است برخی از موارد حذف شوند، در حالی که برخی دیگر باقی بمانند و گزارش‌های حسابرسی ناقص باشند.
```
