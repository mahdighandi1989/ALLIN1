---
task_id: 35f820b1-b2fc-4091-b26a-a6f1b37a3147
title: عدم رسیدگی کامل به کاراکترهای یونیکد/RTL در ورودی‌های جستجو/فیلتر فرانت‌اند
type: bug
priority: medium
...
created_at: '2026-06-07T16:58:26.106761+00:00'
updated_at: '2026-06-07T22:08:56.232246+00:00'
target_files:
- frontend/src/app/customers/page.tsx
- frontend/src/app/facilities/page.tsx
---

# عدم رسیدگی کامل به کاراکترهای یونیکد/RTL در ورودی‌های جستجو/فیلتر فرانت‌اند

## Raw Idea

فایل `README.md` پروژه حاوی متن فارسی است که نشان‌دهنده نیاز به پشتیبانی از چندزبانگی، از جمله زبان‌های راست به چپ (RTL) و مجموعه‌های کاراکتری یونیکد گسترده است. در حالی که پاکسازی XSS سمت کلاینت پیاده‌سازی شده است، فیلدهای ورودی جستجو و فیلتر فعلی در فرانت‌اند (`frontend/src/app/customers/page.tsx` خطوط 144-150، `frontend/src/app/facilities/page.tsx` خطوط 165-171) ورودی‌های متنی استاندارد HTML هستند. هیچ نشانه صریحی در قطعه کدها یا مستندات ارائه شده وجود ندارد که نشان دهد این ورودی‌ها و منطق جستجوی بک‌اند مربوطه، به طور خاص برای کاراکترهای پیچیده یونیکد، نرمال‌سازی کاراکترها یا نمایش و تطابق متن RTL بهینه یا به طور کامل آزمایش شده‌اند. این می‌تواند منجر به نتایج جستجوی ناسازگار، اشکالات نمایش یا تجربه کاربری ضعیف برای کاربرانی شود که با داده‌های غیرلاتین تعامل دارند.
...
- `frontend/src/app/customers/page.tsx:144-150` — `Search input` — ورودی جستجوی مشتریان.
  ```tsx
  <input
    type="text"
    value={search}
  ...
  />
  ```
- `frontend/src/app/facilities/page.tsx:165-171` — `Search input` — ورودی جستجوی تسهیلات.
  ```tsx
  <input
    type="text"
    value={search}
...
Next.js 14 (React 18) frontend, FastAPI backend, PostgreSQL database.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/routers/customers.py` — پیاده‌سازی منطق جستجوی مشتریان.
- `backend/app/routers/facilities.py` — پیاده‌سازی منطق جستجوی تسهیلات.
- `backend/app/database.py` — تنظیمات اتصال و سشن پایگاه داده.
- `backend/tests/test_customers.py` — تست‌های جستجوی مشتریان.
- `backend/tests/test_facilities.py` — تست‌های جستجوی تسهیلات.
...
## 🌐 نقشهٔ وابستگی‌ها
بر رندر UI فرانت‌اند و کوئری‌های پایگاه داده بک‌اند تأثیر می‌گذارد.
...
- [ ] جستجو برای نام مشتری/تسهیلات با استفاده از کاراکترهای فارسی نتایج صحیح را برمی‌گرداند.
- [ ] ورودی‌های جستجوی فرانت‌اند متن فارسی/عربی را به درستی نمایش داده و امکان ورود آن را فراهم می‌کنند.
- [ ] هیچ خطای کدگذاری یا رفتار غیرمنتظره‌ای هنگام فیلتر کردن با رشته‌های یونیکد وجود ندارد.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. **فرانت‌اند**: اطمینان حاصل کنید که ورودی‌های متنی برای جستجو و فیلتر (`<input type="text">`) کاراکترهای یونیکد، از جمله کاراکترهای اسکریپت‌های RTL را به درستی مدیریت و نمایش می‌دهند. این ممکن است شامل تنظیم ویژگی‌های `lang` یا ویژگی‌های `direction` CSS مناسب باشد، اگر قبلاً توسط پیش‌فرض‌های مرورگر مدیریت نشده باشند. 2. **بک‌اند**: تأیید کنید که collation پایگاه داده و کوئری‌های جستجو (به عنوان مثال، بندهای `LIKE` در SQL) برای تطابق و مرتب‌سازی صحیح کاراکترهای یونیکد، از جمله تطابق بدون حساسیت به حروف کوچک/بزرگ و بدون حساسیت به لهجه در صورت لزوم، پیکربندی شده‌اند. 3. **تست**: تست‌های واحد و یکپارچه‌سازی خاصی را برای قابلیت‌های جستجو و فیلتر با استفاده از مجموعه‌های کاراکتری یونیکد و RTL مختلف اضافه کنید تا رفتار صحیح تأیید شود.

## 💡 نمونه‌های قبل/بعد
**قبل (مفهومی فرانت‌اند)**
...
**بعد (مفهومی فرانت‌اند - مثال برای RTL)**

_بعد:_
```
<input type="text" value={search} onChange={(e) => setSearch(e.target.value)} dir="auto" lang="fa" />
```
...
## 🧪 دستورات اعتبارسنجی
- `Manual testing of search bars with Persian/Arabic text.`
- `pytest backend/tests/test_customers.py -k unicode_search`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییرات collation پایگاه داده می‌تواند پیچیده باشد و ممکن است نیاز به مهاجرت داده داشته باشد؛ اطمینان از رفتار سازگار در محیط‌های مختلف.
...
1. جستجو برای نام مشتری/تسهیلات با استفاده از کاراکترهای فارسی نتایج صحیح را برمی‌گرداند. _(verify: ui_interaction)_
2. ورودی‌های جستجوی فرانت‌اند متن فارسی/عربی را به درستی نمایش داده و امکان ورود آن را فراهم می‌کنند. _(verify: ui_interaction)_
3. هیچ خطای کدگذاری یا رفتار غیرمنتظره‌ای هنگام فیلتر کردن با رشته‌های یونیکد وجود ندارد. _(verify: ui_interaction)_
