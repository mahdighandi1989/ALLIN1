---
task_id: 82d15259-0c94-434f-8441-5ca7ef2bbcf7
title: Critical environment variables are not documented in `backend/.env.example`
type: docs
priority: high
...
created_at: '2026-06-07T16:54:00.159421+00:00'
updated_at: '2026-06-07T22:05:00.271005+00:00'
target_files:
- backend/.env.example
---

# Critical environment variables are not documented in `backend/.env.example`

## Raw Idea

While the content of `backend/.env.example` is not provided, the project's `docs/SECURITY.md` and `docs/TASK_STATUS.md` explicitly highlight the importance of environment variables such as `SECRET_KEY` (for JWT signing, with a minimum length requirement) and `DB_SSL_VERIFY` (for database SSL configuration). `AUTH_DISABLED` is also mentioned as a temporary toggle. It is a common oversight for critical new environment variables to not be added to the example `.env` file. This lack of documentation makes initial setup difficult for new developers, increases the risk of misconfiguration in deployments, and could lead to security vulnerabilities if default or weak values are used inadvertently.
...
- `backend/.env.example:N/A` — Add the specified environment variables with comments and placeholder values to this file.
  ```
...
FastAPI application configuration, environment variable management.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/config.py` — This file loads and validates the application's settings, including these environment variables.
- `docs/SECURITY.md` — Details the requirements for `SECRET_KEY`.
- `docs/TASK_STATUS.md` — Mentions `DB_SSL_VERIFY` and `AUTH_DISABLED`.
- `backend/app/database.py` — Uses `DB_SSL_VERIFY` for database connection.

## 🌐 نقشهٔ وابستگی‌ها
Proper documentation of environment variables is crucial for project setup, security, and consistent deployment across different environments.
...
- [ ] `backend/.env.example` contains `SECRET_KEY`, `DB_SSL_VERIFY`, and `AUTH_DISABLED`.
- [ ] Comments explain the purpose and requirements for each variable, especially `SECRET_KEY`'s minimum length.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Create or update `backend/.env.example` to include `SECRET_KEY`, `DB_SSL_VERIFY`, and `AUTH_DISABLED`. Provide clear placeholder values and comments explaining their purpose, security implications (e.g., `SECRET_KEY` minimum length), and recommended production values.

## 💡 نمونه‌های قبل/بعد
**Add critical environment variables to .env.example**
...
# JWT Secret Key: Must be a strong, random string (min 32 chars) for production.
SECRET_KEY="your_super_secret_key_here_min_32_chars"

# Database SSL Verification: Set to 'true' in production with proper certificates.
DB_SSL_VERIFY="false"

# Authentication Disabled: Set to 'true' to bypass login (DEVELOPMENT ONLY, DANGER!)
AUTH_DISABLED="false"
```
...
1. `backend/.env.example` contains `SECRET_KEY`, `DB_SSL_VERIFY`, and `AUTH_DISABLED`. _(verify: static)_
2. Comments explain the purpose and requirements for each variable, especially `SECRET_KEY`'s minimum length. _(verify: static)_
