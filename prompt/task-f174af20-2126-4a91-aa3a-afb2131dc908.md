---
task_id: f174af20-2126-4a91-aa3a-afb2131dc908
title: اعتبارسنجی ناقص `customer_id` در فرم تسهیلات
type: bug
priority: medium
...
created_at: '2026-06-07T16:58:59.559490+00:00'
updated_at: '2026-06-07T22:09:09.803096+00:00'
target_files:
...
# اعتبارسنجی ناقص `customer_id` در فرم تسهیلات

## Raw Idea

کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` (خطوط 338-393) به کاربران امکان ایجاد یا ویرایش سوابق تسهیلات را می‌دهد. یک فیلد حیاتی برای تسهیلات، `customer_id` است. فایل `data-import/README.md` به صراحت بیان می‌کند که برای تسهیلات وارداتی، `customer_id` «باید در جدول مشتریان وجود داشته باشد». در حالی که مودال لیستی از مشتریان (`customersApi.list`) را برای پر کردن یک لیست کشویی واکشی می‌کند، هیچ اعتبارسنجی صریحی در سمت کلاینت نشان داده نشده است که تأیید کند `customer_id` انتخاب شده در فرم *واقعاً در این لیست واکشی شده از مشتریان وجود دارد* قبل از ارسال فرم. `validateFacilityForm` (وارد شده اما ارائه نشده) ممکن است این بررسی وجود وابسته به پایگاه داده را شامل نشود. ارسال یک تسهیلات با `customer_id` غیرموجود می‌تواند منجر به خطاهای بک‌اند (به عنوان مثال، نقض محدودیت کلید خارجی) و تجربه کاربری ضعیف شود.
...
- `frontend/src/app/facilities/page.tsx:349-350` — `FacilityFormModal` — مقداردهی اولیه حالت فرم که شامل `customer_id` است.
  ```tsx
  ...
  ```
- `frontend/src/app/facilities/page.tsx:361-365` — `FacilityFormModal` — واکشی لیست مشتریان برای پر کردن لیست کشویی.
  ```tsx
  useEffect(() => {
    customersApi
  ...
  ```
- `frontend/src/app/facilities/page.tsx:372-376` — `handleSubmit` — فراخوانی `validateFacilityForm` که ممکن است بررسی وجود `customer_id` را شامل نشود.
  ```tsx
  ...
  ```
- `data-import/README.md:81` — قانون اعتبارسنجی برای `customer_id` در واردات اکسل.
  ```markdown
  - `customer_id` (string, must exist in customers table)
  ```
- `backend/app/routers/facilities.py:N/A` — `create/update facility endpoints` — نقطه پایانی بک‌اند برای ایجاد/به‌روزرسانی تسهیلات (ضمنی).
  ```python
  N/A
  ```
...
Next.js 14 (React 18) frontend, FastAPI backend, SQLAlchemy ORM.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/lib/facilityValidation.ts` — فایلی که `validateFacilityForm` در آن تعریف شده است (ضمنی).
- `backend/app/models/customer.py` — مدل مشتری.
- `backend/app/models/facility.py` — مدل تسهیلات.
- `backend/app/services/audit.py` — ثبت رویداد حسابرسی که ممکن است تحت تأثیر ایجاد تسهیلات ناموفق قرار گیرد.
- `backend/tests/test_facilities.py` — تست‌های ایجاد/به‌روزرسانی تسهیلات.
- `frontend/src/app/facilities/page.test.tsx` — این فایل `page.tsx` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
بر یکپارچگی داده‌ها و تجربه کاربری برای مدیریت تسهیلات تأثیر می‌گذارد.
...
- [ ] فرم ایجاد/ویرایش تسهیلات در صورت انتخاب `customer_id` غیرموجود، از ارسال جلوگیری می‌کند.
- [ ] در صورت نامعتبر بودن `customer_id`، یک پیام خطای واضح به کاربر نمایش داده می‌شود.
- [ ] API بک‌اند برای تسهیلات در صورت تلاش برای ایجاد/به‌روزرسانی تسهیلات با `customer_id` غیرموجود، یک خطای خاص (به عنوان مثال، 400 Bad Request یا 404 Not Found) را برمی‌گرداند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. **سمت کلاینت**: اعتبارسنجی سمت کلاینت را در `FacilityFormModal` (یا `validateFacilityForm`) برای انجام یک بررسی وجود بهبود بخشید. قبل از ارسال فرم، تأیید کنید که مقدار `form.customer_id` در آرایه `customers` که واکشی شده و برای پر کردن لیست کشویی استفاده شده است، وجود دارد. در صورت عدم یافتن، یک پیام خطای کاربرپسند نمایش دهید. 2. **سمت سرور**: اطمینان حاصل کنید که API بک‌اند برای ایجاد/به‌روزرسانی تسهیلات (`backend/app/routers/facilities.py`) یک بررسی قوی سمت سرور را برای تأیید وجود `customer_id` در پایگاه داده قبل از تلاش برای ایجاد یا به‌روزرسانی رکورد تسهیلات انجام می‌دهد. این یک اقدام دفاعی حیاتی است.

## 💡 نمونه‌های قبل/بعد
**قبل (اعتبارسنجی سمت کلاینت مفهومی)**
...
**بعد (اعتبارسنجی سمت کلاینت مفهومی)**
...
if (!customers.some(c => c.id === form.customer_id)) {
  toast.error('Selected customer does not exist.')
  return
}
// ... submit
```
...
## 🧪 دستورات اعتبارسنجی
- `Manually attempt to create a facility with an invalid `customer_id` (e.g., by manipulating the dropdown value in browser dev tools).`
- `pytest backend/tests/test_facilities.py -k invalid_customer_id`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به مدیریت دقیق بارگذاری داده‌های مشتری به صورت ناهمزمان و ارسال فرم.
...
1. فرم ایجاد/ویرایش تسهیلات در صورت انتخاب `customer_id` غیرموجود، از ارسال جلوگیری می‌کند. _(verify: ui_interaction)_
2. در صورت نامعتبر بودن `customer_id`، یک پیام خطای واضح به کاربر نمایش داده می‌شود. _(verify: ui_interaction)_
3. API بک‌اند برای تسهیلات در صورت تلاش برای ایجاد/به‌روزرسانی تسهیلات با `customer_id` غیرموجود، یک خطای خاص (به عنوان مثال، 400 Bad Request یا 404 Not Found) را برمی‌گرداند. _(verify: api_response)_

## Task Steps

### Step 1: تحلیل کامپوننت FacilityFormModal و مدیریت customer_id
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق کد موجود در `frontend/src/app/facilities/page.tsx`، به ویژه خطوط 338-393، برای درک نحوه انتخاب، ذخیره‌سازی و ارسال `customer_id` است. همچنین، نحوه واکشی و استفاده از لیست مشتریان (`customersApi.list`) برای پر کردن لیست کشویی `customer_id` شناسایی می‌شود. هدف، ایجاد یک درک جامع از جریان داده و تعاملات فعلی است. خارج از این مرحله، هرگونه تغییر در کد یا پیاده‌سازی اعتبارسنجی جدید است.
**Excerpt:**
```
کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` (خطوط 338-393) به کاربران امکان ایجاد یا ویرایش سوابق تسهیلات را می‌دهد. یک فیلد حیاتی برای تسهیلات، `customer_id` است. ... در حالی که مودال لیستی از مشتریان (`customersApi.list`) را برای پر کردن یک لیست کشویی واکشی می‌کند...
```

### Step 2: پیاده‌سازی منطق اعتبارسنجی وجود customer_id در سمت کلاینت
**Status:** `pending` (0%)
**Scope:** این مرحله شامل توسعه یک تابع یا ماژول مستقل است که `customer_id` انتخاب شده و لیست کامل مشتریان (که از `customersApi.list` واکشی شده) را به عنوان ورودی می‌گیرد. این تابع باید بررسی کند که آیا `customer_id` ارائه شده در لیست مشتریان موجود است یا خیر و یک مقدار بولی (true/false) برگرداند. این منطق باید قابل استفاده مجدد و مستقل از کامپوننت UI باشد. خارج از این مرحله، ادغام این منطق در `FacilityFormModal` یا نمایش پیام خطا در UI است.
**Excerpt:**
```
...هیچ اعتبارسنجی صریحی در سمت کلاینت نشان داده نشده است که تأیید کند `customer_id` انتخاب شده در فرم *واقعاً در این لیست واکشی شده از مشتریان وجود دارد* قبل از ارسال فرم. `validateFacilityForm` (وارد شده اما ارائه نشده) ممکن است این بررسی وجود وابسته به پایگاه داده را شامل نشود.
```

### Step 3: ادغام اعتبارسنجی customer_id در FacilityFormModal
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` برای فراخوانی منطق اعتبارسنجی توسعه‌یافته در مرحله ۲ است. این فراخوانی باید قبل از ارسال فرم انجام شود. این ممکن است شامل به‌روزرسانی تابع `validateFacilityForm` (در صورت وجود و مناسب بودن) یا افزودن یک مرحله اعتبارسنجی جدید در هندلر ارسال فرم باشد. هدف این است که فرم قبل از ارسال، `customer_id` را در برابر لیست مشتریان واکشی شده اعتبارسنجی کند. خارج از این مرحله، طراحی یا پیاده‌سازی نمایش بصری پیام خطا است.
**Excerpt:**
```
...تأیید کند `customer_id` انتخاب شده در فرم *واقعاً در این لیست واکشی شده از مشتریان وجود دارد* قبل از ارسال فرم. `validateFacilityForm` (وارد شده اما ارائه نشده) ممکن است این بررسی وجود وابسته به پایگاه داده را شامل نشود.
```

### Step 4: نمایش بازخورد اعتبارسنجی سمت کلاینت برای customer_id
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بهبود `FacilityFormModal` برای نمایش بصری یک پیام خطا به کاربر است، زمانی که `customer_id` انتخاب شده توسط اعتبارسنجی سمت کلاینت نامعتبر تشخیص داده شود. این پیام خطا باید به وضوح قابل مشاهده باشد، ترجیحاً در نزدیکی فیلد ورودی `customer_id`، تا کاربر بتواند به سرعت مشکل را شناسایی و اصلاح کند. این شامل طراحی و پیاده‌سازی عنصر UI برای نمایش خطا است. خارج از این مرحله، منطق اعتبارسنجی یا ادغام آن با فرم است.
**Excerpt:**
```
...ارسال یک تسهیلات با `customer_id` غیرموجود می‌تواند منجر به خطاهای بک‌اند (به عنوان مثال، نقض محدودیت کلید خارجی) و تجربه کاربری ضعیف شود.
```

### Step 5: افزودن تست‌های واحد/ادغام برای اعتبارسنجی customer_id
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های خودکار برای اطمینان از صحت و قابلیت اطمینان منطق اعتبارسنجی `customer_id` (توسعه‌یافته در مرحله ۲) و همچنین تأیید ادغام صحیح آن در `FacilityFormModal` (مراحل ۳ و ۴) است. تست‌های واحد باید سناریوهای مختلف (مانند `customer_id` معتبر، نامعتبر، لیست مشتریان خالی) را پوشش دهند. تست‌های ادغام باید تأیید کنند که اعتبارسنجی در فرم فعال می‌شود و بازخورد UI به درستی نمایش داده می‌شود. خارج از این مرحله، تست سایر بخش‌های `FacilityFormModal` یا تست‌های بک‌اند است.
**Excerpt:**
```
ارسال یک تسهیلات با `customer_id` غیرموجود می‌تواند منجر به خطاهای بک‌اند (به عنوان مثال، نقض محدودیت کلید خارجی) و تجربه کاربری ضعیف شود.
```
