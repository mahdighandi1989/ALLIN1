---
task_id: b646f989-b200-42ad-b6f9-3b7256903fb5
title: 'env var بلااستفاده: ACCOUNT_LOCKOUT_MINUTES'
type: cleanup
priority: medium
...
created_at: '2026-06-07T16:59:01.692194+00:00'
updated_at: '2026-06-07T16:59:03.153115+00:00'
---

# env var بلااستفاده: ACCOUNT_LOCKOUT_MINUTES

## Raw Idea

## 📋 شرح
env var `ACCOUNT_LOCKOUT_MINUTES` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
---
[scan #2 at 2026-06-07T16:59:01.754107+00:00]
## 📋 شرح
env var `ACCOUNT_LOCKOUT_THRESHOLD` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- علت: documented in
---
[scan #3 at 2026-06-07T16:59:01.788067+00:00]
## 📋 شرح
...
---
[scan #4 at 2026-06-07T16:59:01.818420+00:00]
## 📋 شرح
env var `ALLOWED_EXTENSIONS` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) reader حذف شده و باعث config drift می‌شود، یا (ب) leak اطلاعات حساس به repository است (مخصوصاً اگر secret است).

## 🔍 جزئیات
- علت: documented in .env.e
---
[scan #5 at 2026-06-07T16:59:01.848910+00:00]
## 📋 شرح
env var `API_PREFIX` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) reader حذف شده و باعث config drift می‌شود، یا (ب) leak اطلاعات حساس به repository است (مخصوصاً اگر secret است).

## 🔍 جزئیات
- علت: documented in .env.example/R
---
[scan #6 at 2026-06-07T16:59:01.878710+00:00]
## 📋 شرح
env var `API_TIMEOUT` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- [ ] `ACCOUNT_LOCKOUT_MINUTES` در هیچ کدی خوانده نمی‌شود (تأیید شده)
...
1. گام ۱: grep روی `ACCOUNT_LOCKOUT_MINUTES` در همه کدبیس + CI configs + Dockerfile.
...
1. `ACCOUNT_LOCKOUT_MINUTES` در هیچ کدی خوانده نمی‌شود (تأیید شده) _(verify: static)_
2. از `.env.example` و deployment configs حذف شد _(verify: static)_
3. اگر secret بوده، rotate شد و در deployment new value تنظیم شد _(verify: static)_
