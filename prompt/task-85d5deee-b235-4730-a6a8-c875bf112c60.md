---
task_id: 85d5deee-b235-4730-a6a8-c875bf112c60
title: ناسازگاری نسخهٔ requires-python (>=3.8) با Tech Stack واقعی (Python 3.11+) و syntax مدرن
type: bug
priority: high
...
created_at: '2026-06-14T16:38:17.943429+00:00'
updated_at: '2026-06-14T16:38:17.943433+00:00'
target_files:
- pyproject.toml
- pyproject.toml
---

# ناسازگاری نسخهٔ requires-python (>=3.8) با Tech Stack واقعی (Python 3.11+) و syntax مدرن

## Raw Idea

در pyproject.toml مقدار requires-python = ">=3.8" تعریف شده و classifiers هم تا 3.8 را پشتیبانی اعلام می‌کنند، در حالی که README صراحتاً «FastAPI (Python 3.11+)» و «Python 3.11+» را به‌عنوان prerequisite ذکر کرده است. این ناسازگاری باعث می‌شود نصب پروژه روی Python 3.8/3.9 موفق به‌نظر برسد ولی در runtime با ویژگی‌های مدرن نوع‌دهی یا کتابخانه‌ها شکست بخورد. علاوه‌براین در pyproject.toml تنظیمات mypy روی python_version = "3.8" قفل شده که با کد واقعی پروژه (که برای 3.11 نوشته شده) هم‌خوان نیست و type-checkهای CI نتایج گمراه‌کننده می‌دهند. این یک mismatch مستقیم بین اعلان dependency/runtime و محیط هدف است.
...
- `pyproject.toml:10-24` — `requires-python / classifiers` — اعلان requires-python و classifiers باید با 3.11+ هم‌خوان شود
  ```
  requires-python = ">=3.8"
  classifiers = [
      ...
  ...
  ```
- `pyproject.toml:120-121` — `[tool.mypy].python_version` — mypy روی 3.8 قفل است ولی کد برای 3.11 نوشته شده
  ```
  [tool.mypy]
  python_version = "3.8"
  ```
...
FastAPI + SQLAlchemy 2.0 + pydantic 2 + Python 3.11 (Docker/Render)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/Dockerfile` — ایمیج پایه احتمالاً Python 3.11 است و باید منبع حقیقت نسخه باشد
- `README.md` — Prerequisites صراحتاً Python 3.11+ را الزام می‌کند
- `.github/workflows/ci.yml` — نسخهٔ Python ست‌شده در CI باید با requires-python یکی باشد
- `backend/requirements.txt` — نسخه‌های pin‌شده باید با interpreter هدف سازگار باشند

## 🌐 نقشهٔ وابستگی‌ها
این اعلان نسخه روی نصب، CI، Docker build و نتایج mypy کل بک‌اند اثر می‌گذارد؛ تنها فایل اعلانی است ولی اثر سیستمی دارد.
...
- [ ] requires-python در pyproject.toml برابر ">=3.11" است
- [ ] classifiers فقط نسخه‌های 3.11 و بالاتر را شامل می‌شود
- [ ] [tool.mypy].python_version برابر "3.11" است
- [ ] نسخهٔ Python در Dockerfile و ci.yml با requires-python هم‌خوان است
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. requires-python را به ">=3.11" تغییر بده، classifiers را به 3.11/3.12 محدود کن، و [tool.mypy].python_version را به "3.11" به‌روزرسانی کن تا با Dockerfile/CI و README یکدست شود. اگر پشتیبانی 3.8 واقعاً لازم است، README باید اصلاح شود؛ ولی منبع حقیقت (Dockerfile/Tech Stack) 3.11 است.
...
## 🧪 دستورات اعتبارسنجی
- `python -c "import tomllib,sys;d=tomllib.load(open('pyproject.toml','rb'));assert d['project']['requires-python']=='>=3.11'"`
- `grep -n 'python_version' pyproject.toml`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر محیطی واقعاً روی 3.8/3.9 اجرا می‌شده، نصب آنجا شکست می‌خورد؛ ولی Tech Stack هدف 3.11 است.
...
1. requires-python در pyproject.toml برابر ">=3.11" است
2. classifiers فقط نسخه‌های 3.11 و بالاتر را شامل می‌شود
3. [tool.mypy].python_version برابر "3.11" است
4. نسخهٔ Python در Dockerfile و ci.yml با requires-python هم‌خوان است

## Task Steps

### Step 1: اصلاح requires-python در pyproject.toml به >=3.11
**Status:** `pending` (0%)
**Scope:** در فایل pyproject.toml مقدار requires-python = ">=3.8" را به ">=3.11" تغییر بده تا با prerequisite اعلام‌شده در README («Python 3.11+») هم‌خوان شود. این مرحله فقط شامل کلید requires-python است؛ تغییر classifiers و تنظیمات mypy خارج از این مرحله است و در مراحل بعد انجام می‌شود. نکتهٔ حیاتی: این تغییر باعث می‌شود pip هنگام نصب روی Python 3.8/3.9 صراحتاً خطا بدهد به‌جای اینکه نصب موفق به‌نظر برسد و در runtime شکست بخورد. مسیر دقیق فایل را همان pyproject.toml در ریشهٔ پروژه نگه دار.
**Excerpt:**
```
در pyproject.toml مقدار requires-python = ">=3.8" تعریف شده و classifiers هم تا 3.8 را پشتیبانی اعلام می‌کنند، در حالی که README صراحتاً «FastAPI (Python 3.11+)» و «Python 3.11+» را به‌عنوان prerequisite ذکر کرده است. این ناسازگاری باعث می‌شود نصب پروژه روی Python 3.8/3.9 موفق به‌نظر برسد ولی در runtime با ویژگی‌های مدرن نوع‌دهی یا کتابخانه‌ها شکست بخورد.
```

### Step 2: اصلاح classifiers در pyproject.toml برای حذف نسخه‌های قدیمی Python
**Status:** `pending` (0%)
**Scope:** در فایل pyproject.toml بخش classifiers را به‌روزرسانی کن تا فقط نسخه‌های پشتیبانی‌شدهٔ واقعی (Python 3.11 و بالاتر) را اعلام کند و classifierهای مربوط به 3.8/3.9/3.10 حذف شوند. این مرحله فقط شامل لیست classifiers است؛ تغییر requires-python (مرحلهٔ ۱) و تنظیمات mypy (مرحلهٔ ۳) خارج از scope این مرحله هستند. نکتهٔ حیاتی: classifiers باید با requires-python و README سازگار باشند تا روی PyPI و ابزارهای متادیتا اطلاعات گمراه‌کننده دربارهٔ پشتیبانی Python نمایش داده نشود.
**Excerpt:**
```
در pyproject.toml مقدار requires-python = ">=3.8" تعریف شده و classifiers هم تا 3.8 را پشتیبانی اعلام می‌کنند، در حالی که README صراحتاً «FastAPI (Python 3.11+)» و «Python 3.11+» را به‌عنوان prerequisite ذکر کرده است.
```

### Step 3: اصلاح python_version در تنظیمات mypy به 3.11
**Status:** `pending` (0%)
**Scope:** در فایل pyproject.toml تنظیم mypy که روی python_version = "3.8" قفل شده را به python_version = "3.11" تغییر بده تا با کد واقعی پروژه که برای 3.11 نوشته شده هم‌خوان شود. این مرحله فقط شامل کلید python_version در بخش mypy است؛ تغییرات requires-python و classifiers در مراحل ۱ و ۲ انجام شده‌اند و خارج از این scope هستند. نکتهٔ حیاتی: با این تغییر type-checkهای CI رفتار درست نسخهٔ 3.11 را ارزیابی می‌کنند و نتایج گمراه‌کننده‌ای که از فرض 3.8 ناشی می‌شد حذف می‌شود.
**Excerpt:**
```
علاوه‌براین در pyproject.toml تنظیمات mypy روی python_version = "3.8" قفل شده که با کد واقعی پروژه (که برای 3.11 نوشته شده) هم‌خوان نیست و type-checkهای CI نتایج گمراه‌کننده می‌دهند. این یک mismatch مستقیم بین اعلان dependency/runtime و محیط هدف است.
```
