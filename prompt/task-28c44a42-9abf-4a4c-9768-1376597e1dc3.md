---
task_id: 28c44a42-9abf-4a4c-9768-1376597e1dc3
title: 'dependency بلااستفاده: PyJWT (python)'
type: cleanup
priority: medium
...
created_at: '2026-06-07T16:59:03.153411+00:00'
updated_at: '2026-06-07T16:59:03.314427+00:00'
---

# dependency بلااستفاده: PyJWT (python)

## Raw Idea

## 📋 شرح
package `PyJWT` در `` declare شده ولی در هیچ import کدبیس مصرف نمی‌شود.
...
- نام package: `PyJWT`
- زبان: `python`
- علت: in requirements but no 'import pyjwt' or 'from pyjwt' found
---
[scan #2 at 2026-06-07T16:59:03.221794+00:00]
## 📋 شرح
...
- علت: in requiremen
---
[scan #3 at 2026-06-07T16:59:03.253371+00:00]
## 📋 شرح
package `asyncpg` در `` declare شده ولی در هیچ import کدبیس مصرف نمی‌شود.
...
---
[scan #4 at 2026-06-07T16:59:03.283480+00:00]
## 📋 شرح
package `bandit` در `` declare شده ولی در هیچ import کدبیس مصرف نمی‌شود.
...
---
[scan #5 at 2026-06-07T16:59:03.314421+00:00]
## 📋 شرح
package `celery` در `` declare شده ولی در هیچ import کدبیس مصرف نمی‌شود.
...
- [ ] `PyJWT` در هیچ direct import مصرف نمی‌شود + هیچ transitive dep ای آن را require نمی‌کند
...
1. گام ۱: `npm/pip list --depth=0` و grep import روی `PyJWT`.
...
1. `PyJWT` در هیچ direct import مصرف نمی‌شود + هیچ transitive dep ای آن را require نمی‌کند _(verify: static)_
2. package از manifest حذف شد + lockfile به‌روز _(verify: static)_
3. build/test/CI همچنان عبور می‌کند _(verify: static)_
