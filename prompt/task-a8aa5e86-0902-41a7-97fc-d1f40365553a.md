---
task_id: a8aa5e86-0902-41a7-97fc-d1f40365553a
title: 🔄 page.test.tsx نیازمند بررسی به‌خاطر تغییر در page.tsx
...
created_at: '2026-06-07T16:56:06.331184+00:00'
updated_at: '2026-06-07T22:07:14.979978+00:00'
target_files:
- frontend/src/app/facilities/page.test.tsx
---

# 🔄 page.test.tsx نیازمند بررسی به‌خاطر تغییر در page.tsx
...
## 🔍 جزئیات
- 📂 فایل تغییریافته: `frontend/src/app/facilities/page.tsx`
- 📂 فایل وابسته (نیازمند بررسی): `frontend/src/app/facilities/page.test.tsx`
- 🔥 risk level: **high**
- 📝 reason: The `facilities/page.tsx` file has undergone significant modifications. Key changes include:
1.  **API Call Parameters:** The `loadFacilities` function now passes several new parameters (`facility_type`, `status`, `amount_min`, `amount_max`, `sort_by`, `sort_order`) to `facilitiesApi.list`. Existing API mocks in the test file will likely need to be updated to correctly handle these new parameters.
...
- `frontend/src/app/facilities/page.tsx` — فایل upstream که تغییر کرده

## 🌐 نقشهٔ وابستگی‌ها
فایل `frontend/src/app/facilities/page.test.tsx` یکی از مصرف‌کنندگان `frontend/src/app/facilities/page.tsx` است.
...
- [ ] diff `frontend/src/app/facilities/page.tsx` بررسی شد و تأثیر بر `frontend/src/app/facilities/page.test.tsx` مستند شد
...
1. گام ۱: diff آخر در `frontend/src/app/facilities/page.tsx` را بخوان — چی عوض شده.
گام ۲: کد در `frontend/src/app/facilities/page.test.tsx` که این تغییر را مصرف می‌کند پیدا کن.
...
1. diff `frontend/src/app/facilities/page.tsx` بررسی شد و تأثیر بر `frontend/src/app/facilities/page.test.tsx` مستند شد _(verify: manual_only)_
2. کد dependent با contract جدید align شد _(verify: static)_
3. integration test که هر دو فایل را پوشش می‌دهد عبور می‌کند _(verify: ui_interaction)_
