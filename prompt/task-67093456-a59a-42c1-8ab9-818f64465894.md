---
task_id: 67093456-a59a-42c1-8ab9-818f64465894
title: "دکمه‌ی UI بدون handler: exportFiltered('xlsx')}\n            className=\"flex items-center gap-1 px-3 py-2"
type: bug_fix
priority: medium
...
created_at: '2026-06-07T16:58:59.580499+00:00'
updated_at: '2026-06-07T22:09:19.538905+00:00'
target_files:
- frontend/src/app/customers/page.tsx
---

# دکمه‌ی UI بدون handler: exportFiltered('xlsx')}
            className="flex items-center gap-1 px-3 py-2
...
## 🔍 جزئیات
- label/متن دکمه: `exportFiltered('xlsx')}
            className="flex items-center gap-1 px-3 py-2`
...
---
[scan #2 at 2026-06-07T16:58:59.625356+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/customers/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `exportFiltered('csv')}
            className="flex items-center gap-1 p
---
[scan #3 at 2026-06-07T16:58:59.647816+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/customers/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
- label/متن دکمه: `{ setEditingCustomer(null); setShowForm(true) }}
            className=
---
[scan #4 at 2026-06-07T16:58:59.670604+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/customers/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setSelected(new Set())} className="px-3 py-1.5 text-sm border rounded-l
---
[scan #5 at 2026-06-07T16:58:59.693070+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/app/customers/page.tsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `router.push(`/customer-detail?id=${customer.id}`)}
...
- [ ] git blame مشخص می‌کند چرا این دکمه `exportFiltered('xlsx')}
            className="flex items-center gap-1 px-3 py-2` فاقد handler است
...
1. git blame مشخص می‌کند چرا این دکمه `exportFiltered('xlsx')}
            className="flex items-center gap-1 px-3 py-2` فاقد handler است _(verify: manual_only)_
