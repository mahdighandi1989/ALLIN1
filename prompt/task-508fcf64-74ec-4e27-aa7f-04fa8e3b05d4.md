---
task_id: 508fcf64-74ec-4e27-aa7f-04fa8e3b05d4
title: سرویس حیاتی data_merge.py (هاب با 5+ مصرف‌کننده) فاقد تست واحد مستقیم است
type: other
priority: high
...
created_at: '2026-06-14T16:39:12.654505+00:00'
updated_at: '2026-06-14T16:39:12.654509+00:00'
target_files:
- backend/app/services/data_merge.py
---

# سرویس حیاتی data_merge.py (هاب با 5+ مصرف‌کننده) فاقد تست واحد مستقیم است

## Raw Idea

فایل backend/app/services/data_merge.py یکی از hubهای اصلی پروژه است که داده‌های legacy اکسل (مشتریان، تسهیلات، ضامن‌ها، KYC) را در دیتابیس ادغام می‌کند و طبق نقشهٔ Importهای داخلی، models/customer.py و models/facility.py و models/guarantor.py را import می‌کند. این سرویس در startup به‌صورت خودکار اجرا می‌شود (طبق settings/page.tsx و توضیحات runMerge) و در صورت خطا کل bootstrap اپ آسیب می‌بیند. با وجود اهمیت بالا، در لیست فایل‌های test هیچ test_data_merge.py مستقیمی دیده نمی‌شود (تنها test_customer_listing_merge.py وجود دارد که سطح API را پوشش می‌دهد، نه منطق ادغام را). نبود تست واحد روی منطق merge یعنی regressionهای ادغام (مثل از دست رفتن رکورد، overwrite اشتباه، یا fail در startup) بی‌صدا از CI عبور می‌کنند.
...
- `backend/app/services/data_merge.py:1-50` — `run_merge / merge_status` — هاب با حداقل 5 مصرف‌کننده طبق نقشهٔ import؛ فاقد تست واحد مستقیم
  ```python
  # سرویس ادغام داده legacy — import می‌کند customer.py, facility.py, guarantor.py, crm.py
  ```
...
FastAPI + SQLAlchemy 2.0 async + pytest-asyncio + aiosqlite

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/routers/crm.py` — endpoint runMerge/mergeStatus را expose می‌کند
- `frontend/src/app/settings/page.tsx` (سطر 98) — runMerge و mergeStatus را call می‌کند
- `backend/tests/conftest.py` — fixture دیتابیس in-memory برای تست
- `backend/tests/test_customer_listing_merge.py` — تست سطح API موجود — نه تست واحد منطق merge
- `backend/app/database.py` — `data_merge.py` این فایل را import می‌کند
- `backend/app/models/customer.py` — `data_merge.py` این فایل را import می‌کند
- `backend/app/models/facility.py` — `data_merge.py` این فایل را import می‌کند
- `backend/app/models/guarantor.py` — `data_merge.py` این فایل را import می‌کند
- `backend/app/db_init.py` — این فایل `data_merge.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
data_merge.py مدل‌های customer/facility/guarantor/crm را import می‌کند و در startup (lifespan) و از طریق روتر crm فراخوانی می‌شود؛ نبود تست واحد یعنی خطای ادغام مستقیماً bootstrap و داشبورد را می‌شکند.
...
- [ ] فایل backend/tests/test_data_merge.py ایجاد شود
- [ ] تست ادغام رکورد جدید موفق باشد و رکورد در دیتابیس ساخته شود
- [ ] تست idempotency: اجرای دوبارهٔ merge نباید رکورد تکراری بسازد
- [ ] تست رفتار در برابر فایل JSON ناقص یا خراب (نباید exception مدیریت‌نشده بدهد)
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد backend/tests/test_data_merge.py با تست‌های واحد برای توابع اصلی data_merge: ادغام موفق رکوردهای جدید، update رکوردهای موجود (idempotency)، رفتار در برابر JSON خراب/ناقص، و خروجی mergeStatus. از fixture دیتابیس in-memory (sqlite+aiosqlite) موجود در conftest.py استفاده شود.

## 💡 نمونه‌های قبل/بعد
**تست idempotency merge**

_قبل:_
```
# هیچ تستی برای merge وجود ندارد
```
...
async def test_merge_is_idempotent(db_session):
    await run_merge()
    first = await merge_status()
    await run_merge()
    second = await merge_status()
    assert first == second
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_data_merge.py -v`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به mock کردن مسیر فایل‌های JSON در backend/app/data/merge/؛ احتمال coupling به ساختار واقعی داده
...
1. فایل backend/tests/test_data_merge.py ایجاد شود
2. تست ادغام رکورد جدید موفق باشد و رکورد در دیتابیس ساخته شود
3. تست idempotency: اجرای دوبارهٔ merge نباید رکورد تکراری بسازد
4. تست رفتار در برابر فایل JSON ناقص یا خراب (نباید exception مدیریت‌نشده بدهد)

## Task Steps

### Step 1: بررسی ساختار و رفتار فعلی data_merge.py و تست‌های موجود
**Status:** `pending` (0%)
**Scope:** این مرحله شامل خواندن کامل فایل backend/app/services/data_merge.py و فهمیدن منطق ادغام دادهٔ legacy اکسل (مشتریان، تسهیلات، ضامن‌ها، KYC) است. باید importهای داخلی به models/customer.py و models/facility.py و models/guarantor.py بررسی شوند، نقطهٔ ورود runMerge و نحوهٔ اجرای خودکار در startup شناسایی شود، و تست موجود test_customer_listing_merge.py مرور شود تا مشخص شود چه چیزی (سطح API) پوشش داده شده و چه چیزی (منطق merge) پوشش داده نشده. خارج از این مرحله: نوشتن هیچ تست جدید یا تغییر کد production. نکتهٔ حیاتی: هیچ test_data_merge.py مستقیمی وجود ندارد و این مرحله صرفاً context-gathering است نه پیاده‌سازی.
**Excerpt:**
```
فایل backend/app/services/data_merge.py یکی از hubهای اصلی پروژه است که داده‌های legacy اکسل (مشتریان، تسهیلات، ضامن‌ها، KYC) را در دیتابیس ادغام می‌کند و طبق نقشهٔ Importهای داخلی، models/customer.py و models/facility.py و models/guarantor.py را import می‌کند. این سرویس در startup به‌صورت خودکار اجرا می‌شود (طبق settings/page.tsx و توضیحات runMerge) و در صورت خطا کل bootstrap اپ آسیب می‌بیند.
```

### Step 2: ایجاد فایل تست واحد test_data_merge.py برای منطق ادغام
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ساخت فایل تست مستقیم test_data_merge.py است که منطق ادغام در backend/app/services/data_merge.py را در سطح واحد پوشش می‌دهد — نه سطح API (که test_customer_listing_merge.py از قبل آن را پوشش می‌دهد). تست‌ها باید سناریوهای ادغام دادهٔ legacy اکسل برای مشتریان، تسهیلات، ضامن‌ها و KYC را مستقیماً روی توابع merge اجرا کنند. خارج از این مرحله: تست edge case های regression خاص (که در مراحل بعد می‌آید) و تغییر منطق production. نکتهٔ حیاتی: این فایل نباید منطق سطح API را دوباره تست کند، بلکه باید روی منطق داخلی merge تمرکز کند تا coverage واقعی منطق ادغام ایجاد شود.
**Excerpt:**
```
با وجود اهمیت بالا، در لیست فایل‌های test هیچ test_data_merge.py مستقیمی دیده نمی‌شود (تنها test_customer_listing_merge.py وجود دارد که سطح API را پوشش می‌دهد، نه منطق ادغام را). نبود تست واحد روی منطق merge یعنی regressionهای ادغام ... بی‌صدا از CI عبور می‌کنند.
```

### Step 3: افزودن تست‌های regression برای از دست رفتن رکورد و overwrite اشتباه در merge
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن سناریوهای تست regression خاص به test_data_merge.py است که سه نوع خطای ذکرشده توسط کاربر را پوشش می‌دهد: (۱) از دست رفتن رکورد (record loss)، (۲) overwrite اشتباه دادهٔ موجود، و (۳) fail در startup. هر سناریو باید به‌صورت یک تست مجزا با داده‌های ورودی کنترل‌شده و assert روی نتیجهٔ نهایی پایگاه داده پیاده شود. خارج از این مرحله: ساخت اولیهٔ فایل تست (مرحلهٔ ۲) و تغییر منطق production. نکتهٔ حیاتی: هدف این است که این کلاس از regressionها دیگر بی‌صدا از CI عبور نکنند و در صورت بروز، تست fail شود.
**Excerpt:**
```
نبود تست واحد روی منطق merge یعنی regressionهای ادغام (مثل از دست رفتن رکورد، overwrite اشتباه، یا fail در startup) بی‌صدا از CI عبور می‌کنند. این سرویس در startup به‌صورت خودکار اجرا می‌شود (طبق settings/page.tsx و توضیحات runMerge) و در صورت خطا کل bootstrap اپ آسیب می‌بیند.
```
