---
task_id: cbcad5c4-bf54-4e09-871c-f4b0f74e00ad
title: Missing server-side rate limiting for import operations.
type: security
priority: high
...
# Missing server-side rate limiting for import operations.

## Raw Idea

The `data-import/README.md` explicitly states 'Rate limiting: Maximum 10 import operations per hour per user' as a security measure. However, the provided backend code snippets do not show any implementation of this rate limit for the import endpoints (`/api/imports`). Without server-side rate limiting, an attacker could potentially flood the import system with numerous requests, leading to denial-of-service, resource exhaustion, or bypassing other security checks by overwhelming the system. This contradicts the documented security policy.
...
- `backend/app/routers/imports.py:195` — `imports.router` — The import router defines the endpoints that require rate limiting.
  ```python
  195: app.include_router(imports.router, prefix="/api/imports", tags=["imports"])
  ```
- `backend/app/utils/rate_limit.py:util` — `rate_limit` — Existing rate limiting utility that could be extended or serve as a pattern for import rate limiting.
  ```python
  util  backend/app/utils/rate_limit.py
  ```
...
FastAPI, Redis (for rate limit storage).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `data-import/README.md` (سطر 34) — Documents the claimed rate limiting feature for imports.
- `backend/requirements.txt` — May need new dependencies for rate limiting if not using existing utilities.
- `backend/app/main.py` (سطر 107) — If a new middleware is needed for rate limiting, it would be registered here.
...
## 🌐 نقشهٔ وابستگی‌ها
Backend security feature, impacts import API endpoints and overall system resilience against DoS attacks.
...
- [ ] Import endpoints (`/api/imports/*`) are rate-limited to 10 operations per user per hour.
- [ ] Users exceeding the rate limit receive a 429 HTTP status code with an informative message.
- [ ] Rate limit status and violations are logged for monitoring.
- [ ] New unit or integration tests are added to verify the rate limiting functionality for imports.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a server-side rate limiter for the `/api/imports` endpoints. This can be achieved by extending the existing rate limiting logic in `backend/app/utils/rate_limit.py` or by integrating a dedicated FastAPI rate limiting library (e.g., `fastapi-limiter`). The rate limiter should track import requests per user (e.g., using Redis for storage) and reject requests exceeding the defined threshold (10 per hour) with a 429 HTTP status code.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Send more than 10 import requests within an hour from the same user and verify that subsequent requests receive a 429 response.`
- `Review the `backend/app/routers/imports.py` and `backend/app/utils/rate_limit.py` code for rate limiting implementation.`
- `Run `pytest backend/tests/test_imports.py` (after adding new tests for rate limiting).`

## ⚠️ ریسک‌ها و موارد احتیاط
Low to medium, potential for incorrect configuration leading to legitimate users being blocked or the rate limit being ineffective.
...
### Step 1: تحقیق و انتخاب استراتژی محدودیت نرخ (Rate Limiting)
**Status:** `pending` (0%)
**Scope:** این مرحله بنیادی شامل تحقیق کامل در مورد کتابخانه‌ها، فریم‌ورک‌ها یا الگوهای پیاده‌سازی سفارشی محدودیت نرخ موجود است که برای پشته فناوری بک‌اند فعلی مناسب باشند. هدف انتخاب قوی‌ترین، کارآمدترین و قابل نگهداری‌ترین استراتژی است که بتواند سیاست «حداکثر ۱۰ عملیات ایمپورت در ساعت برای هر کاربر» را اعمال کند. این مرحله صراحتاً شامل هیچ پیاده‌سازی کدی نیست و صرفاً بر فرآیند تصمیم‌گیری و تشریح رویکرد انتخابی تمرکز دارد.
**Excerpt:**
```
The `data-import/README.md` explicitly states 'Rate limiting: Maximum 10 import operations per hour per user' as a security measure. However, the provided backend code snippets do not show any implementation of this rate limit for the import endpoints (`/api/imports`). Without server-side rate limiting, an attacker could potentially flood the import system with numerous requests, leading to denial-of-service, resource exhaustion, or bypassing other security checks by overwhelming the system. This contradicts the documented security policy.
```

### Step 2: پیاده‌سازی منطق اصلی محدودیت نرخ (Core Rate Limiting Logic)
**Status:** `pending` (0%)
**Scope:** این مرحله بر توسعه کامپوننت یا میان‌افزار عمومی و قابل استفاده مجدد تمرکز دارد که منطق اصلی محدودیت نرخ را بر اساس استراتژی انتخاب شده در مرحله ۱ کپسوله می‌کند. این کامپوننت باید به دقت قانون «حداکثر ۱۰ عملیات ایمپورت در ساعت برای هر کاربر» را ردیابی و اعمال کند و کاربران را شناسایی کرده و شمارش‌ها را به صورت ساعتی بازنشانی کند. این کامپوننت باید به گونه‌ای طراحی شود که به راحتی در بخش‌های مختلف برنامه ادغام شود، اما این مرحله شامل ادغام واقعی با نقاط پایانی API خاص نیست.
**Excerpt:**
```
The `data-import/README.md` explicitly states 'Rate limiting: Maximum 10 import operations per hour per user' as a security measure. However, the provided backend code snippets do not show any implementation of this rate limit for the import endpoints (`/api/imports`). Without server-side rate limiting, an attacker could potentially flood the import system with numerous requests, leading to denial-of-service, resource exhaustion, or bypassing other security checks by overwhelming the system.
```

### Step 3: ادغام محدودیت نرخ با نقاط پایانی `/api/imports`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اعمال کامپوننت محدودیت نرخ توسعه یافته در مرحله ۲ به تمام نقاط پایانی API مرتبط تحت مسیر `/api/imports` است. این به معنای اصلاح تعاریف نقطه پایانی (مانند افزودن دکوراتورها به توابع view، پیکربندی میان‌افزار در لایه مسیریابی) برای اطمینان از اینکه هر درخواست به این نقاط پایانی ایمپورت مشمول قانون «حداکثر ۱۰ عملیات ایمپورت در ساعت برای هر کاربر» می‌شود. تمرکز صرفاً بر ادغام است، نه بر اصلاح منطق اصلی محدودیت نرخ.
**Excerpt:**
```
However, the provided backend code snippets do not show any implementation of this rate limit for the import endpoints (`/api/imports`). Without server-side rate limiting, an attacker could potentially flood the import system with numerous requests, leading to denial-of-service, resource exhaustion, or bypassing other security checks by overwhelming the system. This contradicts the documented security policy.
```

### Step 4: افزودن تست‌های واحد و یکپارچه‌سازی برای محدودیت نرخ
**Status:** `pending` (0%)
**Scope:** این مرحله به ایجاد مجموعه‌ای جامع از تست‌های خودکار، شامل تست‌های واحد برای منطق اصلی محدودیت نرخ و تست‌های یکپارچه‌سازی برای اعمال آن در نقاط پایانی `/api/imports` اختصاص دارد. تست‌ها باید سناریوهایی مانند درخواست‌های موفق در محدوده، درخواست‌های رد شده که از حد مجاز فراتر می‌روند، مدیریت صحیح کاربران مختلف و بازنشانی صحیح شمارش پس از پنجره ساعتی را پوشش دهند. این امر تضمین می‌کند که مکانیزم محدودیت نرخ به طور قابل اعتماد و طبق برنامه عمل می‌کند.
**Excerpt:**
```
Without server-side rate limiting, an attacker could potentially flood the import system with numerous requests, leading to denial-of-service, resource exhaustion, or bypassing other security checks by overwhelming the system. This contradicts the documented security policy.
```

### Step 5: به‌روزرسانی `data-import/README.md` با جزئیات پیاده‌سازی
**Status:** `pending` (0%)
**Scope:** این مرحله نهایی شامل بررسی فایل `data-import/README.md` و هر مستندات توسعه‌دهنده مرتبط دیگر برای اطمینان از اینکه به طور دقیق نحوه پیاده‌سازی محدودیت نرخ را منعکس می‌کند، است. در حالی که سیاست قبلاً بیان شده است، جزئیات مربوط به پیاده‌سازی انتخاب شده (مانند کتابخانه خاص مورد استفاده، پارامترهای پیکربندی، مکان میان‌افزار/دکوراتور) باید اضافه یا به‌روزرسانی شوند. اگر مستندات موجود بدون جزئیات پیاده‌سازی کافی تشخیص داده شود، یک کامیت no-op با توضیح این تصمیم انجام خواهد شد.
**Excerpt:**
```
The `data-import/README.md` explicitly states 'Rate limiting: Maximum 10 import operations per hour per user' as a security measure. ... This contradicts the documented security policy.
```
