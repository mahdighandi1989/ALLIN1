---
task_id: b7a433b2-2f6f-4c3f-8f0a-856ab98f3e2f
title: Clarify database schema management responsibilities between Alembic and `init_database`.
type: refactor
priority: medium
...
created_at: '2026-06-07T16:57:40.879328+00:00'
updated_at: '2026-06-07T22:08:02.011223+00:00'
target_files:
- backend/app/main.py
- backend/app/db_init.py
---

# Clarify database schema management responsibilities between Alembic and `init_database`.

## Raw Idea

The `lifespan` function in `backend/app/main.py` calls `init_database()`, which is described as 'Self-heal the DB schema to match the models (independent of Alembic)'. While this can be convenient for development or initial setup, relying on an application-level 'self-healing' mechanism for schema changes in production can lead to inconsistencies, mask issues with formal migrations, or even cause data loss if not handled with extreme care. The `docs/DATABASE_SCHEMA.md` also emphasizes the importance of keeping backend models and database migrations in sync. For a robust system, Alembic migrations should be the single source of truth for schema evolution, and `init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification.
...
- `backend/app/main.py:33-45` — `lifespan` — The `init_database` call within the lifespan function needs its role clarified.
  ```python
  @asynccontextmanager
  async def lifespan(app: FastAPI):
      ...
      yield
  ```
- `backend/app/db_init.py:1-100` — `init_database` — The `init_database` function itself needs to be refactored to remove schema-altering logic.
  ```python
  from app.database import engine
  from app.models import Base
  
  async def init_database():
      # Current implementation (conceptual, as file content not provided) likely includes:
      # async with engine.begin() as conn:
      #     await conn.run_sync(Base.metadata.create_all)
      # This part should be removed or guarded.
  ```
...
FastAPI, SQLAlchemy 2.0, Alembic, PostgreSQL, Docker/CI/CD.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `docs/DATABASE_SCHEMA.md` (سطر 30) — Mentions the importance of syncing models and migrations, which this change addresses.
- `backend/migrations/versions/001_initial_create_tables.py` (سطر 1) — Alembic migration scripts are the authoritative source for schema changes.
- `backend/Dockerfile` (سطر 1) — Deployment configuration where Alembic migrations should be explicitly run.
- `docker-compose.prod.yml` (سطر 1) — Deployment configuration where Alembic migrations should be explicitly run.
...
## 🌐 نقشهٔ وابستگی‌ها
This change affects the application's startup sequence and how database schema changes are managed. It requires coordination between application code and deployment scripts.
...
- [ ] The `init_database()` function in `backend/app/db_init.py` is refactored to perform only idempotent data seeding and *not* schema 'self-healing' (e.g., `Base.metadata.create_all`).
- [ ] The deployment process (e.g., `Dockerfile` or `build.sh`) includes an explicit step to run `alembic upgrade head` before starting the FastAPI application.
- [ ] The `docs/DATABASE_SCHEMA.md` or a new ADR clearly states that Alembic is the primary schema management tool and `init_database` is for data seeding.
- [ ] The application starts successfully in both development and production environments, with the correct schema and seeded data.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. **Refactor `init_database`**: Modify the `init_database` function in `backend/app/db_init.py` to remove any logic that implicitly alters the database schema to match models. Instead, it should focus solely on idempotent data seeding (e.g., creating default users, settings, or demo data if the database is empty). 2. **Enforce Alembic**: Ensure that Alembic migrations are explicitly run as part of the deployment process (e.g., in `Dockerfile` or CI/CD scripts) *before* the FastAPI application starts. 3. **Update Documentation**: Add a clear statement in `docs/DATABASE_SCHEMA.md` or `docs/decisions.md` that Alembic is the authoritative tool for schema management, and `init_database` is for data seeding only.

## 💡 نمونه‌های قبل/بعد
**Before (backend/app/main.py, lines 39-42)**

_قبل:_
```
try:
        from app.db_init import init_database
        await init_database() # Self-heals the DB schema
    except Exception as exc:  # never let startup hard-crash on bootstrap issues
```
...
try:
        from app.db_init import init_database
        await init_database() # Now only seeds or performs idempotent setup
    except Exception as exc:  # never let startup hard-crash on bootstrap issues
```

**After (Conceptual change in `backend/app/db_init.py`)**

_قبل:_
```
async def init_database():
    # Current implementation likely includes Base.metadata.create_all
    pass
```
...
```
async def init_database():
    # This function should NOT create tables from models directly.
    # Instead, it should focus on idempotent data seeding.
    # Example: Check if a specific table exists or if a default user exists.
    # If the database is empty, seed initial data.
    # For schema updates, rely on Alembic.
    pass # Placeholder for actual idempotent seeding logic
```
...
## 🧪 دستورات اعتبارسنجی
- `Run `alembic downgrade base` then `alembic upgrade head` to ensure migrations work as expected.`
- `Build and run the Docker containers (`docker-compose up --build`) and verify the application starts and functions correctly.`
- `Verify that initial data (e.g., admin user) is present after a fresh database setup.`

## ⚠️ ریسک‌ها و موارد احتیاط
High risk if not implemented carefully. Incorrect refactoring could lead to schema mismatches, application startup failures, or data integrity issues if migrations are not properly applied. Requires thorough testing of the entire deployment and database initialization pipeline.
...
1. The `init_database()` function in `backend/app/db_init.py` is refactored to perform only idempotent data seeding and *not* schema 'self-healing' (e.g., `Base.metadata.create_all`). _(verify: static)_
2. The deployment process (e.g., `Dockerfile` or `build.sh`) includes an explicit step to run `alembic upgrade head` before starting the FastAPI application. _(verify: static)_
3. The `docs/DATABASE_SCHEMA.md` or a new ADR clearly states that Alembic is the primary schema management tool and `init_database` is for data seeding. _(verify: static)_
4. The application starts successfully in both development and production environments, with the correct schema and seeded data. _(verify: api_response)_

## Task Steps

### Step 1: Refactor `init_database()` to remove schema modification logic
**Status:** `pending` (0%)
**Scope:** این مرحله بر شناسایی و حذف هرگونه کدی در تابع `init_database()` تمرکز دارد که عملیات DDL (Data Definition Language) را انجام می‌دهد، مانند ایجاد، تغییر یا حذف جداول/ستون‌ها بر اساس مدل‌های برنامه. هدف نهایی حذف رفتار «خود-ترمیم‌کننده» شمای دیتابیس است. این مرحله شامل افزودن منطق جدید برای seed کردن داده یا بررسی‌های اولیه نیست و تابع `lifespan` را نیز تغییر نمی‌دهد.
**Excerpt:**
```
The `lifespan` function in `backend/app/main.py` calls `init_database()`, which is described as 'Self-heal the DB schema to match the models (independent of Alembic)'. While this can be convenient for development or initial setup, relying on an application-level 'self-healing' mechanism for schema changes in production can lead to inconsistencies, mask issues with formal migrations, or even cause data loss if not handled with extreme care. ... `init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification.
```

### Step 2: Implement idempotent data seeding in `init_database()`
**Status:** `pending` (0%)
**Scope:** این مرحله تضمین می‌کند که تابع `init_database()`، پس از حذف منطق تغییر شمای دیتابیس، به درستی seed کردن داده‌های idempotent را پیاده‌سازی یا تأیید می‌کند. این به معنای افزودن یا تنظیم منطق برای درج داده‌های اولیه ضروری در دیتابیس تنها در صورتی است که از قبل وجود نداشته باشند، بدون ایجاد تکرار یا خطا در اجراهای بعدی. این مرحله شامل تغییرات شمای دیتابیس یا سایر بررسی‌های اولیه نیست.
**Excerpt:**
```
...`init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification.
```

### Step 3: Implement initial checks in `init_database()`
**Status:** `pending` (0%)
**Scope:** این مرحله بر پیاده‌سازی یا تأیید اینکه `init_database()` شامل بررسی‌های اولیه ضروری است، تمرکز دارد. این بررسی‌ها می‌توانند شامل تأیید اتصال دیتابیس، بررسی پیکربندی‌های حیاتی، یا اطمینان از دسترسی به سرویس‌های خارجی لازم باشند، بدون انجام تغییرات شمای دیتابیس یا seed کردن گسترده داده. بررسی‌ها باید غیرمخرب و idempotent باشند.
**Excerpt:**
```
...`init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification.
```

### Step 4: Audit and verify Alembic as the single source of truth for schema evolution
**Status:** `pending` (0%)
**Scope:** این مرحله شامل یک فرآیند ممیزی و تأیید برای اطمینان از اینکه Alembic در واقع مکانیزم انحصاری برای مدیریت تغییرات شمای دیتابیس است. این شامل بررسی اسکریپت‌های مهاجرت موجود، اطمینان از اینکه هیچ جزء دیگری از برنامه تلاش برای عملیات DDL نمی‌کند، و تأیید اینکه گردش کار توسعه، Alembic را برای تمام تغییرات شمای دیتابیس الزامی می‌کند. این مرحله شامل ایجاد مهاجرت‌های جدید نیست، بلکه تأیید تنظیمات و فرآیند موجود است.
**Excerpt:**
```
For a robust system, Alembic migrations should be the single source of truth for schema evolution, and `init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification. The `docs/DATABASE_SCHEMA.md` also emphasizes the importance of keeping backend models and database migrations in sync.
```

### Step 5: Update `docs/DATABASE_SCHEMA.md` for `init_database()`'s new role
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی فایل `docs/DATABASE_SCHEMA.md` برای بازتاب دقیق نقش بازنگری شده تابع `init_database()` است. مستندات باید به وضوح بیان کنند که `init_database()` اکنون صرفاً برای seed کردن داده‌های idempotent و بررسی‌های اولیه است و صراحتاً ذکر کند که تغییرات شمای دیتابیس منحصراً توسط مهاجرت‌های Alembic مدیریت می‌شوند. این کار تضمین می‌کند که بین رفتار کد و مستندات پروژه سازگاری وجود دارد.
**Excerpt:**
```
The `docs/DATABASE_SCHEMA.md` also emphasizes the importance of keeping backend models and database migrations in sync. For a robust system, Alembic migrations should be the single source of truth for schema evolution, and `init_database()` should be strictly for idempotent data seeding or initial checks, not for schema modification.
```
