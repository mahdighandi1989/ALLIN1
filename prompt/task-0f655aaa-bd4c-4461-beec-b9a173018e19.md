---
task_id: 0f655aaa-bd4c-4461-beec-b9a173018e19
title: Endpoint تشخیصی /api/simulate-unhandled-error بدون احراز هویت در production قابل دسترسی است و عمداً خطا می‌اندازد
type: security
priority: high
...
created_at: '2026-06-14T16:36:54.663382+00:00'
updated_at: '2026-06-14T16:36:54.663385+00:00'
target_files:
- backend/app/main.py
---

# Endpoint تشخیصی /api/simulate-unhandled-error بدون احراز هویت در production قابل دسترسی است و عمداً خطا می‌اندازد

## Raw Idea

در main.py یک endpoint با مسیر GET /api/simulate-unhandled-error تعریف شده که عمداً RuntimeError می‌اندازد تا مسیر مانیتورینگ خطا تست شود. این endpoint نه auth دارد، نه include_in_schema=False، و نه محدود به محیط non-production است. هر کاربر ناشناسی می‌تواند آن را فراخوانی کند و باعث افزایش مصنوعی شمارندهٔ http_unhandled_errors_total و آلوده‌شدن لاگ‌ها و آلارم‌های production شود (نوعی DoS سبک روی observability). برخلاف /metrics که include_in_schema=False شده، این مسیر در OpenAPI عمومی نیز نمایان است.
...
- `backend/app/main.py:216-223` — `simulate_unhandled_error` — endpoint عمومی بدون گارد محیط/auth که عمداً 500 می‌اندازد
  ```python
...
FastAPI + Prometheus client + structlog

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/monitoring.py` — شمارندهٔ UNHANDLED_ERRORS که با این endpoint افزایش می‌یابد
- `backend/tests/e2e/test_global_exception_handling.py` — تستی که این endpoint را برای بررسی error_id فراخوانی می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این مسیر فقط برای تست مانیتورینگ است اما در production هم فعال می‌ماند و روی متریک‌ها و لاگ‌های جهانی اثر می‌گذارد. گارد محیط روی تست e2e موجود اثر می‌گذارد و باید با settings تست هماهنگ شود.
...
- [ ] در production فراخوانی GET /api/simulate-unhandled-error کد 404 برمی‌گرداند
- [ ] این endpoint با include_in_schema=False از OpenAPI عمومی حذف می‌شود
- [ ] در محیط غیر-production رفتار قبلی (500 با error_id) حفظ می‌شود و تست e2e موجود سبز می‌ماند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. این endpoint را پشت بررسی محیط قرار بده تا فقط وقتی settings.is_production() False است register یا اجرا شود؛ یا حداقل include_in_schema=False و یک گارد در ابتدای تابع اضافه کن که در production یک 404 برگرداند. بهتر است کل ثبت route مشروط به محیط توسعه باشد.
...
_بعد:_
```
@app.get("/api/simulate-unhandled-error", include_in_schema=False)
async def simulate_unhandled_error():
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/e2e/test_global_exception_handling.py`
- `curl -s -o /dev/null -w '%{http_code}' http://localhost:8000/api/simulate-unhandled-error`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر تست e2e انتظار 500 در محیط تست داشته باشد، باید settings تست non-production نگه داشته شود؛ ریسک پایین
...
1. در production فراخوانی GET /api/simulate-unhandled-error کد 404 برمی‌گرداند
2. این endpoint با include_in_schema=False از OpenAPI عمومی حذف می‌شود
3. در محیط غیر-production رفتار قبلی (500 با error_id) حفظ می‌شود و تست e2e موجود سبز می‌ماند
