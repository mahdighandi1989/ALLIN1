---
task_id: d2d7ee48-30d6-4c39-b64a-0ecd3ed01b3b
title: 'endpoint بک‌اند بلااستفاده: GET /{offer_id}'
type: audit
priority: medium
...
created_at: '2026-06-07T16:59:01.258455+00:00'
updated_at: '2026-06-07T22:11:43.382602+00:00'
target_files:
...
---
[scan #2 at 2026-06-07T16:59:01.311230+00:00]
## 📋 شرح
endpoint `PUT /{offer_id}` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `PUT`
- path: `/{offer_id}`
- فایل: `backend/app/routers/offer_letters.py`
- علت: no frontend fetch + no recent
---
[scan #3 at 2026-06-07T16:59:01.338638+00:00]
## 📋 شرح
endpoint `POST /{offer_id}/generate-schedule` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{offer_id}/generate-schedule`
- فایل: `backend/app/routers/offer_letters.py
---
[scan #4 at 2026-06-07T16:59:01.366176+00:00]
## 📋 شرح
endpoint `POST /{offer_id}/status` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{offer_id}/status`
- فایل: `backend/app/routers/offer_letters.py`
- علت: no frontend f
---
[scan #5 at 2026-06-07T16:59:01.393521+00:00]
## 📋 شرح
endpoint `DELETE /{offer_id}` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `DELETE`
- path: `/{offer_id}`
- فایل: `backend/app/routers/offer_letters.py`
- علت: no frontend fetch + no 
---
[scan #6 at 2026-06-07T16:59:01.422073+00:00]
## 📋 شرح
endpoint `POST /{offer_id}/restore` در `backend/app/routers/offer_letters.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{offer_id}/restore`
- فایل: `backend/app/routers/offer_letters.py`
- علت: no frontend
...
1. مشخص شد endpoint `GET /{offer_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
