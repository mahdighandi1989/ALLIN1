---
task_id: 87239b0a-8d03-44bc-8959-5d0cdcdf63ac
title: Inconsistent and limited client-side validation feedback for `FacilityFormModal`.
type: refactor
priority: medium
...
created_at: '2026-06-07T16:56:38.930802+00:00'
updated_at: '2026-06-07T22:07:47.716292+00:00'
target_files:
- frontend/src/app/facilities/page.tsx
---

# Inconsistent and limited client-side validation feedback for `FacilityFormModal`.

## Raw Idea

The `FacilityFormModal` component in `frontend/src/app/facilities/page.tsx` uses `validateFacilityForm` which appears to return a single error string, leading to only one validation message being displayed at a time via `toast.error()`. In contrast, the `CustomerForm` (partially shown in `frontend/src/app/customers/page.tsx`) uses `validateCustomerForm` which returns an object of errors, allowing for more granular and user-friendly feedback by displaying errors next to their respective input fields. This inconsistency and limited feedback can frustrate users and make it harder to correct multiple validation issues, impacting data quality and user experience.
...
- `frontend/src/app/facilities/page.tsx:372-375` — `FacilityFormModal` — This snippet shows where the validation error is currently handled.
  ```tsx
  372:     const validationError = validateFacilityForm(form)
  373:     if (validationError) {
  374:       toast.error(validationError)
  375:       return
  ```
...
Next.js 14 (React 18) functional component with hooks, `react-hot-toast`.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/customers/page.tsx` (سطر 338) — Provides a better pattern for client-side validation error handling (lines 338-375).
- `frontend/src/lib/facilityValidation.ts` — This file (implied) contains the definition of `validateFacilityForm` and needs to be updated to return an error object.
- `frontend/src/types.ts` — May need updates for `FacilityFormErrors` type definition.
- `frontend/src/app/facilities/page.test.tsx` — این فایل `page.tsx` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
Frontend validation logic and UI. Requires changes to a validation utility and the form component.
...
- [ ] `validateFacilityForm` should return an object mapping field names to error messages.
- [ ] `FacilityFormModal` should store and display multiple validation errors simultaneously.
- [ ] Error messages should appear next to their corresponding input fields.
- [ ] A generic toast message should indicate form errors, with details shown on fields.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Modify `validateFacilityForm` (presumably in `src/lib/facilityValidation.ts`) to return an object of errors (e.g., `Partial<Record<FacilityTextField, string>>`) similar to `CustomerFormErrors`. Then, update `FacilityFormModal` to store these errors in its state and display them next to the relevant input fields, providing immediate and specific feedback for all validation failures. A generic `toast.error` can still be used for overall form submission failure.

## 💡 نمونه‌های قبل/بعد
**Conceptual change for error handling in FacilityFormModal**
...
if (Object.keys(errors).length > 0) {
  setErrors(errors); // Assuming a new state variable `errors`
  toast.error('Please correct the errors in the form.');
  return;
}
// ... then in JSX, display errors like {errors.fieldName} next to inputs
```
...
## 🧪 دستورات اعتبارسنجی
- `Manually test the 'Add Facility' and 'Edit Facility' forms: Submit with multiple invalid fields (e.g., empty required, invalid amount, invalid date) and verify all errors are shown.`
- `npm run type-check`

## ⚠️ ریسک‌ها و موارد احتیاط
Moderate, requires changes to validation logic and form rendering.
...
1. `validateFacilityForm` should return an object mapping field names to error messages. _(verify: static)_
2. `FacilityFormModal` should store and display multiple validation errors simultaneously. _(verify: ui_interaction)_
3. Error messages should appear next to their corresponding input fields. _(verify: ui_interaction)_
4. A generic toast message should indicate form errors, with details shown on fields. _(verify: ui_interaction)_

## Task Steps

### Step 1: Refactor `validateFacilityForm` to return an object of errors
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تغییر امضای تابع `validateFacilityForm` و پیاده‌سازی داخلی آن است تا به جای یک رشته خطای واحد، یک شیء (object) از خطاها را برگرداند. این شیء باید فیلدهای فرم را به پیام‌های خطای مربوطه نگاشت کند، مشابه رویکرد `validateCustomerForm`. این تغییر فقط به منطق اعتبارسنجی مربوط می‌شود و هیچ تغییری در UI یا نحوه نمایش خطاها ایجاد نمی‌کند.
**Excerpt:**
```
The `FacilityFormModal` component in `frontend/src/app/facilities/page.tsx` uses `validateFacilityForm` which appears to return a single error string, leading to only one validation message being displayed at a time via `toast.error()`. In contrast, the `CustomerForm` (partially shown in `frontend/src/app/facilities/page.tsx`) uses `validateCustomerForm` which returns an object of errors, allowing for more granular and user-friendly feedback by displaying errors next to their respective input fields.
```

### Step 2: Update `FacilityFormModal` to consume the new error object
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` است تا تابع `validateFacilityForm` به‌روز شده را فراخوانی کند و شیء خطای برگشتی را به درستی مدیریت و در وضعیت (state) داخلی خود ذخیره کند. این شامل به‌روزرسانی منطق فراخوانی و مدیریت وضعیت خطا در داخل کامپوننت است، اما هنوز شامل رندر کردن بصری خطاها نیست.
**Excerpt:**
```
The `FacilityFormModal` component in `frontend/src/app/facilities/page.tsx` uses `validateFacilityForm` which appears to return a single error string, leading to only one validation message being displayed at a time via `toast.error()`. In contrast, the `CustomerForm` (partially shown in `frontend/src/app/facilities/page.tsx`) uses `validateCustomerForm` which returns an object of errors, allowing for more granular and user-friendly feedback by displaying errors next to their respective input fields.
```

### Step 3: Implement granular error display in `FacilityFormModal` UI
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح منطق رندرینگ (JSX) در کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` است. هدف این است که پیام‌های خطای فردی را که از شیء خطای ذخیره‌شده دریافت شده‌اند، مستقیماً در کنار فیلدهای ورودی مربوطه نمایش دهد. این پیاده‌سازی باید از رویکرد `CustomerForm` برای نمایش بازخورد دقیق‌تر و کاربرپسندتر الهام بگیرد.
**Excerpt:**
```
In contrast, the `CustomerForm` (partially shown in `frontend/src/app/facilities/page.tsx`) uses `validateCustomerForm` which returns an object of errors, allowing for more granular and user-friendly feedback by displaying errors next to their respective input fields. This inconsistency and limited feedback can frustrate users and make it harder to correct multiple validation issues, impacting data quality and user experience.
```

### Step 4: Remove `toast.error()` for validation errors in `FacilityFormModal`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و حذف فراخوانی‌های `toast.error()` است که قبلاً برای نمایش خطاهای اعتبارسنجی واحد در کامپوننت `FacilityFormModal` در `frontend/src/app/facilities/page.tsx` استفاده می‌شدند. این یک مرحله پاکسازی است که اطمینان حاصل می‌کند فقط مکانیسم جدید نمایش خطای دقیق‌تر استفاده می‌شود.
**Excerpt:**
```
The `FacilityFormModal` component in `frontend/src/app/facilities/page.tsx` uses `validateFacilityForm` which appears to return a single error string, leading to only one validation message being displayed at a time via `toast.error()`.
```

### Step 5: Add/Update unit tests for `validateFacilityForm`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد جدید یا به‌روزرسانی تست‌های موجود برای تابع `validateFacilityForm` است. هدف اطمینان از این است که تابع به‌روز شده به درستی یک شیء خطا را برای ورودی‌های معتبر و نامعتبر مختلف برمی‌گرداند. این شامل نوشتن موارد آزمایشی برای سناریوهای مختلف اعتبارسنجی (مانند فیلدهای الزامی از دست رفته، فرمت‌های نامعتبر) و تأیید ساختار و محتوای شیء خطای برگشتی است.
**Excerpt:**
```
The `FacilityFormModal` component in `frontend/src/app/facilities/page.tsx` uses `validateFacilityForm` which appears to return a single error string, leading to only one validation message being displayed at a time via `toast.error()`.
```

### Step 6: Add/Update integration/E2E tests for `FacilityFormModal`'s validation UI
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های یکپارچه‌سازی یا سرتاسری جدید یا به‌روزرسانی تست‌های موجود است تا تأیید شود که `FacilityFormModal` خطاهای اعتبارسنجی دقیق را به درستی در کنار فیلدهای ورودی نمایش می‌دهد و `toast.error()` دیگر برای اعتبارسنجی استفاده نمی‌شود. این شامل شبیه‌سازی ورودی کاربر، ارسال فرم و تأیید قابلیت مشاهده و محتوای پیام‌های خطا روی صفحه است.
**Excerpt:**
```
This inconsistency and limited feedback can frustrate users and make it harder to correct multiple validation issues, impacting data quality and user experience.
```
