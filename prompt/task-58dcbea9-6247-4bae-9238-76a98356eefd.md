...
updated_at: '2026-06-07T16:52:01.309679+00:00'
target_files:
...
1. متغیر `AUTH_DISABLED` از `backend/app/config.py` حذف شود.
2. تمام مسیرهای کدی که `AUTH_DISABLED` را بررسی می‌کنند، حذف یا بازسازی شوند تا همیشه فرض شود احراز هویت فعال است.
3. تابع `enforce_security_on_startup()` دیگر شامل منطق مربوط به `AUTH_DISABLED` نباشد.
4. تمام تست‌ها در `backend/tests/test_auth_disabled.py` حذف یا برای تست احراز هویت همیشه فعال تطبیق داده شوند.
5. فایل‌های `docs/SECURITY.md` و `docs/TASK_STATUS.md` برای بازتاب حذف به‌روزرسانی شوند.
