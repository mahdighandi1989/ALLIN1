---
task_id: 58dcbea9-6247-4bae-9238-76a98356eefd
title: پرچم `AUTH_DISABLED` یک خطر امنیتی در تولید است و باید حذف یا در تولید غیرفعال شود
type: security
priority: critical
...
created_at: '2026-06-07T16:52:01.309675+00:00'
updated_at: '2026-06-07T22:03:40.575009+00:00'
target_files:
...
# پرچم `AUTH_DISABLED` یک خطر امنیتی در تولید است و باید حذف یا در تولید غیرفعال شود

## Raw Idea

پرچم `AUTH_DISABLED` صراحتاً در `docs/SECURITY.md` (ADR-003) به عنوان یک «سوئیچ موقت و با هشدار بلند» ذکر شده است که می‌تواند احراز هویت را غیرفعال کند. اگرچه `backend/app/config.py` شامل منطقی برای ثبت خطا در صورت فعال بودن آن در تولید است، اما این مانع از دور زدن نمی‌شود. باقی گذاشتن چنین پرچمی در کدبیس، حتی با هشدارها، یک آسیب‌پذیری امنیتی حیاتی ایجاد می‌کند اگر به طور تصادفی یا مخرب در یک محیط تولید به `True` تنظیم شود، که امکان دسترسی بدون احراز هویت به نقاط پایانی داده‌های حساس را فراهم می‌کند. برای یک «سیستم عملیات بانکی»، این یک خطر غیرقابل قبول برای استقرار در تولید است.
...
- `backend/app/config.py:1-200` — جایی که `AUTH_DISABLED` تعریف و بررسی می‌شود.
- `backend/app/main.py:35-45` — جایی که `enforce_security_on_startup()` فراخوانی می‌شود و ممکن است منطق `AUTH_DISABLED` را شامل شود.
  ```python
  enforce_security_on_startup()
  ```
- `docs/SECURITY.md:79-84` — مستندات مربوط به ADR-003.
  ```markdown
  ...
  **Decision.** Keep the explicitly-requested "login off for now" toggle but
  ...
  ```
- `docs/TASK_STATUS.md:14-15` — جایی که حذف آن به تعویق افتاده است.
  ```markdown
...
FastAPI backend, Pydantic settings.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/utils/security.py` — جایی که احراز هویت اعمال می‌شود.
- `backend/tests/test_auth_disabled.py` — تست‌های مربوط به این پرچم.
- `backend/tests/conftest.py` — پرچم `AUTH_DISABLED=False` را برای تست‌ها تنظیم می‌کند.
- `backend/app/main.py` — این فایل `config.py` را import می‌کند (caller)
- `backend/app/services/google_oauth.py` — این فایل `config.py` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
حذف این پرچم احراز هویت را در تمام نقاط پایانی محافظت شده اعمال خواهد کرد. تست‌هایی که به `AUTH_DISABLED=True` متکی هستند، نیاز به به‌روزرسانی یا حذف خواهند داشت.
...
- [ ] متغیر `AUTH_DISABLED` از `backend/app/config.py` حذف شود.
- [ ] تمام مسیرهای کدی که `AUTH_DISABLED` را بررسی می‌کنند، حذف یا بازسازی شوند تا همیشه فرض شود احراز هویت فعال است.
- [ ] تابع `enforce_security_on_startup()` دیگر شامل منطق مربوط به `AUTH_DISABLED` نباشد.
- [ ] تمام تست‌ها در `backend/tests/test_auth_disabled.py` حذف یا برای تست احراز هویت همیشه فعال تطبیق داده شوند.
- [ ] فایل‌های `docs/SECURITY.md` و `docs/TASK_STATUS.md` برای بازتاب حذف به‌روزرسانی شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. متغیر پیکربندی `AUTH_DISABLED` و تمام منطق مرتبط با آن را از کدبیس حذف کنید. اگر یک قابلیت «غیرفعال کردن موقت احراز هویت» برای توسعه کاملاً ضروری است، باید از طریق یک مکانیزم امن‌تر (به عنوان مثال، یک متغیر محیطی خاص فقط برای توسعه که صراحتاً در بیلدها/استقرارهای تولید مسدود شده است، یا یک توکن موقت برای تست محلی) پیاده‌سازی شود. برای تولید، احراز هویت همیشه باید اجباری باشد.

## 💡 نمونه‌های قبل/بعد
**قبل (backend/app/config.py)**

_قبل:_
```
AUTH_DISABLED: bool = Field(False, description="Temporarily disable auth for dev")
```

**بعد (backend/app/config.py)**

_بعد:_
```
(خط حذف شده، فیلد `AUTH_DISABLED` وجود ندارد)
```
...
- `pytest backend/tests/test_auth_enforcement.py`
- `تلاش برای دسترسی به یک نقطهٔ پایانی محافظت شده بدون توکن؛ باید کد 401 را برگرداند.`

## ⚠️ ریسک‌ها و موارد احتیاط
ممکن است گردش کار توسعهٔ محلی را که به `AUTH_DISABLED=True` متکی است، مختل کند.
...
1. متغیر `AUTH_DISABLED` از `backend/app/config.py` حذف شود. _(verify: static)_
2. تمام مسیرهای کدی که `AUTH_DISABLED` را بررسی می‌کنند، حذف یا بازسازی شوند تا همیشه فرض شود احراز هویت فعال است. _(verify: static)_
3. تابع `enforce_security_on_startup()` دیگر شامل منطق مربوط به `AUTH_DISABLED` نباشد. _(verify: static)_
4. تمام تست‌ها در `backend/tests/test_auth_disabled.py` حذف یا برای تست احراز هویت همیشه فعال تطبیق داده شوند. _(verify: static)_
5. فایل‌های `docs/SECURITY.md` و `docs/TASK_STATUS.md` برای بازتاب حذف به‌روزرسانی شوند. _(verify: static)_

## Task Steps

### Step 1: حذف پرچم امنیتی `AUTH_DISABLED` و منطق مرتبط
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و حذف کامل پرچم `AUTH_DISABLED` از کدبیس، به‌ویژه از فایل `backend/app/config.py`، و همچنین حذف هرگونه منطق شرطی یا ثبت خطای مرتبط با فعال بودن این پرچم در محیط تولید است. هدف اصلی این است که امکان دور زدن احراز هویت از طریق این پرچم به طور کامل از بین برود. همچنین، مستندات مربوطه در `docs/SECURITY.md` باید به‌روزرسانی شود تا منعکس‌کننده حذف این پرچم و ADR-003 مرتبط با آن باشد. این مرحله بر روی از بین بردن یک آسیب‌پذیری امنیتی حیاتی تمرکز دارد و شامل پیاده‌سازی مکانیزم جایگزین برای دور زدن احراز هویت نیست.
**Excerpt:**
```
پرچم `AUTH_DISABLED` صراحتاً در `docs/SECURITY.md` (ADR-003) به عنوان یک «سوئیچ موقت و با هشدار بلند» ذکر شده است که می‌تواند احراز هویت را غیرفعال کند. اگرچه `backend/app/config.py` شامل منطقی برای ثبت خطا در صورت فعال بودن آن در تولید است، اما این مانع از دور زدن نمی‌شود. باقی گذاشتن چنین پرچمی در کدبیس، حتی با هشدارها، یک آسیب‌پذیری امنیتی حیاتی ایجاد می‌کند اگر به طور تصادفی یا مخرب در یک محیط تولید به `True` تنظیم شود، که امکان دسترسی بدون احراز هویت به نقاط پایانی داده‌های حساس را فراهم می‌کند. برای یک «سیستم عملیات بانکی»، این یک خطر غیرقابل قبول برای استقرار در تولید است.
```
