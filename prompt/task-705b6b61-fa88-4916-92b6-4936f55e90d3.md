---
task_id: 705b6b61-fa88-4916-92b6-4936f55e90d3
title: '[منطق] Stats Endpoint May Include Soft-Deleted Records'
type: logic_audit
priority: high
...
created_at: '2026-06-07T16:55:30.267905+00:00'
updated_at: '2026-06-07T16:55:30.267909+00:00'
---

# [منطق] Stats Endpoint May Include Soft-Deleted Records
...
The `stats.py` component aggregates data from models like `customer` and `facility`. The `trash.py` component indicates that these models support soft-deletion. The description for `stats.py` does not specify whether the aggregated statistics should include or exclude soft-deleted entities. If the dashboard is intended to show current, active operational statistics, including soft-deleted records would lead to inaccurate data.

## 💥 پیامد (impact)
Inaccurate dashboard statistics, leading to incorrect business decisions based on inflated or misrepresented data (e.g., total active customers count includes deleted customers). This creates a logical inconsistency between the 'active' state implied by a dashboard and the data being aggregated.

## 🛠 پیشنهاد رفع اولیه
Ensure all data fetching queries within `stats.py` explicitly filter out soft-deleted records (e.g., `WHERE is_deleted = FALSE` or `WHERE deleted_at IS NULL`). This filtering logic should ideally be encapsulated in a reusable service layer or ORM query helper.
...
3. integration test برای pipeline `data` بدون شکست عبور می‌کند
4. PR description توضیح می‌دهد چرا این تصمیم گرفته شد

## Task Steps

### Step 1: شناسایی کوئری‌های واکشی داده در `stats.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق کامپوننت `stats.py` برای شناسایی تمام کوئری‌های پایگاه داده یا فراخوانی‌های ORM است که داده‌ها را از مدل‌های `customer` و `facility` بازیابی می‌کنند. هدف اصلی درک الگوهای فعلی بازیابی داده و مشخص کردن مکان‌های دقیقی است که فیلتر حذف نرم (soft-deletion) باید اعمال شود. این مرحله هیچ تغییر کدی را شامل نمی‌شود، بلکه فقط تحلیل و مستندسازی یافته‌ها را در بر می‌گیرد.
**Excerpt:**
```
The `stats.py` component aggregates data from models like `customer` and `facility`. The `trash.py` component indicates that these models support soft-deletion. The description for `stats.py` does not specify whether the aggregated statistics should include or exclude soft-deleted entities. If the dashboard is intended to show current, active operational statistics, including soft-deleted records would lead to inaccurate data.
```

### Step 2: اعمال فیلتر حذف نرم در کوئری‌های `stats.py`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح کوئری‌های واکشی داده شناسایی شده در `stats.py` تمرکز دارد تا به صراحت رکوردهای حذف نرم شده را حذف کند. این کار شامل افزودن شرط‌هایی مانند `WHERE is_deleted = FALSE` یا `WHERE deleted_at IS NULL` به طور مستقیم به کوئری‌های موجود یا فراخوانی‌های ORM است. هدف فوری، اصلاح عدم دقت داده در داشبورد است. این مرحله شامل ایجاد کامپوننت‌های قابل استفاده مجدد نیست، بلکه فقط اعمال مستقیم منطق فیلترینگ است.
**Excerpt:**
```
Ensure all data fetching queries within `stats.py` explicitly filter out soft-deleted records (e.g., `WHERE is_deleted = FALSE` or `WHERE deleted_at IS NULL`). This filtering logic should ideally be encapsulated in a reusable service layer or ORM query helper.
```

### Step 3: کپسوله‌سازی منطق فیلتر حذف نرم در یک کامپوننت قابل استفاده مجدد
**Status:** `pending` (0%)
**Scope:** این مرحله شامل استخراج منطق فیلتر حذف نرم که در مرحله ۲ پیاده‌سازی شد، به یک تابع لایه سرویس قابل استفاده مجدد یا یک ORM query helper است. هدف، متمرکز کردن این نیاز فیلترینگ مشترک است تا به راحتی در بخش‌های مختلف برنامه قابل اعمال باشد و قابلیت نگهداری و انسجام کد را بهبود بخشد. پس از کپسوله‌سازی، `stats.py` باید به‌روزرسانی شود تا از این کامپوننت قابل استفاده مجدد جدید استفاده کند.
**Excerpt:**
```
This filtering logic should ideally be encapsulated in a reusable service layer or ORM query helper.
```
