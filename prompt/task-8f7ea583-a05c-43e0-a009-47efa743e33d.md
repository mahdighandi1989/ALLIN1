---
task_id: 8f7ea583-a05c-43e0-a009-47efa743e33d
title: توکن JWT ادمین به‌صورت query parameter در URL برای اتصال Google Drive ارسال می‌شود و در لاگ‌ها/history مرورگر لو می‌رود
type: security
priority: high
...
created_at: '2026-06-14T16:38:17.987368+00:00'
updated_at: '2026-06-14T16:38:17.987371+00:00'
target_files:
- frontend/src/app/settings/page.tsx
---

# توکن JWT ادمین به‌صورت query parameter در URL برای اتصال Google Drive ارسال می‌شود و در لاگ‌ها/history مرورگر لو می‌رود

## Raw Idea

در frontend/src/app/settings/page.tsx تابع connectDrive توکن احراز هویت ادمین را از localStorage خوانده و آن را به‌صورت query string (`?token=<jwt>`) در URL یک full-page navigation به backend ارسال می‌کند. توکن‌های انتقال‌یافته در query string در access logهای سرور/پراکسی (Render/NGINX)، در Referer header، و در history مرورگر ذخیره می‌شوند — این یک نشت اعتبارنامه (credential leakage) شناخته‌شده است. چون این توکن ادمین است، افشای آن دسترسی کامل ادمین می‌دهد. هرچند درخواست OAuth top-level navigation است و header قابل ارسال نیست، اما الگوی فعلی توکن بلندمدت را در URL قرار می‌دهد.
...
- `frontend/src/app/settings/page.tsx:62-66` — `connectDrive` — ارسال JWT کامل ادمین در query string
  ```tsx
  const connectDrive = () => {
...
Next.js 14 App Router + FastAPI + Google OAuth (drive.file)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/routers/google_auth.py` — endpoint /api/auth/google/drive/connect که token را از query می‌خواند و validate می‌کند
- `backend/app/utils/security.py` — ساخت/تأیید توکن — برای پیاده‌سازی توکن کوتاه‌عمر drive-connect
- `backend/app/utils/token_blacklist.py` — برای باطل‌کردن توکن یک‌بارمصرف پس از استفاده

## 🌐 نقشهٔ وابستگی‌ها
این جریان فقط در صفحهٔ settings استفاده می‌شود اما به google_auth router و مکانیزم توکن وابسته است؛ تغییر نیازمند هماهنگی front و back است.
...
- [ ] JWT اصلی کاربر دیگر در هیچ query string ظاهر نمی‌شود
- [ ] توکن drive-connect حداکثر TTL کوتاه (مثلاً ≤120 ثانیه) دارد و پس از یک‌بار استفاده باطل می‌شود
- [ ] تست backend عدم پذیرش توکن drive-connect منقضی/استفاده‌شده را تأیید می‌کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. به‌جای ارسال JWT اصلی در query، یک توکن یک‌بارمصرف کوتاه‌عمر (short-lived, single-use nonce) از backend دریافت کن و آن را در URL بگذار؛ backend آن را validate و فوراً باطل کند. یا اتصال را از طریق POST + redirect سروری انجام بده. حداقل، توکن استفاده‌شده در این مسیر باید TTL بسیار کوتاه و scope محدود (drive-connect only) داشته باشد.

## 💡 نمونه‌های قبل/بعد
**استفاده از توکن کوتاه‌عمر به‌جای JWT اصلی**
...
_بعد:_
```
const { connect_token } = await crmApi.mintDriveConnectToken() // POST با Authorization header
window.location.href = `${base}/api/auth/google/drive/connect?ct=${encodeURIComponent(connect_token)}`
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_drive_sync.py`
- `grep -rn 'token=' frontend/src/app/settings/page.tsx`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به افزودن endpoint جدید mint-token و هماهنگی با جریان OAuth موجود؛ احتمال شکست موقت اتصال Drive حین مهاجرت
...
1. JWT اصلی کاربر دیگر در هیچ query string ظاهر نمی‌شود
2. توکن drive-connect حداکثر TTL کوتاه (مثلاً ≤120 ثانیه) دارد و پس از یک‌بار استفاده باطل می‌شود
3. تست backend عدم پذیرش توکن drive-connect منقضی/استفاده‌شده را تأیید می‌کند
