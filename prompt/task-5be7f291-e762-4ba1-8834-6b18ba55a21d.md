---
task_id: 5be7f291-e762-4ba1-8834-6b18ba55a21d
title: 'Contract mismatch: استفادهٔ گستردهٔ any برای پاسخ customersApi.detail در customer-detail و credit-file به‌جای Type قراردادی'
type: refactor
priority: high
...
created_at: '2026-06-14T16:40:40.450313+00:00'
updated_at: '2026-06-14T16:40:40.450315+00:00'
target_files:
- frontend/src/app/customer-detail/page.tsx
- frontend/src/app/credit-file-corporate/page.tsx
---

# Contract mismatch: استفادهٔ گستردهٔ any برای پاسخ customersApi.detail در customer-detail و credit-file به‌جای Type قراردادی

## Raw Idea

صفحهٔ `customer-detail/page.tsx` پاسخ `customersApi.detail(id)` را به‌صورت `useState<any>(null)` نگه می‌دارد و سپس با destructuring بزرگ (`const { customer, facilities = [], offer_letters = [], guarantors = [], ... } = data`) ده‌ها فیلد را از آن استخراج می‌کند بدون هیچ Type قراردادی. همچنین `credit-file-corporate/page.tsx` خط ۱۶۹ `const d: any = await customersApi.detail(a)` می‌کند. این یعنی هیچ بررسی compile-time بین shape واقعی backend (روتر crm/customers) و مصرف frontend وجود ندارد؛ اگر backend نام فیلدی را تغییر دهد (مثل `fixed_deposits` vs `fixedDeposits`) هیچ خطای type رخ نمی‌دهد و فقط در runtime به‌صورت undefined ظاهر می‌شود. این دقیقاً همان نوع contract mismatch است که فاز K باید بگیرد.
...
- `frontend/src/app/customer-detail/page.tsx:35-86` — `CustomerDetailInner / data destructuring` — destructuring بزرگ بدون Type؛ هیچ تطابق compile-time با backend ندارد
  ```tsx
  ...
  ```
- `frontend/src/app/credit-file-corporate/page.tsx:169-171` — `loadAccount`
  ```tsx
  const d
  ```
...
1. یک interface `CustomerDetailResponse` در `frontend/src/types` تعریف کن که دقیقاً shape بازگشتی endpoint `GET /api/customers/{id}` (یا crm detail) را پوشش دهد — شامل customer, facilities, guarantors, securities, tasks, attachments, journal, notes, profile, checklist, summary, properties, fixed_deposits, partners, facility_checklists, relationships. سپس `useState<CustomerDetailResponse | null>` و `customersApi.detail` را با این Type تایپ کن و any را حذف کن.
