---
task_id: 260d0760-cb46-4d13-9338-dea1007699e3
...
created_at: '2026-06-07T16:55:02.752651+00:00'
updated_at: '2026-06-07T22:06:30.608205+00:00'
target_files:
...
If the desired outcome is 'secure remote database connections by default', the current threshold for security is mismatched. It requires an explicit environment variable (`DB_SSL_VERIFY=true`) to *enable* full certificate verification, rather than having security enabled by default and requiring an explicit opt-out to *disable* it. This shifts the burden of ensuring security onto the deployment co

📁 file: backend/app/database.py (line 35)
...
- `backend/app/config.py` — `database.py` این فایل را import می‌کند
- `backend/app/routers/settings.py` — `database.py` این فایل را import می‌کند
- `backend/app/utils/security.py` — این فایل `database.py` را import می‌کند (caller)
- `backend/app/models/audit_log.py` — این فایل `database.py` را import می‌کند (caller)
- `backend/app/models/customer.py` — این فایل `database.py` را import می‌کند (caller)
- `backend/app/models/exchange_rate.py` — این فایل `database.py` را import می‌کند (caller)
