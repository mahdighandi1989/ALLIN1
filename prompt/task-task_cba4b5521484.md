...
title: افزودن ستون amount و به‌روزرسانی داشبورد
type: other
priority: critical
...
updated_at: '2026-06-05T18:46:35.447138+00:00'
tags:
- consolidated
- post_verify_merge
---
...
📋 acceptance_criteria کامل:
  ...
  - مقادیر amount در داشبورد به درستی نمایش داده می‌شوند [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/dashboard"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "screenshot", "label": "dashboard_loaded"}, {"action": "a]
...
عدم وجود ستون amount در جدول facilities باعث خطای 500 در endpoint آمار داشبورد می‌شود

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/migrations/versions/001_initial_create_tables.py:1-50` — `create_facilities_table` — بررسی کنید که ستون amount در این migration ایجاد شده است
  ```python
  def upgrade():
      op.create_table('facilities',
          ...
          ...
          # آیا ستون amount وجود دارد؟
      )
  ```
- `backend/migrations/versions/002_add_missing_columns.py:1-30` — `add_missing_columns` — بررسی کنید که ستون amount در این migration اضافه شده است
  ```python
  def upgrade():
      # آیا ستون amount در این migration اضافه شده است؟
      pass
  ```
...
- `docs/DATABASE_SCHEMA.md` (سطر 15) — مستندات دیتابیس که به وجود ستون amount اشاره دارد
- `backend/app/models/facility.py` (سطر 1) — مدل facilities که باید ستون amount را داشته باشد
- `backend/app/routers/stats.py` (سطر 1) — endpoint آمار داشبورد که از ستون amount استفاده می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این مشکل بر روی endpoint آمار داشبورد و نمایش صحیح اطلاعات مالی تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
بر اساس مستندات DATABASE_SCHEMA.md، ستون amount در جدول facilities برای محاسبه جمع کل در داشبورد ضروری است. عدم وجود این ستون در دیتابیس باعث خطای 500 در endpoint آمار داشبورد می‌شود. این مشکل در migration‌های اولیه (001_initial_create_tables.py و 002_add_missing_columns.py) باید بررسی شود که آیا ستون amount به درستی اضافه شده است یا خیر.
...
1. بررسی migration‌های موجود برای اطمینان از وجود ستون amount در جدول facilities. اگر ستون وجود ندارد، یک migration جدید اضافه کنید که ستون amount را با نوع NUMERIC(15,2) و NOT NULL به جدول facilities اضافه کند.

## 💡 نمونه‌های قبل/بعد
**اضافه کردن ستون amount در migration**

_قبل:_
```
# migration موجود بدون ستون amount
op.create_table('facilities',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
)
...
_بعد:_
```
op.add_column('facilities', sa.Column('amount', sa.Numeric(15, 2), nullable=False, server_default='0'))
```
...
نیاز به اجرای migration روی دیتابیس موجود؛ ممکن است نیاز به مقدار پیش‌فرض برای رکوردهای موجود باشد
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 3
  id: 8241ac2e-ce55-41a0-b2c8-562a2b46f250
  عنوان اصلی: یکپارچه‌سازی default فیلد 'finally'
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - `finally` در همه‌جا از یک منبع default می‌گیرد [verify_method=static] [verify_plan={"grep_patterns": ["finally\\s*=\\s*Field\\(", "finally\\s*:\\s*.*\\s*=\\s*.*"], "files_hint": ["**/*.py"]}]
  - تست fixture رفتار پیش‌فرض را تأیید می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/unit/test_finally_field.py::test_finally_default_behavior", "timeout_seconds": 30}]
...
## 🎯 هدف (خلاصه ساختاریافته)
تضاد default برای فیلد 'finally'
...
## 🔍 جزئیات
- علت: field finally has different defaults: ['"invalid_format",', 'time.perf_counter() - start', 'override_get_db']
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: همه جاهایی که `finally` default می‌گیرد لیست کن.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 3
  id: c47124e0-5848-4ca3-85b6-03106a894117
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["String\\(36\\)", "sqlalchemy.dialects.postgresql.UUID", "UUID\\(", "truncated UUID", "collision", "stale assumption", "future migration"], "files_hint": ["backend/app/models/user.p]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/backend/app/models/test_user_uuid.py::test_uuid_collision_resistance", "timeout_seconds": 60}]
...
- `backend/tests/frontend/test_facilities_reason_validation.py` — این فایل `user.py` را import می‌کند (caller)

## 🔍 Context و وضعیت فعلی
The `id` column uses an 8-character truncated UUID, justified by the current 'small scale' of operator accounts. This is a stale assumption; if the user base grows, collisions will become more frequent, leading to `IntegrityError`s on insert, even though the database rejects them. The `String(8)` column definition also hardcodes this limitation, making future migration to a full UUID (e.g., 32 cha

📁 file: backend/app/models/user.py (line 20)
...
- در commit message: `merged-from: e155869b-cb38-44ec-a932-5ee8164d5907, 8241ac2e-ce55-41a0-b2c8-562a2b46f250, c47124e0-5848-4ca3-85b6-03106a894117`
...
1. ستون amount با نوع NUMERIC(15,2) در جدول facilities وجود دارد _(verify: static)_
2. endpoint /api/stats/dashboard خطای 500 برنمی‌گرداند _(verify: api_response)_
3. مقادیر amount در داشبورد به درستی نمایش داده می‌شوند _(verify: ui_interaction)_
4. `finally` در همه‌جا از یک منبع default می‌گیرد _(verify: static)_
5. تست fixture رفتار پیش‌فرض را تأیید می‌کند _(verify: backend_test)_
6. اگر default value تغییر کرد، migration یا backward-compat layer اضافه شد _(verify: manual_only)_
...
### Step 2: بررسی وجود ستون amount در migration 002_add_missing_columns.py
**Status:** `done` (100%)
**Scope:** بررسی فایل backend/migrations/versions/002_add_missing_columns.py برای اطمینان از وجود ستون amount با نوع NUMERIC(15,2) در جدول facilities. این مرحله فقط شامل بررسی کد migration است و شامل تغییر یا اضافه کردن کد نمی‌شود. اگر ستون وجود نداشته باشد، مرحله بعدی آن را اضافه خواهد کرد.
**Excerpt:**
```
- `backend/migrations/versions/002_add_missing_columns.py:1-30` — `add_missing_columns` — بررسی کنید که ستون amount در این migration اضافه شده است
  ```python
...
### Step 3: اضافه کردن ستون amount به جدول facilities از طریق migration جدید
**Status:** `done` (100%)
**Scope:** اگر ستون amount در migration‌های موجود وجود نداشته باشد، یک فایل migration جدید (003_add_amount_column.py) ایجاد کنید که ستون amount را با نوع NUMERIC(15,2) و nullable=True به جدول facilities اضافه کند. این مرحله شامل نوشتن کد migration و اطمینان از rollback امن است.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
- [ ] ستون amount با نوع NUMERIC(15,2) در جدول facilities وجود دارد
...
### Step 12: تشخیص ریشه anti-pattern 'Stale assumption' در مدل User
**Status:** `done` (100%)
**Scope:** بررسی کامل فایل backend/app/models/user.py برای تشخیص ریشه anti-pattern 'Stale assumption'. این شامل بررسی استفاده از String(8) برای UUID truncated و تحلیل خطر collision در آینده است. نتایج باید مستند شوند.
**Excerpt:**
```
## 🎯 هدف (خلاصه ساختاریافته)
Anti-pattern: Stale assumption
...
## 🔍 Context و وضعیت فعلی
The `id` column uses an 8-character truncated UUID, justified by the current 'small scale' of operator accounts. This is a stale assumption; if the user base grows, collisions will become more frequent, leading to `IntegrityError`s on insert, even though the database rejects them.
```

### Step 13: اصلاح کد یا اضافه کردن کامنت توجیهی برای anti-pattern در مدل User
**Status:** `done` (100%)
**Scope:** بسته به تصمیم، یا کد را اصلاح کنید (مثلاً تغییر String(8) به UUID یا String(36))، یا یک کامنت توجیهی در فایل backend/app/models/user.py اضافه کنید که دلیل استفاده از String(8) و خطرات آن را توضیح می‌دهد. اگر کد اصلاح می‌شود، migration مربوطه نیز باید ایجاد شود.
**Excerpt:**
```
- [ ] یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["String\\(36\\)", "sqlalchemy.dialects.postgresql.UUID", "UUID\\(", "truncated UUID", "collision", "stale assumption", "future migration"], "files_hint": ["backend/app/models/user.p"]}]
```

### Step 14: نوشتن تست edge case برای مقاومت در برابر collision UUID
**Status:** `done` (100%)
**Scope:** نوشتن تست واحد در فایل tests/backend/app/models/test_user_uuid.py که مقاومت در برابر collision UUID را بررسی می‌کند. تست باید شبیه‌سازی کند که دو UUID truncated با هم collision دارند و بررسی کند که سیستم به درستی از insert دوم جلوگیری می‌کند.
**Excerpt:**
```
- [ ] تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/backend/app/models/test_user_uuid.py::test_uuid_collision_resistance", "timeout_seconds": 60}]
```
...
⚠️ **هشدار بحرانی — قاعدهٔ ضد-فرار:** TO-DO فقط برای کارهایی است که
**واقعاً غیرممکن** برای agent است (نیاز به انسان مطلق)...
```
