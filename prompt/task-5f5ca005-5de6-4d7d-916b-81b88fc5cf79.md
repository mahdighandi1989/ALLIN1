---
task_id: 5f5ca005-5de6-4d7d-916b-81b88fc5cf79
title: credit-file-corporate از فیلد branch_code می‌خواند که طبق gap-analysis در مدل وجود ندارد و همیشه خالی می‌ماند
type: bug
priority: high
...
created_at: '2026-06-14T16:36:54.629010+00:00'
updated_at: '2026-06-14T16:36:54.629013+00:00'
target_files:
- frontend/src/app/credit-file-corporate/page.tsx
---

# credit-file-corporate از فیلد branch_code می‌خواند که طبق gap-analysis در مدل وجود ندارد و همیشه خالی می‌ماند

## Raw Idea

در loadAccount صفحهٔ credit-file-corporate، مقدار branchCode از customer?.branch_code خوانده می‌شود (خط 177). اما طبق docs/EXCEL_VS_PANEL_GAP_ANALYSIS.md ردیف ۱ صراحتاً می‌گوید: «credit-file-corporate reads branch_code that doesn't exist → always blank». یعنی این فیلد در مدلِ customers/customer_profiles موجود نیست و در نتیجه فیلد Branch Code همیشه خالی پر می‌شود. این یک باگ دادهٔ مشخص است که خروجیِ چاپیِ فرمِ بانکی را ناقص می‌کند.
...
- `frontend/src/app/credit-file-corporate/page.tsx:164-188` — `loadAccount` — branch_code روی customer وجود ندارد؛ همیشه خالی
  ```tsx
...
SQLAlchemy 2.0 + Alembic + FastAPI + Next.js form

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/customer.py` (سطر 1) — مدلی که باید ستون branch_code داشته باشد
- `backend/app/routers/customers.py` — endpoint detail که داده را برمی‌گرداند
- `backend/migrations/versions/002_add_missing_columns.py` — محل احتمالی افزودن مهاجرتِ ستون جدید
- `docs/EXCEL_VS_PANEL_GAP_ANALYSIS.md` — سند gap که این باگ را مستند کرده

## 🌐 نقشهٔ وابستگی‌ها
مدل customer توسط ۴۳ فایل import می‌شود؛ افزودن ستون nullable کم‌ریسک است اما نیاز به migration و به‌روزرسانی serializer دارد.
...
- [ ] وقتی branch_code در داده موجود است، فیلد Branch Code در فرم پر می‌شود
- [ ] ستون branch_code در مدل و migration اضافه شده (در صورت انتخاب راه‌حل backend)
- [ ] تستی که detail با branch_code را بازمی‌گرداند و frontend آن را نمایش می‌دهد اضافه شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ابتدا تصمیم بگیر منبعِ حقیقتِ branch_code کجاست. گزینهٔ بهتر: افزودن ستون branch_code به مدل customer (یا customer_profiles) و مهاجرت Alembic، سپس پر کردن آن از داده‌های merge. در همین حال در frontend از fallback به profile.data.branch_code یا customer.branch استفاده کن تا فیلد خالی نماند.

## 💡 نمونه‌های قبل/بعد
**fallback در frontend تا migration کامل شود**
...
_بعد:_
```
branchCode: customer?.branch_code || pdata.branch_code || '',
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_customers.py -k branch`
- `npm run`
...
1. وقتی branch_code در داده موجود است، فیلد Branch Code در فرم پر می‌شود
2. ستون branch_code در مدل و migration اضافه شده (در صورت انتخاب راه‌حل backend)
3. تستی که detail با branch_code را بازمی‌گرداند و frontend آن را نمایش می‌دهد اضافه شود
