---
task_id: 623ccf10-68d1-45d8-8a18-3a43957c9345
title: '`xlrd` dependency is missing from `pyproject.toml`'
type: bug
priority: high
...
created_at: '2026-06-07T16:54:00.151280+00:00'
updated_at: '2026-06-07T22:04:48.851448+00:00'
target_files:
- pyproject.toml
---

# `xlrd` dependency is missing from `pyproject.toml`

## Raw Idea

The `xlrd` package, which is essential for parsing legacy `.xls` Excel files within the data import pipeline (`backend/app/services/excel_import.py`), is correctly listed and pinned in `backend/requirements.txt`. However, it is entirely absent from the `dependencies` section of `pyproject.toml`. This inconsistency means that any environment relying solely on `pyproject.toml` for dependency resolution (e.g., modern Python project setups using Poetry or `pip install .`) will fail to install `xlrd`, leading to runtime `ModuleNotFoundError` when attempting to process `.xls` files. The `docs/TASK_STATUS.md` explicitly mentions `xlrd` as part of the Excel pipeline stability task, confirming its necessity.
...
- `pyproject.toml:27-46` — Add `"xlrd==2.0.1"` to this list of dependencies.
  ```
  dependencies = [
      "fastapi>=0.100.0",
      ...
      # Pinned to match backend/requirements.txt. passlib 1.7.4 is incompatible
      # with bcrypt >= 4.1 (it removed the ``__about__`` module and hard-errors on
      # >72-byte inputs), so the version is pinned exactly to stop CI/Docker from
      # installing a divergent, broken version.
      "bcrypt==4.0.1",
      "python-dotenv>=1.0.0",
...
Python dependency management (`pyproject.toml`, `requirements.txt`), Excel file parsing.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/requirements.txt` (سطر 21) — Source of truth for the `xlrd` version.
- `backend/app/services/excel_import.py` — `xlrd` is used here for parsing `.xls` files.
- `docs/TASK_STATUS.md` — Mentions `xlrd` as part of task 7 (Excel pipeline stability).

## 🌐 نقشهٔ وابستگی‌ها
`xlrd` is a core dependency for the Excel import functionality, specifically for handling older `.xls` file formats. Its absence in `pyproject.toml` will break this feature.
...
- [ ] `xlrd==2.0.1` is present in `pyproject.toml`'s `dependencies` section.
- [ ] The backend's test suite, including tests for `.xls` file imports, passes after the change.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Add `xlrd==2.0.1` to the `dependencies` list in `pyproject.toml` to ensure consistency with `requirements.txt` and guarantee that the Excel import functionality for `.xls` files works reliably across all installation methods.
...
## 🧪 دستورات اعتبارسنجی
- `pip install -e .  # from backend root`
...
1. `xlrd==2.0.1` is present in `pyproject.toml`'s `dependencies` section. _(verify: static)_
2. The backend's test suite, including tests for `.xls` file imports, passes after the change. _(verify: backend_test)_
