---
task_id: 836a7fc8-11a3-4971-9318-ff148caea609
title: سرویس customer_link.py (reconcile_orphan_collateral در startup) فاقد تست است
type: other
priority: high
...
created_at: '2026-06-14T16:39:40.582296+00:00'
updated_at: '2026-06-14T16:39:40.582299+00:00'
target_files:
- backend/app/services/customer_link.py
---

# سرویس customer_link.py (reconcile_orphan_collateral در startup) فاقد تست است

## Raw Idea

تابع reconcile_orphan_collateral در backend/app/services/customer_link.py مستقیماً در lifespan اپ (backend/app/main.py خط 50-54) فراخوانی می‌شود تا برای هر collateral بدون مشتری، یک profile stub بسازد. این یک critical path در bootstrap است: اگر این تابع باگ داشته باشد، یا رکوردها orphan می‌مانند یا خطای startup رخ می‌دهد (هرچند با try/except پوشش داده شده، اما خطا فقط log می‌شود و بی‌صدا داده ناقص می‌ماند). در لیست فایل‌های test هیچ test_customer_link.py دیده نمی‌شود. منطق reconcile (matching بر اساس account_no، ساخت stub، اجتناب از duplicate) نیاز به تست واحد دارد.
...
- `backend/app/services/customer_link.py:1-40` — `reconcile_orphan_collateral` — در lifespan فراخوانی می‌شود (main.py:52)؛ فاقد تست
  ```python
  async def reconcile_orphan_collateral():
      # ساخت stub customer profile برای collateral بدون مشتری
  ```
...
FastAPI lifespan + SQLAlchemy 2.0 async + pytest-asyncio

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 52) — reconcile_orphan_collateral را در lifespan call می‌کند
- `backend/app/models/customer.py` — مدل customer که stub در آن ساخته می‌شود
- `backend/tests/conftest.py` — fixture دیتابیس in-memory
- `backend/app/database.py` — `customer_link.py` این فایل را import می‌کند
- `backend/app/services/collateral.py` — `customer_link.py` این فایل را import می‌کند
- `backend/app/routers/crm.py` — این فایل `customer_link.py` را import می‌کند (caller)
- `backend/app/routers/properties.py` — این فایل `customer_link.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این تابع در startup اجرا می‌شود و روی یکپارچگی دادهٔ collateral اثر می‌گذارد؛ باگ آن منجر به دادهٔ یتیم یا خطای bootstrap بی‌صدا می‌شود.
...
- [ ] فایل backend/tests/test_customer_link.py ایجاد شود
- [ ] تست: برای collateral یتیم، یک customer/profile stub ساخته شود
- [ ] تست: اگر customer از قبل وجود دارد، duplicate ساخته نشود
- [ ] تست: اگر هیچ orphan نباشد، تابع بدون خطا no-op شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد backend/tests/test_customer_link.py با تست‌های: ساخت stub profile برای collateral یتیم، عدم ساخت duplicate وقتی customer از قبل موجود است، و رفتار وقتی هیچ orphan وجود ندارد (no-op).
...
_بعد:_
```
async def test_reconcile_creates_stub_for_orphan(db_session):
    # seed collateral با account_no بدون customer
    await reconcile_orphan_collateral()
    cust = await get_customer_by_account('999')
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_customer_link.py -v`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به seed دادهٔ collateral بدون مشتری در fixture
...
1. فایل backend/tests/test_customer_link.py ایجاد شود
2. تست: برای collateral یتیم، یک customer/profile stub ساخته شود
3. تست: اگر customer از قبل وجود دارد، duplicate ساخته نشود
4. تست: اگر هیچ orphan نباشد، تابع بدون خطا no-op شود

## Task Steps

### Step 1: نوشتن تست واحد برای reconcile_orphan_collateral در test_customer_link.py
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد فایل تست جدید برای تابع reconcile_orphan_collateral در backend/app/services/customer_link.py است که در حال حاضر هیچ test_customer_link.py برای آن وجود ندارد. باید منطق reconcile به‌طور کامل پوشش داده شود: matching بر اساس account_no، ساخت profile stub برای هر collateral بدون مشتری، و اجتناب از ساخت duplicate. خارج از scope این مرحله: تغییر منطق خود تابع reconcile_orphan_collateral یا refactor آن (مگر اینکه باگی کشف شود)، و تغییر نحوهٔ فراخوانی در lifespan اپ در backend/app/main.py. نکتهٔ حیاتی: این یک critical path در bootstrap است، پس تست باید سناریوهای orphan ماندن رکورد و خطای startup را پوشش دهد؛ همچنین چون خطا در lifespan با try/except فقط log می‌شود و بی‌صدا داده ناقص می‌ماند، تست باید مطمئن شود که reconcile به‌درستی و بدون خطا کار می‌کند.
**Excerpt:**
```
تابع reconcile_orphan_collateral در backend/app/services/customer_link.py مستقیماً در lifespan اپ (backend/app/main.py خط 50-54) فراخوانی می‌شود تا برای هر collateral بدون مشتری، یک profile stub بسازد. این یک critical path در bootstrap است: اگر این تابع باگ داشته باشد، یا رکوردها orphan می‌مانند یا خطای startup رخ می‌دهد (هرچند با try/except پوشش داده شده، اما خطا فقط log می‌شود و بی‌صدا داده ناقص می‌ماند). در لیست فایل‌های test هیچ test_customer_link.py دیده نمی‌شود. منطق reconcile (matching بر اساس account_no، ساخت stub، اجتناب از duplicate) نیاز به تست واحد دارد.
```
