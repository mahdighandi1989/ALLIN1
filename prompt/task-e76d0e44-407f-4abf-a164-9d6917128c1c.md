---
task_id: e76d0e44-407f-4abf-a164-9d6917128c1c
title: عدم وجود تایم‌اوت صریح برای فراخوانی‌های API خارجی/طولانی‌مدت
type: bug
priority: high
...
created_at: '2026-06-07T16:54:34.143151+00:00'
updated_at: '2026-06-07T22:05:39.004001+00:00'
target_files:
- backend/app/services/google_oauth.py
---

# عدم وجود تایم‌اوت صریح برای فراخوانی‌های API خارجی/طولانی‌مدت

## Raw Idea

پروژه از `httpx` برای درخواست‌های HTTP استفاده می‌کند و با سرویس‌های خارجی مانند Google Drive برای پشتیبان‌گیری یکپارچه شده است. با این حال، هیچ شواهد صریحی در کد یا مستندات ارائه شده وجود ندارد که نشان دهد این فراخوانی‌های API خارجی با تایم‌اوت‌های مشخصی پیکربندی شده‌اند. بدون تایم‌اوت‌های صریح، یک سرویس خارجی کند یا غیرپاسخگو می‌تواند باعث توقف نامحدود فرآیند بک‌اند، مصرف منابع و منجر به شکست‌های آبشاری در سراسر برنامه شود. این یک حالت شکست حیاتی برای سیستم‌هایی است که به وابستگی‌های خارجی متکی هستند.
...
- `backend/app/services/google_oauth.py:N/A` — `N/A` — فایل ضمنی که مسئول تعامل با Google Drive است و باید تایم‌اوت‌ها را پیکربندی کند.
  ```python
  N/A
  ```
...
FastAPI backend, `httpx` library.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `requirements.txt` — لیست `httpx` به عنوان یک وابستگی.
- `pyproject.toml` — لیست `httpx` به عنوان یک وابستگی.
- `README.md` — اشاره به 'Google Drive Backup'.
- `backend/app/main.py` — نقطه ورودی اصلی، جایی که سرویس‌ها ممکن است مقداردهی اولیه شوند.
- `backend/app/config.py` — `google_oauth.py` این فایل را import می‌کند
- `backend/app/routers/settings.py` — `google_oauth.py` این فایل را import می‌کند
- `backend/app/routers/google_auth.py` — این فایل `google_oauth.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
`httpx` یک کتابخانه اصلی کلاینت HTTP است. سرویس‌هایی که از آن برای فراخوانی‌های خارجی استفاده می‌کنند، تحت تأثیر قرار خواهند گرفت.
...
- [ ] تمام درخواست‌های HTTP خارجی در بک‌اند دارای تایم‌اوت صریح پیکربندی شده باشند.
- [ ] یک تست (واحد یا یکپارچه‌سازی) تأیید کند که فراخوانی‌های خارجی به تایم‌اوت پیکربندی شده احترام می‌گذارند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تمام کامپوننت‌های بک‌اند که درخواست‌های HTTP خارجی را انجام می‌دهند (مانند `backend/app/services/google_oauth.py` برای Google Drive یا هر سرویس دیگری که از `httpx` استفاده می‌کند) را شناسایی کنید. نمونه‌های `httpx.Client` یا درخواست‌های فردی را با مقادیر تایم‌اوت صریح و معقول (به عنوان مثال، `timeout=10.0` ثانیه) پیکربندی کنید. این کار تضمین می‌کند که برنامه حتی در صورت بروز مشکل در سرویس‌های خارجی، پاسخگو باقی می‌ماند.
...
import httpx
# ...
async with httpx.AsyncClient(timeout=10.0) as client: # Or per-request timeout
    response = await client.get("https://external.api/data")
```
...
## 🧪 دستورات اعتبارسنجی
- `Simulate a slow Google Drive API response in a test environment and verify that the backup operation times out gracefully.`
- `pytest backend/tests/`

## ⚠️ ریسک‌ها و موارد احتیاط
تنظیم تایم‌اوت‌ها به صورت تهاجمی ممکن است باعث شکست عملیات‌های کند مشروع شود؛ نیاز به تنظیم دقیق دارد.
...
1. تمام درخواست‌های HTTP خارجی در بک‌اند دارای تایم‌اوت صریح پیکربندی شده باشند. _(verify: static)_
2. یک تست (واحد یا یکپارچه‌سازی) تأیید کند که فراخوانی‌های خارجی به تایم‌اوت پیکربندی شده احترام می‌گذارند. _(verify: backend_test)_
