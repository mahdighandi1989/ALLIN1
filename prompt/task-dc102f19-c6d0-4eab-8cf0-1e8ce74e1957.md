---
task_id: dc102f19-c6d0-4eab-8cf0-1e8ce74e1957
title: Incomplete `useEffect` dependencies in `FacilitiesPage` lead to stale filter application.
type: bug
priority: high
...
created_at: '2026-06-07T16:52:41.708627+00:00'
updated_at: '2026-06-07T22:03:59.113815+00:00'
target_files:
- frontend/src/app/facilities/page.tsx
---

# Incomplete `useEffect` dependencies in `FacilitiesPage` lead to stale filter application.

## Raw Idea

The `useEffect` hook responsible for loading facilities (`loadFacilities`) in `frontend/src/app/facilities/page.tsx` does not include all its dependencies in its dependency array. Specifically, the `loadFacilities` function directly uses `search`, `fType`, `fStatus`, `amountMin`, and `amountMax` from component state. However, these state variables are not listed in the `useEffect`'s dependency array (`[page, sortBy, sortOrder]`). This means that when a user types in the search box or changes other filter dropdowns/inputs, the displayed data will not automatically update until pagination or sorting changes, or the 'Filter' button is explicitly clicked. This leads to a confusing and broken user experience where filters appear to be set but are not applied, potentially causing users to act on outdated information.
...
- `frontend/src/app/facilities/page.tsx:40-76` — `FacilitiesPage` — The useEffect hook and loadFacilities function need to be updated.
  ```tsx
  40:   useEffect(() => {
  41:     loadFacilities()
  42:     // eslint-disable-next-line react-hooks/exhaustive-deps
  43:   }, [page, sortBy, sortOrder])
  44: 
  45:   const loadFacilities = async () => {
  46:     setLoading(true)
  47:     // Use Promise.allSettled so a single rejected request surfaces a toast
  48:     // instead of throwing and leaving the table stuck on the spinner.
  49:     const [facilitiesResult] = await Promise.allSettled([
  50:       facilitiesApi.list({
  51:         page,
  52:         page_size: 20,
  53:         search: search || undefined,
  54:         facility_type: fType || undefined,
  55:         status: fStatus || undefined,
  56:         amount_min: amountMin ? parseFloat(amountMin) : undefined,
  57:         amount_max: amountMax ? parseFloat(amountMax) : undefined,
  58:         sort_by: sortBy,
  59:         sort_order: sortOrder,
  60:       }),
  61:     ])
  62:     if (facilitiesResult.status === 'fulfilled') {
  63:       setData(facilitiesResult.value)
  64:     } else {
  65:       console.error('Failed to load facilities:', facilitiesResult.reason)
  66:       toast.error(parseApiError(facilitiesResult.reason))
  67:     }
  68:     setLoading(false)
  69:   }
  70: 
  71:   const handleSearch = (e: React.FormEvent) => {
  72:     e.preventDefault()
  73:     setPage(1)
  74:     loadFacilities()
  75:   }
...
Next.js 14 (React 18) functional component with hooks.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/customers/page.tsx` (سطر 25) — Provides a correct example of using useRef for stable filter dependencies (lines 25-32).
- `frontend/src/lib/api.ts` — Contains the facilitiesApi.list function called by loadFacilities.
- `frontend/src/app/facilities/page.test.tsx` — این فایل `page.tsx` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This is a frontend UI logic bug. Changes will affect the `FacilitiesPage` rendering and data fetching.
...
- [ ] Changing the search input or any filter dropdown/input on the Facilities page should trigger a data reload.
- [ ] The `eslint-disable-next-line react-hooks/exhaustive-deps` comment should be removed.
- [ ] The `loadFacilities` function should be wrapped in `useCallback` and its dependencies correctly managed.
- [ ] A `useRef` should be used to hold the latest filter values to maintain `loadFacilities` referential stability.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Refactor the `loadFacilities` function to use a `useRef` pattern for filter values, similar to how `CustomersPage` handles its search and account type filters. This will ensure `loadFacilities` maintains referential stability while still accessing the current filter values, allowing the `useEffect` to correctly track its dependencies. Remove the `eslint-disable-next-line react-hooks/exhaustive-deps` comment.

## 💡 نمونه‌های قبل/بعد
**Conceptual change for useEffect and loadFacilities**
...
_بعد:_
```
const filtersRef = useRef({ search, fType, fStatus, amountMin, amountMax });
filtersRef.current = { search, fType, fStatus, amountMin, amountMax };

const loadFacilities = useCallback(async () => {
  // ... use filtersRef.current.search, etc. ...
}, [page, sortBy, sortOrder]);
...
## 🧪 دستورات اعتبارسنجی
- `Manually test the Facilities page: Change search, type, status, min/max amount filters and observe if the table updates without clicking 'Filter'.`
- `npm run lint`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor, primarily related to ensuring correct data fetching logic and avoiding infinite re-renders.
...
1. Changing the search input or any filter dropdown/input on the Facilities page should trigger a data reload. _(verify: ui_interaction)_
2. The `eslint-disable-next-line react-hooks/exhaustive-deps` comment should be removed. _(verify: static)_
3. The `loadFacilities` function should be wrapped in `useCallback` and its dependencies correctly managed. _(verify: static)_
4. A `useRef` should be used to hold the latest filter values to maintain `loadFacilities` referential stability. _(verify: static)_

## Task Steps

### Step 1: اصلاح وابستگی‌های `useEffect` برای `loadFacilities` در صفحه Facilities
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ویرایش هوک `useEffect` موجود در فایل `frontend/src/app/facilities/page.tsx` است که مسئول فراخوانی تابع `loadFacilities` می‌باشد. هدف اصلی این است که اطمینان حاصل شود تمام متغیرهای حالت (state variables) شامل `search`, `fType`, `fStatus`, `amountMin`, و `amountMax` که مستقیماً توسط `loadFacilities` استفاده می‌شوند، به آرایه وابستگی‌های `useEffect` اضافه گردند. این تغییر باعث می‌شود که داده‌ها به طور خودکار با تغییر پارامترهای فیلتر مجدداً واکشی (re-fetch) شوند و تجربه کاربری پاسخگوتر و صحیح‌تری ارائه گردد. این مرحله شامل بازسازی تابع `loadFacilities`، تغییر عناصر UI مربوط به فیلترها، یا اصلاح منطق واکشی داده فراتر از فعال‌سازی صحیح آن نیست.
**Excerpt:**
```
The `useEffect` hook responsible for loading facilities (`loadFacilities`) in `frontend/src/app/facilities/page.tsx` does not include all its dependencies in its dependency array. Specifically, the `loadFacilities` function directly uses `search`, `fType`, `fStatus`, `amountMin`, and `amountMax` from component state. However, these state variables are not listed in the `useEffect`'s dependency array (`[page, sortBy, sortOrder]`). This means that when a user types in the search box or changes other filter dropdowns/inputs, the displayed data will not automatically update until pagination or sorting changes, or the 'Filter' button is explicitly clicked. This leads to a confusing and broken user experience where filters appear to be set but are not applied, potentially causing users to act on outdated information.
```
