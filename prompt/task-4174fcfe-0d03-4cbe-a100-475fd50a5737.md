---
task_id: 4174fcfe-0d03-4cbe-a100-475fd50a5737
title: تضاد build خروجی استاتیک Next.js در دو مسیر backend/static و frontend/out که هردو commit شده‌اند
type: refactor
priority: high
...
created_at: '2026-06-14T16:40:40.414749+00:00'
updated_at: '2026-06-14T16:40:40.414753+00:00'
target_files:
- backend/app/main.py
- backend/static/auth/callback/index.html
- frontend/out/auth/callback/index.html
---

# تضاد build خروجی استاتیک Next.js در دو مسیر backend/static و frontend/out که هردو commit شده‌اند

## Raw Idea

خروجی build فرانت‌اند (Next.js static export) هم در `backend/static/` و هم در `frontend/out/` به‌صورت کامل commit شده‌اند و محتوای آن‌ها duplicate است (مثلاً `backend/static/auth/callback/index.html` و `frontend/out/auth/callback/index.html` کاملاً یکسان هستند). این یک تضاد ساختاری است: artifactهای build نباید در ریپو نگه‌داری شوند چون: (۱) با هر deploy باید دوباره ساخته شوند، (۲) دو نسخه که می‌توانند از هم drift کنند، (۳) main.py فقط `static_dir = "static"` را mount می‌کند، پس `frontend/out` کاملاً بلااستفاده است و فقط نویز ریپو است. وجود همزمان این دو پوشه باعث می‌شود مشخص نباشد کدام منبعِ حقیقت برای deploy است و ریسک serve شدن نسخهٔ کهنه را افزایش می‌دهد.
...
- `backend/app/main.py:253-254` — `static_dir` — فقط backend/static را mount می‌کند؛ frontend/out هرگز استفاده نمی‌شود
  ```python
  # The static directory should contain the frontend build files.
  static_dir = "static"
  ```
- `backend/static/auth/callback/index.html:1` — `static build artifact` — duplicate کامل با frontend/out/auth/callback/index.html
  ```
  <!DOCTYPE html>...Signing you in…...
  ```
- `frontend/out/auth/callback/index.html:1` — `static build artifact (orphan)` — نسخهٔ موازی که هیچ‌جا serve نمی‌شود
  ```
  <!DOCTYPE html>...Signing you in…...
  ```
...
FastAPI StaticFiles mount + Next.js 14 static export

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/auth/callback/page.tsx` — منبع اصلی source که این HTMLها از آن build شده‌اند
- `.gitignore` — باید build artifacts را ignore کند
- `backend/Dockerfile` — باید مرحلهٔ build فرانت و کپی به static داشته باشد
...
- `backend/tests/test_postgres_regression.py` — این فایل `main.py` را import می‌کند (caller)
- `backend/tests/conftest.py` — این فایل `main.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
main.py تنها static را mount می‌کند؛ حذف frontend/out هیچ مصرف‌کننده‌ای را نمی‌شکند چون بلااستفاده است. backend/static باید توسط build pipeline تولید شود نه commit.
...
- [ ] پوشه‌های backend/static و frontend/out در .gitignore باشند
- [ ] Dockerfile یا CI مرحلهٔ build فرانت + کپی به backend/static داشته باشد
- [ ] git status بعد از build تغییری در ریپو نشان ندهد (artifacts ignore شوند)
- [ ] endpoint / هنوز SPA را با موفقیت serve کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. هر دو پوشهٔ `backend/static/` و `frontend/out/` را از version control حذف کن و به `.gitignore` اضافه کن. مرحلهٔ build در CI/Dockerfile باید فرانت‌اند را بسازد و خروجی را در زمان build داخل `backend/static` کپی کند (همان‌طور که در `mount_static_frontend` انتظار می‌رود). یک اسکریپت Makefile یا مرحلهٔ Docker اضافه کن که `next build && cp -r frontend/out/* backend/static/` را اجرا کند.
...
## 🧪 دستورات اعتبارسنجی
- `git check-ignore backend/static/index.html`
- `cd frontend && npm run build && ls ../backend/static/index.html`
- `curl -s localhost:8000/ | grep -q 'Banking Operations'`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر pipeline build درست تنظیم نشود، deploy بدون فرانت‌اند بالا می‌آید (main.py در production به ERROR لاگ می‌کند اما crash نمی‌کند).
...
1. پوشه‌های backend/static و frontend/out در .gitignore باشند
2. Dockerfile یا CI مرحلهٔ build فرانت + کپی به backend/static داشته باشد
3. git status بعد از build تغییری در ریپو نشان ندهد (artifacts ignore شوند)
4. endpoint / هنوز SPA را با موفقیت serve کند

## Task Steps

### Step 1: حذف artifactهای build از frontend/out/ و خارج کردن آن از ریپو
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل پوشهٔ `frontend/out/` از git tracking است (با `git rm -r --cached frontend/out` یا حذف فیزیکی فایل‌های commit‌شده) چون این پوشه کاملاً بلااستفاده است — main.py فقط `static_dir = "static"` را mount می‌کند و `frontend/out` هرگز serve نمی‌شود. خارج از این مرحله: حذف `backend/static/` (آن در مرحلهٔ جداگانه بررسی می‌شود چون منبع فعلیِ serve است). نکتهٔ حیاتی: نباید پوشهٔ source فرانت‌اند یا فایل‌های build config حذف شوند — فقط خروجیِ static export که duplicate و drift‌پذیر است.
**Excerpt:**
```
خروجی build فرانت‌اند (Next.js static export) هم در `backend/static/` و هم در `frontend/out/` به‌صورت کامل commit شده‌اند و محتوای آن‌ها duplicate است (مثلاً `backend/static/auth/callback/index.html` و `frontend/out/auth/callback/index.html` کاملاً یکسان هستند). ... main.py فقط `static_dir = "static"` را mount می‌کند، پس `frontend/out` کاملاً بلااستفاده است و فقط نویز ریپو است.
```

### Step 2: حذف artifactهای build از backend/static/ از git tracking
**Status:** `pending` (0%)
**Scope:** این مرحله شامل خارج کردن پوشهٔ `backend/static/` (خروجی Next.js static export کامیت‌شده) از git tracking است چون artifactهای build نباید در ریپو نگه‌داری شوند: (۱) با هر deploy باید دوباره ساخته شوند، (۲) ریسک serve شدن نسخهٔ کهنه. خارج از این مرحله: حذف خود مکانیزم mount در main.py — `static_dir = "static"` باید کماکان مسیر معتبری برای زمان deploy باشد (پوشه در زمان build دوباره ساخته می‌شود). نکتهٔ حیاتی: چون main.py این پوشه را serve می‌کند، باید مطمئن شد که pipeline build این پوشه را در زمان deploy تولید می‌کند پیش از حذف از ریپو.
**Excerpt:**
```
artifactهای build نباید در ریپو نگه‌داری شوند چون: (۱) با هر deploy باید دوباره ساخته شوند، (۲) دو نسخه که می‌توانند از هم drift کنند، (۳) main.py فقط `static_dir = "static"` را mount می‌کند ... وجود همزمان این دو پوشه باعث می‌شود مشخص نباشد کدام منبعِ حقیقت برای deploy است و ریسک serve شدن نسخهٔ کهنه را افزایش می‌دهد.
```

### Step 3: افزودن مسیرهای build artifact به .gitignore برای جلوگیری از کامیت مجدد
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن `backend/static/` و `frontend/out/` به فایل `.gitignore` است تا پس از حذف، این artifactهای build دیگر به‌صورت تصادفی دوباره commit نشوند. خارج از این مرحله: تغییر در ساختار build یا منطق serve. نکتهٔ حیاتی: باید دقت شود که اگر فایل‌های ضروری غیر-build (مثل placeholder یا فایل‌های نگه‌دارندهٔ ساختار) در این پوشه‌ها وجود دارند، استثنا شوند؛ ولی طبق توضیح کاربر کل محتوا خروجی Next.js static export است و duplicate.
**Excerpt:**
```
این یک تضاد ساختاری است: artifactهای build نباید در ریپو نگه‌داری شوند چون: (۱) با هر deploy باید دوباره ساخته شوند، (۲) دو نسخه که می‌توانند از هم drift کنند ... وجود همزمان این دو پوشه باعث می‌شود مشخص نباشد کدام منبعِ حقیقت برای deploy است.
```

### Step 4: تأیید منبع حقیقت deploy و مستندسازی فرایند build خروجی static
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و تأیید اینکه پس از حذف artifactها، فرایند deploy همچنان `backend/static/` را به‌درستی از طریق build تولید و سرور آن را از `static_dir = "static"` در main.py serve می‌کند. باید مستند یا verify شود که چه command/pipeline‌ای خروجی Next.js static export را در `backend/static/` می‌سازد تا منبع حقیقت deploy واضح باشد. خارج از این مرحله: نوشتن CI جدید از صفر اگر موجود نیست. نکتهٔ حیاتی: باید تضمین شود که ریسک serve شدن نسخهٔ کهنه از بین رفته و فقط یک منبع حقیقت (build تازه) باقی مانده است.
**Excerpt:**
```
main.py فقط `static_dir = "static"` را mount می‌کند ... وجود همزمان این دو پوشه باعث می‌شود مشخص نباشد کدام منبعِ حقیقت برای deploy است و ریسک serve شدن نسخهٔ کهنه را افزایش می‌دهد.
```
