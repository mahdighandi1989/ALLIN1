---
task_id: 074dddf5-f902-4211-bc3a-567cdf4947a0
title: 'Anti-pattern: Stale assumption / Under-engineering'
type: bug_fix
priority: high
...
created_at: '2026-06-07T16:55:02.764428+00:00'
updated_at: '2026-06-07T22:06:43.512156+00:00'
target_files:
- backend/app/models/facility.py
---

# Anti-pattern: Stale assumption / Under-engineering

## Raw Idea

The `tenor_months` column is defined as `String(4)`. The name 'tenor_months' strongly implies a numerical value (e.g., '12', '24', '36'). Storing numerical data as a string prevents proper numerical operations (sorting, aggregation, range queries) and can lead to data integrity issues if non-numeric strings are accidentally stored. If it's always a number, it should be `Integer`. If it's meant to 

📁 file: backend/app/models/facility.py (line 60)
...
- `backend/app/models/facility.py:60`
...
- `backend/app/utils/id_generator.py` — `facility.py` این فایل را import می‌کند
- `backend/app/models/enum_utils.py` — `facility.py` این فایل را import می‌کند
- `backend/app/models/__init__.py` — این فایل `facility.py` را import می‌کند (caller)
- `backend/app/services/facility_authorization.py` — این فایل `facility.py` را import می‌کند (caller)
- `backend/app/services/snapshots.py` — این فایل `facility.py` را import می‌کند (caller)
