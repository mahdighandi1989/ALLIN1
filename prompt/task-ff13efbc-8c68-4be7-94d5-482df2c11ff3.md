---
task_id: ff13efbc-8c68-4be7-94d5-482df2c11ff3
...
The type validation in `update_settings` is very basic, only checking for `number` (as `int`). `EDITABLE_SETTINGS` implies other types exist (e.g., `string`, `boolean`). Storing all values as strings in the DB and not performing conversions on read or write for other types means the `type` metadata is largely decorative for the API consumer, requiring them to handle conversions. This leads to inco

📁 file: backend/app/routers/settings.py (line 65)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-07T16:55:02.729501+00:00]
The `EDITABLE_SETTINGS` constant defines a `type` for each setting, but the `update_settings` endpoint only implements specific validation logic for `type == "number"`. This creates an inconsistency where the schema declares types, but the enforcement logic only partially covers them. Other types ar
---
[scan #3 at 2026-06-07T16:55:02.741026+00:00]
The `SystemSetting.value` field is always stored as a string (`str(value)`), even if `EDITABLE_SETTINGS` defines its type as 'number'. While `update_settings` validates numeric input as `int`, it stores it as `str`. Consequently, `get_settings` and the return of `update_settings` retrieve and return
---
[scan #4 at 2026-06-07T16:55:02.752436+00:00]
The 'facilities' relationship uses `cascade="all, delete-orphan"`. The `delete-orphan` cascade rule is very aggressive and implies that any `Facility` object removed from a `Customer`'s `facilities` collection (or whose `customer_id` is set to `None`) will be automatically deleted from the database.
...
- `backend/app/routers/settings.py:65`
...
- `backend/app/database.py` — `settings.py` این فایل را import می‌کند
- `backend/app/config.py` — `settings.py` این فایل را import می‌کند
- `backend/app/models/system_setting.py` — `settings.py` این فایل را import می‌کند
- `backend/app/routers/auth.py` — `settings.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `settings.py` را import می‌کند (caller)
- `backend/app/utils/security.py` — این فایل `settings.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/routers/settings.py`
- `ruff check backend/app/routers/settings.py`
- `pytest -x`
