...
updated_at: '2026-06-07T16:53:26.961284+00:00'
target_files:
...
1. JWTs are stored in `HttpOnly` and `Secure` cookies on the client-side.
2. Frontend JavaScript cannot directly access the JWT (e.g., `localStorage.getItem('accessToken')` returns null).
3. Backend authentication endpoints correctly set these cookies upon successful login.
4. All authenticated API requests automatically send the cookies without manual JavaScript intervention.
5. XSS vulnerabilities for token theft are mitigated.
