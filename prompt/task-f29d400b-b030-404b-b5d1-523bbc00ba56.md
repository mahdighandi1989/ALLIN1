---
task_id: f29d400b-b030-404b-b5d1-523bbc00ba56
title: Lack of explicit frontend token storage strategy, potential XSS vulnerability.
type: security
priority: high
...
created_at: '2026-06-07T16:53:26.961281+00:00'
updated_at: '2026-06-07T22:04:28.540482+00:00'
target_files:
...
# Lack of explicit frontend token storage strategy, potential XSS vulnerability.

## Raw Idea

The frontend application (`Next.js`) relies on JWT for authentication. While the backend's JWT handling is robust (as per `docs/SECURITY.md`), the method of storing the JWT on the client-side is not explicitly defined or visible in the provided frontend code snippets. If JWTs are stored in `localStorage` or `sessionStorage`, they are vulnerable to Cross-Site Scripting (XSS) attacks, where a malicious script injected into the page could steal the token. The most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
...
- `frontend/src/lib/api.ts:7` — `customersApi` — This file (implied) likely contains the Axios client configuration, where token attachment logic resides.
  ```tsx
  7: import { customersApi, parseApiError, downloadFile } from '@/lib/api'
  ```
- `backend/app/routers/auth.py:184` — `auth.router` — The authentication router is responsible for issuing JWTs, which should be set as HttpOnly cookies.
  ```python
  184: app.include_router(auth.router, prefix="/api/auth", tags=["auth"])
  ```
- `frontend/src/app/login/page.tsx:entry` — `LoginPage` — The login page (implied from static files) handles successful authentication and would need to be updated to reflect the new token storage method.
  ```tsx
  entry  backend/static/login/index.html
  ```
...
Next.js 14, React 18, FastAPI, JWT.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/utils/security.py` — Contains JWT verification logic, might need adjustments for cookie-based tokens.
- `docs/SECURITY.md` (سطر 1) — Documents backend JWT hardening, but client-side storage is a related concern.
- `frontend/src/lib/axios.ts` — `api.ts` این فایل را import می‌کند
- `frontend/src/lib/auth.tsx` — این فایل `api.ts` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
Core authentication mechanism, impacts both frontend and backend token management.
...
- [ ] JWTs are stored in `HttpOnly` and `Secure` cookies on the client-side.
- [ ] Frontend JavaScript cannot directly access the JWT (e.g., `localStorage.getItem('accessToken')` returns null).
- [ ] Backend authentication endpoints correctly set these cookies upon successful login.
- [ ] All authenticated API requests automatically send the cookies without manual JavaScript intervention.
- [ ] XSS vulnerabilities for token theft are mitigated.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a secure client-side token storage strategy. The recommended approach is to store JWTs in `HttpOnly` and `Secure` cookies. This requires the backend (`backend/app/routers/auth.py`) to set these cookies upon successful authentication and the frontend (`frontend/src/lib/api.ts`, `frontend/src/app/login/page.tsx`) to rely on the browser automatically sending them with API requests, rather than manually attaching them from JavaScript. If `localStorage` is currently used, migrate to `HttpOnly` cookies.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Inspect browser developer tools (Application tab, Cookies) after login to verify `HttpOnly` and `Secure` flags on the JWT cookie.`
- `Attempt an XSS attack (e.g., inject `<script>alert(localStorage.getItem('accessToken'))</script>`) to confirm token inaccessibility.`
- `Run frontend E2E tests for login/logout and authenticated API calls to ensure functionality.`

## ⚠️ ریسک‌ها و موارد احتیاط
High, requires changes to authentication flow on both frontend and backend, potential for breaking existing authentication if not carefully implemented.
...
1. JWTs are stored in `HttpOnly` and `Secure` cookies on the client-side. _(verify: static)_
2. Frontend JavaScript cannot directly access the JWT (e.g., `localStorage.getItem('accessToken')` returns null). _(verify: ui_interaction)_
3. Backend authentication endpoints correctly set these cookies upon successful login. _(verify: api_response)_
4. All authenticated API requests automatically send the cookies without manual JavaScript intervention. _(verify: ui_interaction)_
5. XSS vulnerabilities for token theft are mitigated. _(verify: manual_only)_

## Task Steps

### Step 1: بررسی وضعیت فعلی ذخیره‌سازی JWT در فرانت‌اند Next.js
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق کدبیس فرانت‌اند `Next.js` برای شناسایی نحوه فعلی ذخیره‌سازی و دسترسی به JWTها است. هدف اصلی، تأیید اینکه آیا `localStorage` یا `sessionStorage` برای نگهداری توکن‌ها استفاده می‌شود یا خیر، و مستندسازی وضعیت موجود است. این مرحله هیچ تغییری در کد ایجاد نمی‌کند، بلکه صرفاً به کشف و ارزیابی می‌پردازد.
**Excerpt:**
```
The frontend application (`Next.js`) relies on JWT for authentication. While the backend's JWT handling is robust (as per `docs/SECURITY.md`), the method of storing the JWT on the client-side is not explicitly defined or visible in the provided frontend code snippets. If JWTs are stored in `localStorage` or `sessionStorage`, they are vulnerable to Cross-Site Scripting (XSS) attacks, where a malicious script injected into the page could steal the token.
```

### Step 2: پیکربندی بک‌اند برای صدور کوکی‌های HttpOnly و Secure
**Status:** `pending` (0%)
**Scope:** این مرحله بر تأیید و در صورت لزوم، پیکربندی بک‌اند تمرکز دارد تا JWTها را به عنوان کوکی‌های `HttpOnly` و `Secure` صادر کند. اطمینان حاصل می‌شود که بک‌اند هنگام ارسال توکن‌های احراز هویت به کلاینت، این پرچم‌های کوکی را به درستی تنظیم می‌کند. این گام به عنوان یک پیش‌نیاز حیاتی برای فرانت‌اند جهت اتخاذ روش ذخیره‌سازی امن جدید عمل می‌کند.
**Excerpt:**
```
While the backend's JWT handling is robust (as per `docs/SECURITY.md`), the most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
```

### Step 3: تطبیق فرانت‌اند Next.js برای استفاده از JWTهای مبتنی بر کوکی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح برنامه فرانت‌اند `Next.js` است تا JWTهایی را که اکنون توسط بک‌اند به عنوان کوکی‌های `HttpOnly` و `Secure` ارسال و مدیریت می‌شوند، به درستی مدیریت کند. این شامل به‌روزرسانی هر منطق سمت کلاینت است که قبلاً JWTها را در `localStorage` یا `sessionStorage` انتظار داشت، تا اکنون به مدیریت خودکار این کوکی‌ها توسط مرورگر برای درخواست‌های احراز هویت شده تکیه کند. فرانت‌اند دیگر نباید سعی کند JWT را مستقیماً از JavaScript سمت کلاینت بخواند.
**Excerpt:**
```
The most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
```

### Step 4: حذف ذخیره‌سازی ناامن JWT در سمت کلاینت
**Status:** `pending` (0%)
**Scope:** این مرحله به طور خاص حذف هر کد موجود در فرانت‌اند `Next.js` را هدف قرار می‌دهد که JWTها را با استفاده از `localStorage` یا `sessionStorage` ذخیره، بازیابی یا دستکاری می‌کند. این یک گام پاکسازی و تقویت امنیتی است که اطمینان حاصل می‌کند روش‌های ذخیره‌سازی آسیب‌پذیر به طور کامل از کدبیس حذف می‌شوند. این فرض می‌کند که مرحله ۱ چنین استفاده‌ای را شناسایی کرده است.
**Excerpt:**
```
If JWTs are stored in `localStorage` or `sessionStorage`, they are vulnerable to Cross-Site Scripting (XSS) attacks, where a malicious script injected into the page could steal the token.
```

### Step 5: به‌روزرسانی جریان احراز هویت در فرانت‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تنظیم جریان کلی احراز هویت در فرانت‌اند `Next.js` است تا به طور یکپارچه با JWTهای مبتنی بر کوکی `HttpOnly` و `Secure` جدید ادغام شود. این ممکن است شامل اصلاح فرآیندهای ورود/خروج، مدیریت نشست، و مدیریت خطا باشد تا وضعیت احراز هویت را بر اساس وجود و اعتبار کوکی امن، به جای توکن‌های ذخیره شده در سمت کلاینت، به درستی منعکس کند.
**Excerpt:**
```
The frontend application (`Next.js`) relies on JWT for authentication. The most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
```

### Step 6: تست جامع جریان احراز هویت
**Status:** `pending` (0%)
**Scope:** این مرحله شامل انجام تست‌های گسترده برای اعتبارسنجی کل سیستم احراز هویت و مجوز پس از پیاده‌سازی تغییرات کوکی `HttpOnly` و `Secure` است. این شامل تست‌های عملکردی برای ورود، خروج، دسترسی به مسیرهای محافظت شده، و تست‌های امنیتی خاص برای تأیید اینکه JWTها واقعاً برای JavaScript سمت کلاینت غیرقابل دسترسی هستند و آسیب‌پذیری‌های XSS مربوط به سرقت توکن کاهش یافته‌اند.
**Excerpt:**
```
The frontend application (`Next.js`) relies on JWT for authentication. The most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
```

### Step 7: به‌روزرسانی مستندات امنیتی
**Status:** `pending` (0%)
**Scope:** این مرحله مستلزم به‌روزرسانی فایل `docs/SECURITY.md` و احتمالاً سایر مستندات مرتبط است تا استراتژی جدید ذخیره‌سازی و مدیریت JWT را به دقت منعکس کند. مستندات باید به وضوح بیان کنند که JWTها اکنون در کوکی‌های `HttpOnly` و `Secure` ذخیره می‌شوند و مزایای امنیتی و نحوه کاهش حملات XSS را توضیح دهند.
**Excerpt:**
```
While the backend's JWT handling is robust (as per `docs/SECURITY.md`), the method of storing the JWT on the client-side is not explicitly defined or visible in the provided frontend code snippets. The most secure approach for JWTs in a browser environment is often using `HttpOnly` and `Secure` cookies, which are inaccessible to client-side JavaScript, mitigating XSS token theft.
```
