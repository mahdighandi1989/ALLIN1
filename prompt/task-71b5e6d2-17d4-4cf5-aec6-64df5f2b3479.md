...
updated_at: '2026-06-07T16:51:46.879641+00:00'
target_files:
...
1. File uploads exceeding 50MB are rejected with an appropriate HTTP status code (e.g., 413 Payload Too Large).
2. Files with incorrect content type (e.g., an executable renamed to `.xlsx`) are rejected based on magic bytes or content analysis.
3. All uploaded files are scanned for viruses, and infected files are rejected.
4. Appropriate, user-friendly error messages are returned for all security violations.
5. Unit and integration tests are added to verify each of these security checks.
