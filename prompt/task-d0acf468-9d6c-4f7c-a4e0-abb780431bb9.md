---
task_id: d0acf468-9d6c-4f7c-a4e0-abb780431bb9
title: 'ناهماهنگی مسیر OAuth callback: کد بک‌اند به /auth/callback ریدایرکت می‌کند اما توکن در همان‌جا خوانده می‌شود در حالی که axios باید آن را به هدر اضافه کند — نیاز به تأیید تطابق روت بک‌اند google_auth ب'
type: bug
priority: high
...
created_at: '2026-06-14T16:37:33.896281+00:00'
updated_at: '2026-06-14T16:37:33.896284+00:00'
target_files:
- frontend/src/app/auth/callback/page.tsx
- backend/app/routers/google_auth.py
---

# ناهماهنگی مسیر OAuth callback: کد بک‌اند به /auth/callback ریدایرکت می‌کند اما توکن در همان‌جا خوانده می‌شود در حالی که axios باید آن را به هدر اضافه کند — نیاز به تأیید تطابق روت بک‌اند google_auth ب

## Raw Idea

صفحهٔ فرانت‌اند `frontend/src/app/auth/callback/page.tsx` انتظار دارد بک‌اند پس از تبادل OAuth code، مرورگر را به `/auth/callback?token=<jwt>&role=<role>` ریدایرکت کند (طبق کامنت خطوط ۵-۱۲). این صفحه `token` و `error` را از query string می‌خواند. باید تأیید شود که روتر بک‌اند `backend/app/routers/google_auth.py` (mount شده روی `/api/auth/google`) دقیقاً به همین مسیر نسبی `/auth/callback` با همین نام پارامترها (`token`, `role`, `error`) ریدایرکت می‌کند. هرگونه mismatch در نام پارامتر (مثلاً `access_token` به‌جای `token`) یا مسیر (مثلاً `/dashboard` مستقیم) باعث می‌شود ورود با گوگل به‌صورت خاموش شکست بخورد و کاربر به `/login?error=missing_token` پرتاب شود. در فهرست فایل‌ها روتر google_auth.py وجود دارد ولی محتوای آن نمایش داده نشده، پس این تطابق باید بررسی و قفل شود.
...
- `frontend/src/app/auth/callback/page.tsx:16-41` — `AuthCallbackPage.useEffect` — قرارداد فرانت: انتظار پارامترهای token و error در query string مسیر /auth/callback
  ```tsx
  ...
  ```
- `backend/app/routers/google_auth.py:0-0` — `google_callback` — endpoint callback که باید با نام‌پارامتر و مسیر فرانت هم‌تراز باشد
  ```python
  # باید بررسی شود: RedirectResponse به /auth/callback?token=...&role=...
  ```
...
FastAPI + Google OAuth 2.0 + Next.js 14 App Router (static export)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 228) — روتر google_auth با prefix /api/auth/google مونت شده
- `frontend/src/app/settings/page.tsx` (سطر 62) — connectDrive نیز از مسیر /api/auth/google/drive/connect استفاده می‌کند و قرارداد redirect مشابه دارد
- `backend/app/config.py` — `google_auth.py` این فایل را import می‌کند
- `backend/app/routers/settings.py` — `google_auth.py` این فایل را import می‌کند
- `backend/app/database.py` — `google_auth.py` این فایل را import می‌کند
- `backend/app/models/user.py` — `google_auth.py` این فایل را import می‌کند
- `backend/tests/test_google_auth.py` — این فایل `google_auth.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این flow توسط صفحهٔ login، AuthProvider و روتر google_auth استفاده می‌شود؛ هر mismatch کل ورود با گوگل را می‌شکند.
...
- [ ] endpoint GET /api/auth/google/callback پس از موفقیت به /auth/callback?token=<jwt>&role=<role> ریدایرکت می‌کند
- [ ] در خطا به /auth/callback?error=<code> یا /login?error=<code> با پارامتر error ریدایرکت می‌شود
- [ ] نام پارامتر دقیقاً token و error است (نه access_token/err)
- [ ] تست integration برای callback نام پارامتر redirect را بررسی می‌کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. محتوای `backend/app/routers/google_auth.py` را بررسی کن و مطمئن شو endpoint callback (`GET /api/auth/google/callback`) دقیقاً با `RedirectResponse` به `/auth/callback?token=<jwt>&role=<role>` (مسیر نسبی فرانت) ریدایرکت می‌کند و در خطا به `/auth/callback?error=<code>` یا `/login?error=<code>`. نام پارامتر باید دقیقاً `token` و `error` باشد تا با خطوط ۱۹ و ۲۶ صفحهٔ فرانت بخواند. اگر mismatch هست، بک‌اند را با مسیر/نام‌های فرانت هم‌تراز کن (فرانت ground truth برای قرارداد redirect است).
...
_قبل:_
```
return RedirectResponse(f"/dashboard?access_token={jwt}")
```

_بعد:_
```
return RedirectResponse(f"/auth/callback?token={jwt}&role={role}")
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/integration/test_auth.py -k google`
- `grep -n 'auth/callback' backend/app/routers/google_auth.py`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر مسیر redirect قبلاً درست بوده، تغییر بی‌مورد ممکن است flow سالم را بشکند — ابتدا بررسی، سپس اصلاح
...
1. endpoint GET /api/auth/google/callback پس از موفقیت به /auth/callback?token=<jwt>&role=<role> ریدایرکت می‌کند
2. در خطا به /auth/callback?error=<code> یا /login?error=<code> با پارامتر error ریدایرکت می‌شود
3. نام پارامتر دقیقاً token و error است (نه access_token/err)
4. تست integration برای callback نام پارامتر redirect را بررسی می‌کند

## Task Steps

### Step 1: بررسی و خواندن قرارداد فرانت‌اند در page.tsx برای کشف پارامترهای مورد انتظار
**Status:** `pending` (0%)
**Scope:** در این مرحله فایل `frontend/src/app/auth/callback/page.tsx` خوانده می‌شود تا دقیقاً مشخص شود چه پارامترهایی از query string خوانده می‌شوند و چه مسیری مورد انتظار است. طبق کامنت خطوط ۵-۱۲، صفحه انتظار دارد بک‌اند مرورگر را به `/auth/callback?token=<jwt>&role=<role>` ریدایرکت کند و این صفحه `token` و `error` را از query string می‌خواند. این مرحله فقط بررسی و مستندسازی قرارداد سمت فرانت است؛ هیچ تغییری در فرانت‌اند انجام نمی‌شود و تطبیق با بک‌اند خارج از این مرحله است (مرحلهٔ بعد). نکتهٔ حیاتی: نام دقیق پارامترها (`token`, `role`, `error`) و مسیر نسبی `/auth/callback` باید verbatim ثبت شوند تا مبنای مقایسه باشند.
**Excerpt:**
```
صفحهٔ فرانت‌اند `frontend/src/app/auth/callback/page.tsx` انتظار دارد بک‌اند پس از تبادل OAuth code، مرورگر را به `/auth/callback?token=<jwt>&role=<role>` ریدایرکت کند (طبق کامنت خطوط ۵-۱۲). این صفحه `token` و `error` را از query string می‌خواند.
```

### Step 2: بررسی محتوای روتر google_auth.py و تأیید تطابق مسیر و نام پارامترهای ریدایرکت با قرارداد فرانت
**Status:** `pending` (0%)
**Scope:** در این مرحله فایل `backend/app/routers/google_auth.py` (که روی `/api/auth/google` mount شده) خوانده و بررسی می‌شود تا تأیید شود پس از تبادل OAuth code، مرورگر دقیقاً به مسیر نسبی `/auth/callback` با همان نام پارامترها (`token`, `role`, `error`) ریدایرکت می‌شود. باید هرگونه mismatch شناسایی شود: مثلاً اگر بک‌اند از `access_token` به‌جای `token` استفاده کرده، یا مستقیماً به `/dashboard` ریدایرکت کرده باشد. چنین mismatch‌هایی باعث می‌شود ورود با گوگل به‌صورت خاموش شکست بخورد و کاربر به `/login?error=missing_token` پرتاب شود. نکتهٔ حیاتی: محتوای فایل در فهرست repo نمایش داده نشده بود، پس ابتدا باید grep/خواندن انجام شود؛ اگر تطابق برقرار است یک کامیت no-op توضیحی ثبت شود، و اگر mismatch وجود دارد همان نقطه اصلاح و «قفل» شود تا نام پارامترها و مسیر دقیقاً مطابق فرانت باشند.
**Excerpt:**
```
باید تأیید شود که روتر بک‌اند `backend/app/routers/google_auth.py` (mount شده روی `/api/auth/google`) دقیقاً به همین مسیر نسبی `/auth/callback` با همین نام پارامترها (`token`, `role`, `error`) ریدایرکت می‌کند. هرگونه mismatch در نام پارامتر (مثلاً `access_token` به‌جای `token`) یا مسیر (مثلاً `/dashboard` مستقیم) باعث می‌شود ورود با گوگل به‌صورت خاموش شکست بخورد و کاربر به `/login?error=missing_token` پرتاب شود. در فهرست فایل‌ها روتر google_auth.py وجود دارد ولی محتوای آن نمایش داده نشده، پس این تطابق باید بررسی و قفل شود.
```
