...
- amount: NUMERIC(15, 2) NOT NULL  -- این ستون برای ذخیره مبلغ تسهیلات است و باید در دیتابیس وجود داشته باشد
- currency: VARCHAR
...
## 🛠️ Notes:
- ستون `amount` در جدول `facilities` برای محاسبه جمع کل در داشبورد استفاده می‌شود. عدم وجود این ستون در دیتابیس باعث خطای 500 در endpoint آمار داشبورد می‌شود.
- اطمینان حاصل کنید که مدل‌های backend با این طرح دیتابیس هماهنگ باشند و مهاجرت‌های پایگاه داده به درستی اجرا شده باشند.
- برای رفع خطاهای گزارش شده، باید مطمئن شوید که ستون `amount` در جدول `facilities` در پایگاه داده وجود دارد.