...
TOOLS: Dict[str, Dict[str, str]] = {
    ...
    "paragraphs": {
        ...
        "guide": (
            "ترتیب و انسجام پاراگراف‌ها، جملات بریده یا تکراری را بهبود بده؛ فقط با text_replace روی "
            "متنِ موجود. اگر جمله‌های یک موضوع در متن پراکنده‌اند یا ترتیبِ منطقی ندارند، بازچینش کن: "
            "برای هر پاراگراف یک text_replace بده که نسخهٔ مرتب و منسجمِ همان محتوا را بگذارد (جملهٔ "
            "جابه‌جاشده باید از جای قبلی‌اش هم برداشته شود — با یک text_replace جدا برای همان جای قبلی). "
            "چیزی از محتوای واقعی حذف یا اضافه نکن؛ فقط جای‌گذاری و پیوند (category=paragraphs)."
        ),
    },
    "tables": {
...
SYSTEM_PROMPT = (
    ...
    "ظاهر مرتب: سرستونِ روشن، ترازبندی و عرضِ ستون‌ها با style).\n"
    ...
    "بازسازی کن و هر پاراگرافِ نهایی را با یک text_replace روی معادلِ فعلی‌اش بده؛ ترتیبِ منطقی "
    "(مقدمه/استناد، اصلِ درخواست، اقدامِ خواسته‌شده، پایان‌بندی) را رعایت کن. معنا، مبالغ، تاریخ‌ها و "
