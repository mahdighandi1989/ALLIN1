...
## سقفِ اندازهٔ بدنهٔ درخواست (multipart) — ثبت 2026-08-08

بازبینیِ اجراییِ v103 نشان داد Starlette 0.27 فیلدهای غیرفایلیِ فرم
(`instructions`، `model_id`، …) را پیش از هر سقفِ منطقی کاملاً در RAM
بافر می‌کند (`max_part_size` در این نسخه وجود ندارد) و middleware فعلی
سقفِ content-length ندارد؛ یعنی یک فیلدِ فرمِ چند-مگابایتی پشتِ auth
ویرایشگر حافظه مصرف می‌کند (کلاسِ مشکل از قبل موجود بود، v103 فقط
مستندش کرد؛ سقفِ منطقیِ ۶۰۰۰ کاراکتر بعد از parse اعمال می‌شود).
اقدامِ پیشنهادی: سقفِ body-size در middleware (مثلاً 64MB همسو با سقفِ
PDF) یا ارتقایِ Starlette با `max_part_size` — تغییرِ حساسِ زیرساختی،
جدا از v103 انجام شود.
